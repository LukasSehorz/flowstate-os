    ...
    const { rows } = await q(
      `update content_ideen
          set status = $2, entschieden_am = now(), entschieden_von = $3,
              -- Von Hand abgelehnt heisst: soll nicht wiederkommen. Der
              -- Abendlauf schreibt hier stattdessen 'ersetzt' — das ist ein
              -- anderer Fall und darf nicht im selben Stapel landen.
              verworfen_art = case when $2 = 'verworfen' then 'hand' else null end
        where id = $1
...
// Eine verworfene Idee zurueck in die Entscheidung holen.
//
// Der eigentliche Sinn des Archivs: wenn spaeter mehr produziert werden kann,
// muss man an die zurueckgestellten Ideen wieder herankommen. Ohne diesen Weg
// waere "bleibt stehen" nur eine hoefliche Umschreibung fuer geloescht.
//
// Die Idee landet wieder bei den Vorschlaegen, nicht direkt bei den
// angenommenen: sie soll dieselbe Entscheidung durchlaufen wie jede andere.
module.exports.zurueckholen = async function (user, id) {
  return alsNutzer(user.id, async (q) => {
    const { rows } = await q(
      `update content_ideen
          set status = 'vorschlag', verworfen_art = null,
              entschieden_am = null, entschieden_von = null
        where id = $1 and status = 'verworfen'
        returning *`, [id]);
    if (!rows.length) return { ok: false, grund: "nicht-verworfen" };
    return { ok: true, idee: rows[0] };
  });
};
...
async function bisher(user, marke) {
  return alsNutzer(user.id, async (q) => {
    const { rows } = await q(
      // Von Hand Abgelehntes bleibt 60 Tage gesperrt — es soll nicht
      // wiederkommen. Zurueckgestelltes ('ersetzt') nur 14 Tage: diese Ideen
      // waren brauchbar, und nach zwei Wochen darf das Modell in die Richtung
      // wieder denken, wenn sie inzwischen besser passt.
      `select titel, status from content_ideen
        where ($1::uuid is null or marke = $1)
          and (
            (verworfen_art = 'ersetzt' and erstellt >= now() - interval '14 days')
            or (coalesce(verworfen_art, 'hand') <> 'ersetzt'
...
# Recherchiere zuerst
Du hast eine Websuche. Nutze sie, bevor du etwas vorschlägst:
- Welche Formate tragen auf ${kanal} gerade in vergleichbaren Branchen
  (Handwerk, Praxen, lokale Dienstleister, Agenturen)?
- Was schreiben Leute über Hooks und Aufbau, die zurzeit funktionieren?
- Gibt es aktuelle Änderungen an der Verteilung, die den Aufbau beeinflussen?

Du siehst dabei NUR öffentliche Seiten — keine Zahlen aus ${kanal} selbst.
Behandle Gelesenes darum als Hinweis, nicht als Messung, und stelle die eigenen
gemessenen Zahlen weiter oben darüber, wenn beides sich widerspricht.
Erfinde keine Quelle und keinen Videotitel, den du nicht wirklich gelesen hast.
...
module.exports.ideenErzeugen = async function (user, { marke, kanal = "TikTok", rubrik = "ki" } = {}) {
  ...
  try {
    // Websuche laeuft auf Anthropics Seite — nichts wird bei uns ausgefuehrt.
    // Damit recherchiert der Lauf tatsaechlich, statt nur aus dem Gedaechtnis
    // des Modells zu schoepfen: oeffentliche Artikel, Fachbeitraege und
    // Erklaerinhalte darueber, welche Kurzvideo-Formate gerade tragen.
    //
    // WAS DAS NICHT IST: kein Einloggen bei TikTok oder Instagram und kein
    // Abgreifen ihrer Zahlen. Das gaebe es weder als Schnittstelle, noch waere
    // es nach deren Bedingungen erlaubt. Gelesen wird, was oeffentlich im Netz
    // steht — also worueber geschrieben wird, nicht was gemessen wurde.
    const werkzeuge = [{ type: "web_search_20260209", name: "web_search", max_uses: 8 }];

    // Serverseitige Werkzeuge laufen in Runden. Sind nach zehn Runden noch
    // Suchen offen, meldet die Antwort "pause_turn" statt fertig zu sein —
    // dann wird der bisherige Verlauf zurueckgeschickt und weitergemacht.
    // Ohne das kaeme in dem Fall gar kein Text zurueck und der Lauf saehe
    // aus wie ein Fehlschlag.
    let nachrichten = [{ role: "user", content: auftrag({ kanal, rubrik, zahlen, frueher, anzahl: 3 }) }];
    let antwort = null;
    ...
    if (antwort.stop_reason === "pause_turn") return { ok: false, grund: "suche-zu-lang" };
    const text = (antwort.content || []).filter((b) => b.type === "text").map((b) => b.text).join("");
    if (!text.trim()) return { ok: false, grund: "leer" };
    // Wie viele Quellen tatsaechlich gelesen wurden — steht spaeter auf der
    // Seite, damit man einer Idee ansieht, ob sie recherchiert oder geraten ist.
    const quellen = (antwort.content || [])
      ...
      for (const stufe of stufen) {
        ...
        const { rows } = await q(
          ...
          [marke || user.id, kanal, rubrik, stufe,
           ...
           // Beim Beispiel steht jetzt auch, auf wie vielen gelesenen Quellen
           // der Lauf fusst — sonst sieht man einer Idee nicht an, ob sie
           // recherchiert oder aus dem Gedaechtnis geschrieben ist.
           [String(i.beispiel || "").trim(),
            quellen.length ? "(recherchiert: " + quellen.length + " Quellen gelesen)" : ""]
             .filter(Boolean).join(" ") || null]);
        angelegt.push(rows[0].id);
      }
    });
    if (!angelegt.length) return { ok: false, grund: "keine-verwertbare-idee" };
    return { ok: true, anzahl: angelegt.length, ids: angelegt, quellen: quellen.length };
  } catch (fehler) {
...
Wähle genau ${behalten} aus und nenne ihre Kennungen.` }],
    ...
    await alsNutzer(user.id, async (q) => {
      for (const r of raus) {
        // 'ersetzt' statt eines Textanhangs in warum: so laesst sich der Stapel
        // spaeter filtern und durchsuchen. Diese Ideen waren nicht schlecht,
        // nur die schwaecheren von zweien — sie sind der Vorrat fuer Wochen,
        // in denen mehr produziert werden kann.
        await q(`update content_ideen set status = 'verworfen', entschieden_am = now(),
                        verworfen_art = 'ersetzt',
                        warum = coalesce(warum, '') || $2
                  where id = $1`,
          [r.id, "\n\nAm " + new Date().toLocaleDateString("de-DE") +
                 " zugunsten eines stärkeren Vorschlags zurückgestellt: " +
                 String(begruendung).slice(0, 300)]);
      }
    });
