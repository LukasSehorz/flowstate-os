...
function spanne(vonTag, bisTag, max = 200) {
  // Der kurze Weg zuerst: direkt bei Google statt ueber gws-cli.
  //
  // Gemessen am 20.08.2026: gws-cli braucht fuer EINEN Tag 9,1 Sekunden und
  // beim zweiten Mal 9,4 — es startet jedes Mal ein Python-Programm neu und
  // entschluesselt den Zugang. Die Google-API antwortet in Millisekunden.
  //
  // Faellt der kurze Weg aus (kein Token, Google zickt), geht es unveraendert
  // ueber gws-cli weiter. Der lange Weg bleibt also die Rueckfallebene und
  // wird nicht schlechter, nur seltener gebraucht.
  const direkt = require("./kalender-direkt.js");
  if (direkt.bereit()) {
    const vonD = alsDate(vonTag), bisD = alsDate(tagPlus(bisTag, 1));
    return direkt.termine(
      `${vonTag}T00:00:00${berlinOffset(vonD)}`,
      `${tagPlus(bisTag, 1)}T00:00:00${berlinOffset(bisD)}`,
      max, process.env.KALENDER_ID || undefined
    ).then((termine) => ({ ok: true, termine }))
      .catch((e) => {
        console.error("Kalender direkt fehlgeschlagen, weiche auf gws-cli aus:",
          String(e.message).slice(0, 120));
        return spanneUeberCli(vonTag, bisTag, max);
      });
  }
  return spanneUeberCli(vonTag, bisTag, max);
}

function spanneUeberCli(vonTag, bisTag, max = 200) {
    ...
    execFile(
      ...
      (err, stdout, stderr) => {
        try {
          ...
          fertig({ ok: true, termine: anreichern(termineLesen(stdout), stdout) });
        } catch (e) {
...
// Ein Termin gezielt nach ID — nur ueber den Direktweg (events/<id>). gws-cli
// hat im Haus keinen geprueften "get"; ohne Token-Datei gibt es also null,
// und der Aufrufer liest das Fenster (kalender-akte.js spanneVoll).
async function terminNachId(id) {
  if (!id) return null;
  const direkt = require("./kalender-direkt.js");
  if (!direkt.bereit()) return null;
  try { return await direkt.terminNachId(String(id), process.env.KALENDER_ID || undefined); }
  catch (e) {
    console.error("Kalender direkt (nach ID) fehlgeschlagen:", String(e.message).slice(0, 120));
    return null;
  }
}

// Die rohe Terminliste aus der gws-cli-Antwort — dieselbe Huelle, die
// termineLesen() in lib/zustand.js auspackt ({ events: { data: "<json>" } }
// mit security_warnings daneben). termineLesen() liest bewusst nur
// id/start/ende/titel/ort: Der STAND fuer die Sprache soll klein bleiben, und
// Alexandra soll keine Beschreibungen vorlesen. Die Kalenderseite braucht mehr
// (Beschreibung, Google-Link, Farbe fuer das Termin-Popover) — also hier je id
// nachgetragen, ohne zustand.js anzufassen.
function rohListe(stdout) {
  const json = (s) => { try { return JSON.parse(s); } catch { return null; } };
  const d = json(stdout);
  const huelle = d && d.events;
  let roh = null;
  if (huelle && typeof huelle === "object" && typeof huelle.data === "string") {
    const innen = json(huelle.data);
    if (Array.isArray(innen)) roh = innen;
  }
  if (!roh) {
    for (const k of [d, d && d.events, d && d.items, d && d.data]) {
      if (Array.isArray(k)) { roh = k; break; }
    }
  }
  if (!Array.isArray(roh)) return [];
  return roh.map((x) => (x && x.event) || x).filter((x) => x && typeof x === "object");
}

function anreichern(termine, stdout) {
  const je = new Map();
  for (const r of rohListe(stdout)) {
    const id = r.id || r.event_id || r.eventId;
    if (id) je.set(String(id), r);
  }
  for (const t of termine) {
    const r = je.get(String(t.id)) || {};
    t.beschreibung = String(r.description || "");
    t.htmlLink = String(r.htmlLink || r.html_link || "");
    t.farbe = r.colorId ? String(r.colorId) : "";
    t.ganztags = istGanztags(t);
  }
  return termine;
}

// Googles elf Termin-Farben (colorId "1"–"11"), so wie der Google Kalender sie
// im Raster zeigt — nicht die blassen Werte aus colors.event der API, denn
// die Seite soll aussehen wie Google, und dort sieht man die kraeftigen.
// Nur fuer Termine, denen jemand in Google ausdruecklich eine Farbe gegeben
// hat; alle anderen tragen die Kalenderfarbe (--primary). Banane ist die
// einzige, auf der Google dunkle Schrift setzt — weisse waere unlesbar.
const FARBEN = {
  "1": { name: "Lavendel", hex: "#7986cb", schrift: "#fff" },
  "2": { name: "Salbei", hex: "#33b679", schrift: "#fff" },
  "3": { name: "Traube", hex: "#8e24aa", schrift: "#fff" },
  "4": { name: "Flamingo", hex: "#e67c73", schrift: "#fff" },
  "5": { name: "Banane", hex: "#f6bf26", schrift: "#1d1d1d" },
  "6": { name: "Mandarine", hex: "#f4511e", schrift: "#fff" },
  "7": { name: "Pfau", hex: "#039be5", schrift: "#fff" },
  "8": { name: "Graphit", hex: "#616161", schrift: "#fff" },
  "9": { name: "Blaubeere", hex: "#3f51b5", schrift: "#fff" },
  "10": { name: "Basilikum", hex: "#0b8043", schrift: "#fff" },
  "11": { name: "Tomate", hex: "#d50000", schrift: "#fff" },
};
const farbeVon = (t) => FARBEN[String((t && t.farbe) || "")] || null;
...
module.exports = {
  ...
  // Zusatzfelder am gws-cli-Weg und Googles Termin-Farben (05.09.2026) —
  // exportiert fuer scripts/test-kalender-akte.js und die Kalenderseite.
  rohListe, anreichern, FARBEN, farbeVon, terminNachId,
