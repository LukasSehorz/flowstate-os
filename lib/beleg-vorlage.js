...
function absatzWeg(text, inhalt) {
  const stelle = text.indexOf(`>${inhalt}<`);
...
function angebotFuellen(text, d) {
  ...
  pruefeVorlage(text, [ANGEBOT.nummer, ANGEBOT.firma, ANGEBOT.grussformel,
    ANGEBOT.postenTitel, ANGEBOT.betrag, ANGEBOT.datum, ANGEBOT.gueltig]);

  const e = d.empfaenger || {};
  const i = d.inhalt || {};
  const ersetze = (von, nach) => { text = text.split(`>${von}<`).join(`>${xml(nach)}<`); };
