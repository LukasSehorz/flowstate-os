...
// Ungelesen-Zaehler je Chat, von der Bruecke gepflegt (siehe dort mergeChats).
// Fehlt die Datei, faellt das Lesen auf eine erklaerte Schaetzung zurueck.
const CHATS = path.join(WA_DIR, "chats.json");
// Bis hierher gilt Ungelesenes als "neu". Alles Aeltere hat Lukas entweder
// laengst gesehen oder bewusst liegen lassen — in beiden Faellen ist es keine
// Antwort auf "habe ich neue Nachrichten?".
const FRISCH_STUNDEN = Number(process.env.WA_UNGELESEN_STUNDEN || 48);
...
async function senden({ an, text }) {
  // Probelauf (20.08.2026): Der ganze Weg bis hierher laeuft echt — Kontakt
  // aufloesen, Text formulieren, Freigabe einholen. Nur die Bruecke bekommt
  // nichts zu sehen. Ohne das haette der Anzeigen-Testlauf Jannik mit
  // Probenachrichten zugeschuettet. Siehe lib/probemodus.js.
  const probemodus = require("./probemodus.js");
  if (probemodus.aktiv()) {
    probemodus.notieren("whatsapp", { an, text: String(text || "").slice(0, 800) });
    return { ok: true, probe: true, an, id: "probe", status: 2, zugestellt: true };
  }
...
async function neueNachrichten({ stunden = 12, maxChats = 5, nurEine = false, seitEigener = false } = {}) {
  ...
  let rein = alle.filter((m) => m.richtung === "sie" && m.text && m.ts >= abWann(m.jid));
  let echterZaehler = false;

  // DER ECHTE ZAEHLER (27.07., zweiter Anlauf).
  //
  // Oben steht die Schaetzung. Sie war noetig, weil die Bruecke nie auf
  // "chats.*" hoerte — genau dort liefert WhatsApp unreadCount je Chat, und
  // "chats.update" kommt auch dann, wenn Lukas einen Chat am HANDY oeffnet.
  // Die Bruecke schreibt das jetzt nach chats.json.
  //
  // Liegt die Datei vor, gilt sie: Je Chat werden die letzten <unread>
  // eingehenden Nachrichten genommen, Chats mit 0 fallen ganz weg. Das ist
  // dieselbe Zahl, die auf Lukas' Handy im gruenen Kreis steht.
  //
  // Fehlt die Datei (Bruecke noch nicht neu gestartet, frisch gekoppelt), oder
  // kennt sie keinen einzigen ungelesenen Chat, bleibt es bei der Schaetzung.
  // Lieber eine erklaerte Naeherung als eine leere Antwort.
  if (seitEigener) {
    let stand = null;
    try { stand = JSON.parse(fs.readFileSync(CHATS, "utf-8")); } catch {}
    // Sobald es den Zaehler ueberhaupt gibt, ist er MASSGEBLICH — auch wenn er
    // null ungelesene meldet. Der erste Entwurf sprang bei "alles gelesen" in
    // den else-Zweig und las prompt wieder Gelesenes vor: Die Schaetzung stand
    // ja noch in "rein". Ein Zaehler, der nur bei Treffern gilt, ist keiner.
    if (stand && Object.keys(stand).length) {
      echterZaehler = true;
      const proChat = new Map();
      for (const m of alle) {
        if (m.richtung !== "sie" || !m.text) continue;
        if (!proChat.has(m.jid)) proChat.set(m.jid, []);
        proChat.get(m.jid).push(m);
      }
      rein = [];
      let altVergessen = 0;
      // Was NIE eine ungelesene Nachricht ist: Statusmeldungen und Kanaele.
      // WhatsApp zaehlt sie im selben Feld mit; fuer Lukas sind es keine
      // Nachrichten, die jemand an ihn geschrieben hat (07.08.2026).
      const keineNachricht = (jid) => jid === "status@broadcast" || jid.includes("@newsletter");
      for (const [jid, c] of Object.entries(stand)) {
        if (!c || !(c.unread > 0) || keineNachricht(jid)) continue;
        // ALTES UNGELESENES IST KEINE NEUIGKEIT (07.08.2026).
        //
        // Lukas fragte "habe ich neue ungelesene Nachrichten?" und bekam eine
        // vorgelesen, die er laengst gelesen hatte. Im Zaehler standen acht
        // Chats auf ungelesen — der juengste 18, der aelteste 246 Stunden alt.
        // 47 von 55 Chats stehen korrekt auf null, der Mechanismus stimmt also;
        // einzelne bleiben haengen, wenn WhatsApp das Zuruecksetzen nie meldet.
        //
        // Statt zu raten, welcher Zaehler stimmt: Was Tage alt ist, ist keine
        // Antwort auf "was ist NEU". Es faellt nicht unter den Tisch, sondern
        // wird als Nebensatz erwaehnt — verschweigen waere auch gelogen.
        const alter = c.t ? (Date.now() / 1000 - c.t) / 3600 : 0;
        if (alter > FRISCH_STUNDEN) { altVergessen++; continue; }
        const msgs = (proChat.get(jid) || []).sort((a, b) => a.ts - b.ts);
        // Nur so viele, wie WhatsApp als ungelesen zaehlt — die juengsten.
        rein.push(...msgs.slice(-c.unread));
      }
      if (!rein.length) {
        return { ok: true, reply: altVergessen
          ? `Nichts Neues. Es liegen noch ${altVergessen} ältere ungelesene Chats von vor Tagen — die kannst du am Handy wegräumen.`
          : "Nichts Ungelesenes — du bist überall durch." };
      }
    } else if (!rein.length) {
      return { ok: true, reply: "Seit du zuletzt selbst geschrieben hast, ist nichts Neues gekommen." };
    }
  }

  if (!rein.length && !seitEigener) {
  ...
  // Sagen, WORAUF sich die Auskunft bezieht. Mit echtem Zaehler darf "ungelesen"
  // stehen — das ist dieselbe Zahl wie im gruenen Kreis auf dem Handy. Ohne ihn
  // waere es geraten, dann wird die Naeherung benannt.
  const kopf = !seitEigener ? ""
    : echterZaehler ? "Ungelesen: "
    : "Seit du zuletzt geschrieben hast: ";
