...
async function neueNachrichten({ stunden = 12, maxChats = 5, nurEine = false } = {}) {
  ...
  for (const c of personen.slice(0, maxChats)) {
    const name = kontakte.nameZuJid(c.jid, c.letzte.von);
    saetze.push(c.anzahl === 1
      ? `Von ${name}: „${kurz(c.letzte.text, 200)}“`
      : `${c.anzahl} von ${name}, zuletzt: „${kurz(c.letzte.text, 200)}“`);
  }
  ...
  return { ok: true, reply: saetze.join(". ") + "." };
}
