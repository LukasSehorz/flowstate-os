...
function leseChat(name, { max = 10 } = {}) {
  const ziel = kontakte.finde(name);
  if (!ziel || !ziel.nummer) return { ok: true, reply: `Ich find keinen Kontakt „${name}“ — nenn mir den Namen genauer.` };
  const jid = ziel.nummer + "@s.whatsapp.net";
  let roh;
  try { roh = fs.readFileSync(VERLAUF, "utf-8"); } catch { roh = ""; }
  const msgs = roh.split("\n").filter(Boolean)
    .map((z) => { try { return JSON.parse(z); } catch { return null; } })
    .filter((m) => m && m.jid === jid).slice(-max);
  if (!msgs.length) return { ok: true, reply: `Von ${ziel.name} hab ich seit dem Koppeln nichts mitbekommen. Ältere Chats zieht WhatsApp leider nicht rüber.` };
  const zeilen = msgs.map((m) => `${m.richtung === "ich" ? "Lukas" : ziel.name}: ${m.text}`).join("\n");
  return { ok: true, reply: `Chat mit ${ziel.name} (neueste zuletzt):\n${zeilen}`, kontakt: ziel.name };
}
