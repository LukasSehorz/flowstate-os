...
// --- Ist das eine Frage nach draussen? -------------------------------------
//
// GEMESSEN AM 20.08.2026, /api/chat, Lauf 2 von 5 — die Zeile aus dem
// Sprachprotokoll:
//
//   "verstehen": { "aufrufe": [], "tokenDenken": 100 },
//   "gesagt": "Das Ergebnis ist gerade durch, aber bei mir kommt keine
//              Tabellenplatzierung an. Sag mir die Liga oder Saison, dann
//              geh ich gezielt ran.",
//   "quelle": "zustand"
//
// `aufrufe: []`. Es wurde NIE gesucht. Das Modell hat 100 Token gedacht, sich
// aus dem Gespraechsverlauf zusammengereimt, die Suche liefere nichts, und das
// als Tatsache vorgetragen — vier Sekunden lang, ohne einen einzigen Aufruf.
// Vier von fuenf Laeufen waren richtig; dieser eine ist der, der in einer
// Aufnahme steht.
//
// Am Prompt allein ist das nicht dichtzukriegen. Dieselbe Erfahrung steht in
// dieser Datei schon zweimal (Zusage-Netz 07.08., Termin-Netz 20.08.): Die
// Regel steht drin, das Modell biegt sie trotzdem. Also eine Pruefung, die
// nicht vom Modell abhaengt.
//
// Sie beantwortet EINE Frage: Kann der STAND das ueberhaupt wissen? Steht in
// der Frage ein Name, den der STAND nicht kennt, und wird nach etwas gefragt,
// das sich taeglich aendert — dann nein, und dann wird gesucht, egal was das
// Modell gerade meint.
//
// BEWUSST ENG. Sie darf nur dort greifen, wo eine Websuche die einzig moegliche
// Antwort ist. Drei Riegel halten sie klein: ein Befehl ist keine Frage, alles
// mit "wir/uns/mein" ist eine Frage nach dem eigenen Laden, und ein Name, der
// im STAND vorkommt, ist kein Fremder.
const AUSSENFRAGE = /\btabelle|preis|\b(tabellenplatz|tabellenstand|platz|rang|punkte|spielt?ag|spiel|spiele|gespielt|ergebnis|ergebnisse|gewonnen|verloren|unentschieden|torverh\w*|liga|saison|kurs|aktie\w*|kostet|kosten|wetter|temperatur|einwohner|b[üu]rgermeister|[öo]ffnungszeiten|wechselkurs|schlagzeile\w*|nachrichten|news|passiert)\b/i;

// Ein Auftrag ist keine Nachschlagefrage. Wenn das Verstehen bei "schreib
// Jannik ..." nichts gerufen hat, ist eine Websuche die falsche Rettung.
const BEFEHLSWORT = /^\s*(schreib|schick|send|mail|trag|setz|leg|mach|erstell|bau|ruf|notier|erinner|verschieb|sag\s+\w+\s+ab|buch|zahl|hol|starte?|beende?|l[öo]sch|streich)/i;

// Alles mit "wir/uns/mein/ich" fragt nach dem eigenen Laden — das steht im
// STAND oder in der Datenbank, nie im Internet.
const INNENFRAGE = /\b(bei uns|unser\w*|wir|uns|mein\w*|mir|mich|ich)\b/i;

function istNachschlagefrage(frage, standText = "", verlauf = []) {
  const f = String(frage || "").trim();
  if (f.length < 6 || f.length > 300) return false;
  if (BEFEHLSWORT.test(f)) return false;
  if (INNENFRAGE.test(f)) return false;
  if (!AUSSENFRAGE.test(f)) return false;

  // NUR DER ECHTE STAND ZAEHLT (20.08.2026, beim Messen gefunden).
  //
  // sprache-routes.js haengt an den STAND die Liste der laufenden Auftraege —
  // und dort steht Lukas' Frage WOERTLICH drin ("LAEUFT SEIT 3 s: Wie steht
  // der SV Oberbergkirchen in der Tabelle?"). Ab der zweiten Runde stand der
  // Vereinsname damit im Text, die Pruefung unten sagte "kennt der STAND
  // schon", und das Netz griff nie: 20 Laeufe, 5 richtig, obwohl es eingebaut
  // war. Ein STAND, der die Frage nur zurueckwirft, weiss gar nichts.
  //
  // Der Aufrufer uebergibt seit dem Fund den STAND ohne Anhang. Hier wird er
  // trotzdem abgeschnitten: Die Pruefung soll auch dann richtig antworten,
  // wenn jemand die beiden Texte spaeter wieder zusammenlegt.
  const stand = String(standText || "").split("DEINE LAUFENDE ARBEIT")[0].toLowerCase();
  const namen = namensKandidaten(f).filter((n) => n.length >= 4);
  // Ein Name, den der STAND nicht kennt: Dann kann die Antwort nicht von dort
  // kommen. Ein bekannter Name (eigene Firma, Kunde aus dem CRM) schon.
  if (namen.some((n) => !stand.includes(n.toLowerCase()))) return true;

  // Kein eigener Name, aber ein Zeiger nach hinten ("Wie haben SIE am
  // Wochenende gespielt?") — dieselbe Aussenfrage, nur verkuerzt. Das Thema
  // steht im Verlauf oder im Gedaechtnis der Suche.
  if (!namen.length && STARKE_ANAPHER.test(f)) {
    const thema = themaAusVerlauf(verlauf) || themaHolen("standard");
    if (thema && !stand.includes(String(thema).toLowerCase())) return true;
  }
  return false;
}
...
async function websuche(rohFrage, {
  ...
  try {
    ...
    // EIN ZWEITER VERSUCH FUER DIE ERSTE STUFE (20.08.2026).
    //
    // Gemessen ueber /api/chat: 93 Suchen, 92 sauber, EINE mit
    // "The operation was aborted due to timeout" nach 3.502 ms — die Frist
    // steht auf 3.500. Das war kein Denkfehler und keine schlechte Frage,
    // sondern Netz-Zittern zwei Millisekunden ueber der Grenze. Lukas hoerte
    // dafuer "Da komm ich gerade nicht durch — frag mich gleich nochmal."
    //
    // Der zweite Versuch kostet im Normalfall NICHTS: Die Suche ist sonst nach
    // 0,7 bis 1,8 s da, und dieser Zweig wird nie betreten. Nur im
    // Zitterfall wird gewartet, und dafuer gibt es dann eine Antwort statt
    // einer Entschuldigung.
    //
    // BEWUSST NUR HIER und bewusst nur EINMAL: Die erste Stufe ist ein einziger
    // HTTP-Aufruf ohne Nebenwirkung, den kann man gefahrlos wiederholen. Und
    // nur, wenn das Gesamtbudget es noch hergibt — sonst waere der zweite
    // Versuch eine laengere Stille mit demselben Ausgang.
    let haupt = a;
    if ((haupt.status !== "fulfilled" || !haupt.value.ok)
        && Date.now() - start < timeoutMs - frist - 1500) {
      stufen.zweiterVersuch = true;
      [haupt] = await Promise.allSettled([serper(frage, { timeoutMs: frist })]);
      stufen.sucheMs = Date.now() - t0;
    }

    if (haupt.status !== "fulfilled" || !haupt.value.ok) {
      return scheitern(start, stufen, haupt.status === "fulfilled" ? haupt.value.grund : String(haupt.reason?.message).slice(0, 120));
    }
    ...
    fund = fundBauen(news ? { ...haupt.value.d, news } : haupt.value.d);
  } catch (e) {
