...
module.exports = function (app) {
  ...
  // Kennzahlen als JSON fuer die Content-Karte auf der Zentrale. Antwortet mit
  ...
  app.get("/api/content/stats", async (req, res) => {
    ...
    try {
      res.json({ ok: true, ...(await ct.kennzahlen(u)) });
    } catch (e) {
      console.error("Content-Kennzahlen:", e.message);
      res.json({ ok: false, hint: "Content-Daten nicht erreichbar." });
    }
  });
