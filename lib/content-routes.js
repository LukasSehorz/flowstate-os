...
module.exports = function (app) {
  ...
  app.get("/content", angemeldet, async (req, res, next) => {
    try {
      const u = req.nutzer;
      const [plan, raus, masse, einst] = await Promise.all([
        ct.redaktionsplan(u),
        ct.letzteVeroeffentlicht(u, 10),
        ct.massenZaehler(u),
        ct.einstellungen(u),
      ]);
      ...
      const rausBlock = raus.length
        ? `<div class="mk-raus">${raus.map((p) => `<div class="mk-raus-zeile">
            ...
            <span class="mk-raus-titel">${p.link
              ...
              : e(p.titel)}
              <i>${e(p.kanal)}${p.format ? " · " + e(p.format) : ""}${p.wer ? " · " + e(p.wer.split(" ")[0]) : ""}</i></span>
      ...
      res.send(schale({
        ...
        <div class="kacheln">
          ${kachel("Diese Woche", `${wocheIst}<span class="kachel-von"> / ${ziel}</span>`,
        ...
        <dialog id="dlg-post"><form method="post" action="/content/post/anlegen">
          ...
          <div class="feld-paar">
            <div class="feld"><label>Kanal</label>${auswahl("kanal", ct.KANAELE, "Instagram")}</div>
            <div class="feld"><label>Format</label>${auswahl("format", ct.FORMATE, null, "— egal —")}</div>
          </div>
          <div class="feld-paar">
