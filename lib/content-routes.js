...
module.exports = function (app) {
  ...
  app.get("/content", angemeldet, async (req, res, next) => {
    try {
      ...
      const [plan, raus, masse, einst, jeMarke] = await Promise.all([
        ct.redaktionsplan(u, gewaehlt),
        ct.letzteVeroeffentlicht(u, 10, gewaehlt),
        ct.massenZaehler(u, gewaehlt),
        ct.einstellungen(u),
        ct.jeMarke(u),
      ]);
      const markeVon = (id) => marken.find((m) => m.id === id);
      const aktuelle = gewaehlt ? markeVon(gewaehlt) : null;
      const mUrl = (id) => "/content" + (id ? "?marke=" + encodeURIComponent(id) : "");
      ...
      const planZeile = (p) => `<div class="mk-post ${p.ueberfaellig ? "spaet" : ""}">
        <div class="mk-post-kopf">
          ...
          <span class="mk-post-titel">${e(p.titel)}
            <i>${!gewaehlt && p.marke_name ? `<b class="ct-marke-tag">${e(p.marke_name.split(" ")[0])}</b> · ` : ""}${e(p.kanal)}${p.format ? " · " + e(p.format) : ""}${p.zielgruppe ? " · " + e(p.zielgruppe) : ""}</i></span>
      ...
      res.send(schale({
        ...
        <div class="seiten-kopf">
          <div><p class="sub">${aktuelle
            ? `${e(aktuelle.name)} — diese Woche ${wocheIst} von ${zielGesamt} Posts`
            : `Alle Marken zusammen — diese Woche ${wocheIst} von ${zielGesamt} Posts`}</p></div>
          <div class="bh-tasten">
