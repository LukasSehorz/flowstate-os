...
module.exports = function (app) {
  ...
  app.get("/content", angemeldet, async (req, res, next) => {
    try {
      ...
      const [plan, raus, masse, einst, jeMarke, jeKanal, zieleKanal,
             uploads, summen, offeneMess] = await Promise.all([
      ...
      const planZeile = (p) => `<div class="mk-post ${p.ueberfaellig ? "spaet" : ""}">
        <div class="mk-post-kopf">
          ...
          <span class="mk-post-titel">${e(p.titel)}
            <i>${!gewaehlt && p.marke_name ? `<b class="ct-marke-tag">${e(p.marke_name.split(" ")[0])}</b> · ` : ""}${p.sorte ? e(p.sorte) : ""}${kanal ? "" : (p.sorte ? " · " : "") + (p.kanaele||[]).map(e).join(" · ")}${p.zielgruppe ? " · " + e(p.zielgruppe) : ""}</i></span>
      ...
      const rausBlock = raus.length
        ? `<div class="mk-raus">${raus.map((p) => `<div class="mk-raus-zeile">
            ...
            <span class="mk-raus-titel">${p.link
              ...
              : e(p.titel)}
              <i>${!gewaehlt && p.marke_name ? `<b class="ct-marke-tag">${e(p.marke_name.split(" ")[0])}</b> · ` : ""}${p.sorte ? e(p.sorte) + " · " : ""}${(p.kanaele||[]).map(e).join(" · ")}</i></span>
      ...
      const summenZeile = (k) => {
        const w = summen.woche[k.id], m = summen.monat[k.id];
        if (!w && !m) return "";
        return `<tr><td>${e(k.titel)}</td>
          <td class="rechts"><b>${zahl(w)}</b></td>
          <td class="rechts">${zahl(m)}</td></tr>`;
      };
      ...
      const summenBlock = summenZeilen
        ? `<div class="tabelle-huelle"><table class="tabelle ct-summen">
            <thead><tr><th></th><th class="rechts">Diese Woche</th><th class="rechts">Dieser Monat</th></tr></thead>
            <tbody>
              <tr class="ct-summen-kopf"><td>Videos</td>
                <td class="rechts"><b>${zahl(summen.stuecke_woche)}</b></td>
                <td class="rechts">${zahl(summen.stuecke_monat)}</td></tr>
              ${summenZeilen}
            </tbody></table></div>
          <p class="caption mk-legende">${ICON.info}
            Gezählt wird je Video und Kanal die jeweils neueste Messung — ein Video,
            das nach 24 Stunden und nach 7 Tagen gemessen wurde, zählt einmal.</p>`
  ...
  //
  // Eine Seite je Stueck Content. Sie zeigt eine Spalte je Kanal, darin eine
  // Zeile je Messpunkt — so steht "nach 24 Stunden" direkt ueber "nach 7 Tagen"
  // und man sieht auf einen Blick, was nachgezogen hat und was verpufft ist.
  app.get("/content/post/:id", angemeldet, async (req, res, next) => {
    try {
      ...
      const wert = (z, k) => {
        ...
        return k.art === "komma" ? Number(z[k.id]) : Number(z[k.id]);
      };
      const zeig = (z, k) => {
        const v = wert(z, k);
        if (v === null) return `<span class="ct-leer">–</span>`;
        return k.art === "komma"
          ? v.toLocaleString("de-DE", { minimumFractionDigits: 1, maximumFractionDigits: 2 })
          : zahl(v);
      };
      // Veraenderung zwischen zwei Messpunkten. Das ist die eigentliche Aussage
      // der Seite: nicht wie viel, sondern ob es nachgezogen hat.
      const wachstum = (frueh, spaet, k) => {
        const a = wert(frueh, k), b = wert(spaet, k);
        if (a === null || b === null || a === 0) return "";
        const proz = Math.round(((b - a) / a) * 100);
        if (proz === 0) return "";
        return `<i class="ct-wachstum ${proz > 0 ? "auf" : "ab"}">${proz > 0 ? "+" : ""}${proz} %</i>`;
      };

      const kanalKarte = (kanal) => {
        const punkte = ct.MESSPUNKTE;
        const vorhanden = punkte.filter((mp) => zahlVon(kanal, mp.id));
        return `<div class="karte ct-analyse"><div class="karte-kopf"><div>
          <h2>${e(kanal)}</h2>
          <div class="sub">${vorhanden.length
            ? vorhanden.length + " Messung" + (vorhanden.length === 1 ? "" : "en")
            : "noch nicht gemessen"}</div></div>
          <button type="button" class="sekundaer klein"
            onclick="messenOeffnen(${JSON.stringify(kanal).replace(/"/g, "&quot;")})">Zahlen eintragen</button></div>
          ${vorhanden.length ? `<div class="tabelle-huelle"><table class="tabelle ct-analyse-tab">
            <thead><tr><th></th>${punkte.map((mp) =>
              zahlVon(kanal, mp.id) ? `<th class="rechts">${e(mp.titel)}</th>` : "").join("")}</tr></thead>
            <tbody>${ct.KENNZAHLEN.map((k) => {
              const zellen = punkte.filter((mp) => zahlVon(kanal, mp.id));
              if (zellen.every((mp) => wert(zahlVon(kanal, mp.id), k) === null)) return "";
              return `<tr><td>${e(k.titel)}</td>${zellen.map((mp, i) => {
                const z = zahlVon(kanal, mp.id);
                const vor = i > 0 ? zahlVon(kanal, zellen[i - 1].id) : null;
                return `<td class="rechts">${zeig(z, k)}${vor ? wachstum(vor, z, k) : ""}</td>`;
              }).join("")}</tr>`;
      ...
      res.send(schale({
        ...
        <div class="seiten-kopf">
          ...
          ${p.link ? `<div class="bh-tasten">
            <a href="${e(p.link)}" target="_blank" rel="noopener" class="knopf sekundaer">Video ansehen ↗</a>
          </div>` : ""}
        </div>

        <div class="karte" style="margin-bottom:16px"><div class="karte-kopf"><div>
          <h2>${e(p.titel)}</h2>
          <div class="sub">Läuft auf ${p.kanaele.map(e).join(", ")}${
            p.zielgruppe ? " · Zielgruppe " + e(p.zielgruppe) : ""}</div></div>
          <span class="badge b-gruen">${e(ct.STATUS_LABEL[p.status])}</span></div>
          ${p.notiz ? `<p class="mk-post-notiz">${e(p.notiz)}</p>` : ""}
        </div>
