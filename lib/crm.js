  ...
  return alsNutzer(user.id, async (q) => {
    ...
    const { rows } = await q(
      `select
        ...
        (select coalesce(sum(z.sekunden), 0)::bigint from zeiterfassung z
          where z.nutzer_id = $1${seit("z.datum")}) as zeit_sekunden,
        (select count(*)::int from zeiterfassung z
          where z.nutzer_id = $1 and z.sekunden > 0${seit("z.datum")}) as zeit_tage
      `, [personId]);
    return rows[0];
  });
};
  ...
  return alsNutzer(user.id, async (q) => {
    const { rows } = await q(
      `select date_trunc('${stufe}', z.datum)::date as punkt,
              sum(z.sekunden)::bigint as sekunden
