...
function anrufAufgabePlanen(ergebnis, extra) {
  ...
  if (wieder && wieder.uhrzeit) {
    // uhrzeit geht MIT hinaus (0068): anrufAufgabe schreibt sie nach
...
async function anrufAufgabe(user, firmaId, ergebnis, extra = {}, firmaName = "") {
  ...
  try {
    ...
    if (da) {
      ...
      // geplant_um MIT nachfuehren (0068): Wird beim zweiten Anruf eine neue
      ...
      await alsNutzer(user.id, (q) => q(
        `update aufgaben set titel = $2, geplant_am = $3, faellig = $3,
          ...
          where id = $1`, [da.id, titel, plan.tag, extra.notiz || null, plan.uhrzeit || null]));
...
// Minuten seit Mitternacht aus aufgaben.geplant_um (0068). Die time-Spalte
...
function aufgabeKalenderPlan(aufgabe, optionen = {}) {
  ...
  // Steht aber eine Uhrzeit in der Aufgabe (geplant_um, 0068), ist sie
