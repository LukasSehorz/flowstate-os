  ...
  return alsNutzer(user.id, async (q) => {
    ...
    if (ergebnis === "gebucht") {
      ...
      await q(`update firmen set status='lead', temperatur='heiss', anruf_notiz=$2,
               verlust_grund=null, wiedervorlage=$3 where id=$1`,
        [firmaId, extra.notiz || null, extra.datum || null]);
...
module.exports.kundeAendern = async function (user, id, d) {
  return alsNutzer(user.id, async (q) => {
    ...
    if (d.besitzer) {
      // Uebergabe an jemand anderen. Es reicht NICHT, die Firma umzuhaengen:
      // Deals, Projekte und Aufgaben tragen ihren eigenen Besitzer, und daran
      // haengen die Zeilenrechte. Bleiben sie stehen, sieht der neue
      // Verantwortliche die Akte, aber nicht das Projekt darin — und auf dem
      // Brett steht weiter der alte Name.
      //
      // Direkt gegen den jeweiligen Besitzer pruefen, nicht gegen den der Firma:
      // sonst bleibt etwas haengen, wenn die Stände auseinanderlaufen.
      const { rows: bewegt } = await q(
        `update deals set besitzer = $2
          where firma_id = $1 and besitzer is distinct from $2 returning id`, [id, d.besitzer]);
      const { rowCount: proj } = await q(
        `update projekte set besitzer = $2
          where firma_id = $1 and besitzer is distinct from $2`, [id, d.besitzer]);
      const { rowCount: auf } = await q(
        `update aufgaben set besitzer = $2
          where firma_id = $1 and erledigt = false and besitzer is distinct from $2`, [id, d.besitzer]);
      if (bewegt.length || proj || auf) {
        const wer = (await q(`select name from profiles where id=$1`, [d.besitzer])).rows[0];
        const teile = [];
        if (bewegt.length) teile.push(`${bewegt.length} ${bewegt.length === 1 ? "Abschluss" : "Abschlüsse"}`);
        if (proj) teile.push(`${proj} ${proj === 1 ? "Projekt" : "Projekte"}`);
        if (auf) teile.push(`${auf} ${auf === 1 ? "Aufgabe" : "Aufgaben"}`);
...
module.exports.leadsAnlegen = async function (user, leads,
  { sparte = "", branche = "", besitzer = null, gewonnenDurch = null } = {}) {
  return alsNutzer(user.id, async (q) => {
    let angelegt = 0; const uebersprungen = [], wemGehoert = {};
    for (const l of leads) {
      ...
      await q(
        `insert into firmen (name, status, branche, ort, telefon, website, geschaeftsfuehrer,
                             email, adresse, plz, quelle, tags, besitzer, gewonnen_durch)
         values ($1,'lead',$2,$3,$4,$5,$6,$7,$8,$9,'Cold Calling',$10,$11,$12)`,
  ...
  return alsNutzer(user.id, async (q) => {
    ...
    const { rows: [t] } = await q(
      `select
       ...
       from aufgaben where besitzer = $1`, [user.id]);
...
module.exports.todoEinstufen = async function (user, id, { wichtigkeit, dringlichkeit } = {}) {
  ...
  return alsNutzer(user.id, async (q) => {
    ...
    const { rowCount } = await q(
      `update aufgaben set ${setzen.join(", ")} where id = $1 and besitzer = $2`, werte);
