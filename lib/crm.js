...
// Datei einmal war.
async function leadListen(user, { besitzer = "" } = {}) {
  return alsNutzer(user.id, async (q) => {
    const a = [], w = [];
    ...
    for (const l of leads) {
      ...
      const neu = (await q(
        ...
        [name, (l.branche || "").trim() || null, (l.ort || "").trim() || null,
         tel || null, webseite(l.website), (l.chef || "").trim() || null,
    ...
    if (neueIds.length) {
      ...
      liste = (await q(
        `insert into call_listen (name, besitzer)
         ...
         returning id, name`,
        [listenName, besitzer || user.id])).rows[0];
      await q(
