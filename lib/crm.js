...
function anrufAufgabePlanen(ergebnis, extra) {
  ...
  // Follow-up und "Später nochmal": Der Tag kommt aus der Wiedervorlage, wenn
  // eine gesetzt wurde. Die Leads-Maske schickt bei Follow-up bisher keine —
  // dann sind es drei Werktage: nah genug, dass das Gespraech noch praesent
  // ist, weit genug, dass man nicht am naechsten Morgen schon wieder anruft.
  // Bei "Später nochmal" traegt die Maske die Wiedervorlage ein (sie schreibt
  // sie in firmen.wiedervorlage, siehe anrufSpeichern) — dann steht die
  // Aufgabe genau an dem Tag, den der Angerufene selbst genannt hat.
  const wieder = zeitpunktLesen(extra.datum);
  ...
  return { wichtigkeit: 2, dringlichkeit: "Bald", tag,
           titel: `Nachfassen — kein Termin${notiz ? " · " + notiz : ""}` };
}
...
// einen ganztaegigen Eintrag am geplanten Tag; ohne Datum keinen, denn dann
// wuesste niemand, wohin damit.
//
...
// Uhrzeit und 45 Minuten Dauer, und haengt die Akte als Verknuepfung mit
...
//   einen ganztaegigen Eintrag am selben Tag — genau die Erinnerung, die man
//   dafuer braucht. Umgekehrt bekommt ein Follow-up bei einer Firma, die vor
...
// -> { eintrag:false, grund } | { eintrag:true, tag, titel, ganztags:true }
function aufgabeKalenderPlan(aufgabe, optionen = {}) {
  ...
  return { eintrag: true, tag, titel, ganztags: true };
}
...
function aufgabeKalenderText(aufgabe, personName = "") {
  const a = aufgabe || {};
  ...
  try {
    ...
    if (a.kalender_termin) {
      const r = await w.terminAendern({ id: a.kalender_termin, titel: plan.titel,
        start: plan.tag, ende: plan.tag, ganztags: true, beschreibung });
    ...
    const r = await w.terminEintragen({ titel: plan.titel, start: plan.tag, ende: plan.tag,
                                        ganztags: true, beschreibung });
    if (!r.ok || !r.id) {
