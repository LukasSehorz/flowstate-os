...
//
// "Eins von beiden" sind die zwei Ergebnisse, die Arbeit hinterlassen:
//   gebucht    Ein Termin steht — er will vorbereitet und gehalten werden.
//   follow-up  Interesse ohne Termin — jemand muss nachfassen.
// Die uebrigen vier legen NICHTS an: Bei "Nein" und "Webseite zu gut" ist der
// Lead zu, da ist nichts mehr zu tun. "Nicht erreicht" und "Später nochmal"
// stehen als Liste und Wiedervorlage ohnehin schon in der Anrufliste — jeder
// nicht abgenommene Hoerer als eigene Tafelzeile, und nach zwei Tagen liest
// niemand mehr hin.
//
...
// die Kundenakte an den Kunden-Block der verantwortlichen Person. Hier steht
// nur, WAS in der Zeile steht und WER sie bekommt.
...
function anrufAufgabePlanen(ergebnis, extra) {
  ...
  // Follow-up: Der Tag kommt aus der Wiedervorlage, wenn eine gesetzt wurde.
  // Die Leads-Maske schickt bisher keine — dann sind es drei Werktage: nah
  // genug, dass das Gespraech noch praesent ist, weit genug, dass man nicht
  // am naechsten Morgen schon wieder anruft.
  const wieder = zeitpunktLesen(extra.datum);
  const notiz = stichpunkt(extra.notiz, 40);
  return { wichtigkeit: 2, dringlichkeit: "Bald",
           tag: wieder ? wieder.tag : werktageSpaeter(3),
...
async function todoAufTafel(user, aufgabe) {
  let r;
  try { r = await require("./aufgaben-tafel.js").aufgabeAufTafel(user, aufgabe); }
