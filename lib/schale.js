...
const ICON = {
  ...
  zurueck: S('<path d="M15 19l-7-7 7-7"/>'),
 ...
 * @param {string} [o.zurueck]  Ziel des Zurueck-Pfeils, wenn es keinen Verlauf
 *   gibt (frischer Tab, Lesezeichen). Ohne Angabe leitet zurueckZiel() es aus
 *   der Navigation ab — das stimmt fast immer. Setzen, wenn eine Seite es
 *   besser weiss, etwa weil sie aus mehreren Listen erreichbar ist und die
 *   Herkunft in der Adresse mitfuehrt.
 */
// Wohin fuehrt "Zurueck", wenn es keinen eigenen Verlauf gibt (05.09.2026)?
// Lukas' Wunsch: "Es soll auch immer ein Zurueck-Button geben, oben links, so
// einen Pfeil, und dann soll man da zurueckkommen, wo man davor war."
//
// "Wo man davor war" ist im Normalfall der Verlauf des Browsers — den kennt nur
// die Seite selbst, deshalb entscheidet darueber das Skript weiter unten. Kommt
// jemand aber frisch von aussen (Lesezeichen, Link aus einer Mail, neuer Tab),
// gibt es keinen Verlauf, und dann braucht der Knopf ein Ziel, das der Server
// schon kennt: die Seite, unter der der aktuelle Eintrag in der Navigation
// haengt. Genau das steht in MODULE — die Kundenakte laeuft als "crm-kunden",
// ihr Rueckfall ist also /crm/kunden, die Lead-Akte landet auf /crm/leads.
// Module, die es besser wissen, uebergeben zurueck: "/eigener/weg".
function zurueckZiel(aktiv) {
  const id = String(aktiv || "");
  for (const m of MODULE) {
    if (m.id === id) return m.href || "/";
    for (const u of m.unter || []) if (u.id === id) return u.href || m.href || "/";
  }
  // Unbekannte id: der Teil vor dem Bindestrich benennt fast immer den Bereich
  // ("buchhaltung-kosten" -> "buchhaltung"), sonst bleibt die Zentrale.
  const wurzel = id.split("-")[0];
  for (const m of MODULE) if (m.id === wurzel) return m.href || "/";
  return "/";
}

function schale({ titel, aktiv, inhalt, unterzeile = "", reiter = "", nutzer = null, suche = "", hinweise = [], zurueck = "" }) {
  ...
  // Der Pfeil steht auf jeder Arbeitsseite, aber nicht auf der Zentrale (von
  // dort geht es nirgendwo "zurueck", sie ist der Anfang) und nicht auf der
  // Sprachbuehne, die abgefilmt wird und in der Ecke keinen Knopf duldet.
  // Es ist ein echter Link, kein Knopf: ohne JavaScript fuehrt er auf den
  // Rueckfall, mit Mittelklick oeffnet er einen neuen Tab, und eine Seite mit
  // ungespeicherten Aenderungen darf ihre beforeunload-Warnung zeigen — was ein
  // location.replace verschlucken wuerde.
  // Ausgenommen sind genau zwei ids — bewusst nicht das ganze Modul "zentrale":
  //   "zentrale-start" — die Startseite selbst (server.js:1587). Sie ist der
  //     Anfang, von dem es nicht zurueckgeht.
  //   "sprache" — /sprache UND /buehne (sprache-routes.js:1357 und 1376 nehmen
  //     dieselbe id). Beide werden abgefilmt.
  // Die blosse Modul-id "zentrale" bleibt drin, denn sie traegt nicht die
  // Startseite, sondern /umsatz (server.js:1604/1652/1670) und die Fehlerseite
  // (server.js:2528) — Seiten, in die man hineingeht und aus denen man wieder
  // heraus will. Ihr Rueckfall ist ueber MODULE dieselbe "/", auf die auch der
  // vorhandene Knopf "← Zentrale" zeigt. Die Unterpunkte zentrale-todos und
  // zentrale-whiteboard bekommen den Pfeil ebenso.
  // Zusaetzlich blendet os-ui.css ihn unter body.jarvis aus: diese Klasse setzt
  // server.js erst NACH dem Rendern, hier ist sie noch nicht zu sehen.
  const zeigeZurueck = aktiv !== "zentrale-start" && aktiv !== "sprache";
  const zurueckKnopf = zeigeZurueck
    ? `<a class="topbar-zurueck" id="zurueck-knopf" href="${e(zurueck || zurueckZiel(aktiv))}"
        title="Zurück (Alt+←)" aria-label="Zurück">${ICON.zurueck}</a>`
    : "";
...
<!-- os-ui (05.09.): gemeinsames Bausteinsystem fuer alle Arbeitsansichten, additiv ueber crm.css. Global, damit kein Modul es selbst verlinken muss. -->
<link rel="stylesheet" href="${v("/os-ui.css")}">
...
<div class="shell">
  ...
  <div class="inhalt">
    <header class="topbar">
      ${zurueckKnopf}<div><div class="topbar-titel">${e(titel)}</div>${unterzeile ? `<div class="caption">${unterzeile}</div>` : ""}</div>
...
// Der Zurueck-Pfeil (05.09.2026). Drei Faelle, in dieser Reihenfolge:
//
// 1. Es gibt einen eigenen Verlauf — der Verweis (document.referrer) kommt vom
//    selben Server und der Browser hat mehr als einen Eintrag: dann fuehrt
//    history.back() genau dorthin zurueck, wo man war, samt Rollposition und
//    gesetzten Filtern. Das ist der Normalfall und der Wortlaut des Wunsches.
// 2. Kein Verlauf (neuer Tab, Lesezeichen, Link aus einer Mail): dann greift
//    das Ziel aus dem href, das der Server aus der Navigation abgeleitet hat.
// 3. Vorher wird dieses Ziel noch am Pfad geschaerft: steht man tiefer als das
//    Ziel und ist das Ziel ein Anfang des eigenen Pfades, ist die Ebene
//    darueber die bessere Antwort — /buchhaltung/rechnungen/2026-014 fuehrt so
//    auf /buchhaltung/rechnungen statt auf /buchhaltung. Bei /crm/firma/52
//    greift die Regel nicht (/crm/firma ist keine Seite), dort bleibt es beim
//    /crm/kunden aus der Navigation.
//
// Bewusst ein Link und kein location.replace: Seiten mit ungespeicherten
// Aenderungen (die Kundenakte hat eine beforeunload-Wache) sollen weiter fragen.
(function(){
  var k=document.getElementById("zurueck-knopf"); if(!k) return;
  // Bewusst ohne regulaeren Ausdruck: dieser Text steht in einem Template-String
  // in schale.js, und dort verschluckt \/ den Backslash — die Regex kaeme als
  // // im Browser an und waere ein Kommentar. Gemessen am 05.09.2026.
  function ohneSchraeg(w){ while(w.length>1&&w.charAt(w.length-1)==="/") w=w.slice(0,-1); return w; }
  var pfad=ohneSchraeg(location.pathname);
  var ziel=ohneSchraeg((k.getAttribute("href")||"/").split("?")[0]);
  var eltern=pfad.slice(0,pfad.lastIndexOf("/"));
  if(ziel===pfad) k.setAttribute("href", eltern||"/");
  else if(eltern && eltern.length>ziel.length && (eltern+"/").indexOf(ziel+"/")===0) k.setAttribute("href", eltern);
  function eigenerVerlauf(){
    try{ return Boolean(document.referrer) && new URL(document.referrer).origin===location.origin
      && document.referrer!==location.href && history.length>1; }catch(e){ return false; }
  }
  function zurueck(ev){ if(ev) ev.preventDefault();
    if(eigenerVerlauf()) history.back(); else location.href=k.getAttribute("href"); }
  k.addEventListener("click",function(ev){
    if(ev.button!==0||ev.metaKey||ev.ctrlKey||ev.shiftKey||ev.altKey) return; // neuer Tab bleibt neuer Tab
    zurueck(ev); });
  document.addEventListener("keydown",function(ev){
    if(!ev.altKey||ev.key!=="ArrowLeft") return;
    if(document.querySelector("dialog[open]")) return;      // im Dialog gehoert der Pfeil dem Dialog
    var a=document.activeElement;
    if(a&&(a.isContentEditable||(a.matches&&a.matches("input,select,textarea")))) return; // im Feld ist er Textcursor
    zurueck(ev); });
})();
</script>
`;
