...
</script></head><body${["crm-start", "crm-leads", "crm-kunden"].includes(aktiv) ? ' class="crm-dashboard-design"' : ""}>
<div class="shell">
  <aside class="rail">
...
(function(){
  ...
  if (!el) return;
  var INAKTIV_MS = 10 * 60 * 1000;   // muss zu ZEIT_INAKTIV_SEK in lib/crm.js passen
  var HERZSCHLAG_MS = 15 * 1000;
  ...
  var basisSekunden = 0, seitWann = Date.now(), laeuft = false, letzteAktivitaet = Date.now(), tickTimer = null;
  ...
  setInterval(function () {
    if (!laeuft) return;
    if (Date.now() - letzteAktivitaet >= INAKTIV_MS) window.zeitAktion("pause");
    else anfragen("/crm/zeit/herzschlag", "POST").then(zustandUebernehmen);
  }, HERZSCHLAG_MS);

  ["mousemove", "keydown", "click", "scroll", "touchstart"].forEach(function (ev) {
    document.addEventListener(ev, function () { letzteAktivitaet = Date.now(); }, { passive: true });
  });
...
<script src="/sprache.js" defer></script>`;
}
