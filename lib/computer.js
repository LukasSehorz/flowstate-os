...
function angebotZeile(a) {
  const stueck = [
    `${a.preis} €`,
    a.linie,
    // "20:15–22:25 Uhr" wurde vorgelesen als "zwanzig Uhr fünfzehn bis
    // zweiundzwanzig Uhr fünfundzwanzig UHR" — lib/aussprache.js setzt das Wort
    // schon selbst. Das angehängte "Uhr" muss also weg (20.08.2026).
    a.ab && a.an ? `${a.ab}–${a.an}` : "",
    a.dauer,
    a.stopps === 0 ? "nonstop"
      : a.stopps ? `${a.stopps === 1 ? "ein" : a.stopps} Stopp${a.stopps > 1 ? "s" : ""}${a.umstieg ? " in " + (FLUGHAFEN[a.umstieg] || a.umstieg) : ""}` : "",
  ].filter(Boolean);
  return stueck.join(" · ");
}

// ------------------------------------------------- Zusicherung für die Stimme
//
// WARUM DAS EINE FUNKTION IM CODE IST UND KEINE EINMALIGE MESSUNG (20.08.2026):
// Der gesprochene Satz war gemessen 37 Wörter — und die VOLLSTÄNDIGE Antwort
// trotzdem 152. Denn hinter dem guten Satz hing die Liste: derselbe Flug ein
// zweites Mal, "eins. … zwei. … drei.", die Google-Adresse samt Abfrage und ein
// zweites "Gebucht hab ich nichts".
//
// Eine Messung hätte das gefunden. Sie hätte es aber nur EINMAL gefunden — beim
// nächsten Umbau wäre es zurück. Deshalb prüft der Code seinen eigenen
// Sprechtext, bevor er ihn herausgibt: Fällt er durch, geht der knappe
// Notfallsatz raus statt eines Maschinenabzugs. Lieber dünn als unbrauchbar.
//
// Geprüft wird, was die STIMME sagt (nach lib/aussprache.js), nicht der Rohtext
// — ausgeschriebene Zahlen sind länger als ihre Ziffern.
const SPRECH_GRENZE = 60;

function sprechtextPruefen(satz) {
  const gesprochen = require("./aussprache.js").fuerStimme(String(satz || ""));
  const maengel = [];
  const woerter = gesprochen.split(/\s+/).filter(Boolean).length;
  if (woerter > SPRECH_GRENZE) maengel.push(`${woerter} Wörter (Grenze ${SPRECH_GRENZE})`);
  if (/https?:|www\./i.test(satz)) maengel.push("enthält eine Adresse");
  if (/(^|\s)(eins|zwei|drei|vier)\.\s/.test(gesprochen)) maengel.push("liest eine Nummernliste vor");
  if (/Uhr\s+\S+\s+Uhr\b/.test(gesprochen) || /\bUhr\s+Uhr\b/.test(gesprochen)) maengel.push("sagt zweimal „Uhr“");
  if (/eins Stopp|eins Stunden?/.test(gesprochen)) maengel.push("Einzahl falsch");
  if ((gesprochen.match(/Gebucht hab ich nichts/g) || []).length > 1) maengel.push("sagt die Absage zweimal");
  if (/kg CO2|gesch(ä|ae)tzt|Durchgef(ü|ue)hrt von/i.test(satz)) maengel.push("enthält Rohdaten");
  return { ok: !maengel.length, maengel, woerter, gesprochen };
}
...
async function computerAuftrag(auftrag) {
  ...
  // DER SATZ IST DIE GANZE ANTWORT (20.08.2026).
  //
  // reply UND gesprochen tragen denselben einen Satz. Vorher stand in reply
  // zusätzlich die Liste, und /api/chat hängte sie an den Sprechtext an — aus
  // 37 Wörtern wurden 152, samt Adresse und doppelter Absage. Liste, Quelle und
  // Rohzeilen leben ab jetzt NUR in der Karte. Eine Karte wird angezeigt, nicht
  // vorgelesen; genau darum geht es hier.
  let satz = satzSprechen(zerlegt[0], { ort, hin, zurueck, grund: kalenderGrund });
  const probe = sprechtextPruefen(satz);
  if (!probe.ok) {
    // Der Notfallsatz. Er sagt das Nötigste und kann die Prüfung nicht
    // verfehlen, weil er nichts Zusammengesetztes enthält.
    satz = `Der günstigste Flug nach ${ort} kostet ${beste[0].preis} Euro. ` +
      "Die Verbindungen hab ich dir hingelegt. Gebucht hab ich nichts, das machst du selbst.";
    // Nicht still verschlucken: Wer das im Protokoll sieht, weiß, dass der
    // Satzbau nachgebessert gehört.
    try {
      require("./sprachlog.js").schreiben({
        art: "sprechtext-verworfen", was: "computer",
        maengel: probe.maengel, woerter: probe.woerter, original: probe.gesprochen.slice(0, 300),
      });
    } catch { /* ein Protokollfehler darf die Antwort nicht kippen */ }
  }
  ...
  return {
    ...
    reply: satz,
    gesprochen: satz,
    // Die Karte: zum Ansehen, nicht zum Vorlesen.
    karte: {
      art: "fluege",
      ueberschrift: `${w.von || "MUC"} → ${ort}, ${hin}${zurueck ? " bis " + zurueck : ""}`,
      hinweis: kalenderSatz.trim() || null,
      zeilen: zerlegt.map((a) => angebotZeile(a)),
      quelle: erg.url,
      fussnote: NICHT_GEBUCHT,
    },
    quelle: erg.url,
...
module.exports = { computerAuftrag, fensterSuchen, angeboteLesen, preisAus, chromePfad,
  ...
  tagSprechbar, terminKurz, sprechtextPruefen, SPRECH_GRENZE };
