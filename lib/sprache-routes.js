...
gehoert in "text" (KEINE Aktion) — fuer alles andere je eine Aktion. Sie laufen
gleichzeitig. Beispiel "was steht morgen an, sind neue Mails da, wie wird das
Wetter" -> text = die Termine, aktionen = mail + wetter.
Jede Aktion bekommt NUR ihren Arbeitsteil, nie die ganze Aeusserung.

wetter   — Wetter holen. auftrag MIT Tag ("Wetter uebermorgen in Dorfen").
mail     — Posteingang pruefen. auftrag = Zeitfenster ("letzte 2 Stunden").
           Nur lesen, nie senden.
wa_lesen — WhatsApp lesen. Fragt Lukas nach einem BESTIMMTEN Chat ("was hat
           Mama geschrieben"), ist auftrag NUR der Kontakt- oder Gruppenname.
           Fragt er ALLGEMEIN ("gibt's neue WhatsApp-Nachrichten", "was ist
           reingekommen"), setz auftrag auf "neue Nachrichten" — dann fasst der
           Server alle Chats zusammen. Er kennt nur Nachrichten seit dem
           Koppeln. Erfinde keine "nicht verbunden"-Ausrede.
gehirn   — Wissen, das NICHT unter "FIRMA" im STAND steht: frueher Entschiedenes,
           Projektnotizen, Wiki, Referenzen. auftrag = die Wissensfrage.
           Preise, Leistungen, Team und Positionierung stehen im STAND — die
           beantwortest du DIREKT, ohne Aktion. NICHT fuer Server-/
           Automatisierungs-Status (das weiss nur hermes). Nie gehirn UND
           hermes fuer DASSELBE Anliegen.
sonnet   — kurze Recherche mit Websuche: Fakten, Preise, News, Zahlen vergleichen.
hermes   — lange Arbeit UND JEDES Kommando ans System: Praesentation, Angebot,
           Marktrecherche, Website, externe Mails; Einstellungen/Modelle aendern,
           Cron-Jobs und Routinen anlegen, Automatisierungen bauen, Status
           abfragen. auftrag = das Kommando im Klartext, in Lukas' Worten.
...
SENDEN, Chats lesen, recherchieren und ueber hermes alles am OS aendern.
Kontakte loest der SERVER auf — du brauchst nie eine Nummer. Sag nie "ich kenne
X nicht", "das kann ich nicht", "dafuer bin ich nicht zustaendig", und erfinde
nie fehlende "API-Verbindungen". Im Zweifel geht es an hermes, niemals ablehnen.
Bei klarem Auftrag legst du sofort los und bestaetigst ruhig und verbindlich:
"Alles klar, das richte ich dir ein — ich meld mich, sobald's steht."
...
"wofuer brauchst du das?". Du liest Vorbereitetes immer von selbst vor. Nur wenn
ohne eine fehlende Angabe gar nicht startbar ist, fragst du kurz und konkret.

NIE FALSCH DEMENTIEREN: Behaupte nie, du haettest etwas NICHT getan, nur weil es
nicht im Verlauf steht. Sag ehrlich: "Hab ich gerade nicht mehr im Gespraech —
sag mir kurz, worum's ging."
...
## Sprechen
- ZUSAGE ZUERST, dann der Inhalt: erst ein Satz, was du anstoesst, dann die
  Termine/Zahlen. So verschwindet die Wartezeit hinter dem Vorlesen. Gibt es
  nichts anzustossen, bleibt "zusage" leer.
...
async function ausZustand(frage, zustandText, history = []) {
  ...
  try {
    ...
    return { zusage: "", text: "", mail: null, whatsapp: null, termin: null, aktionen: [{ was: "hermes", auftrag: "" }], zeige: [], _diag: diag };
  }
}
...
// Nennt dieser Satz etwas Konkretes — oder ist er eine leere Taetigkeitsfloskel?
//
// Rueckmeldung Lukas 25.07.: "Er sagt drei Mal: ich schau jetzt nach, ich schau
// jetzt nach, ich schau jetzt nach, nur in anderen Worten. Aber er beschreibt
// nie den Prozess." Der Prompt verlangt Konkretes bereits — Haiku haelt sich
// nicht immer daran. Also wird hier nachgeprueft statt gehofft.
//
// Der Trick fuer Deutsch: Substantive sind grossgeschrieben. Ein Satz, in dem
// nach dem ersten Wort kein eigenes Substantiv (und keine Zahl) vorkommt, nennt
// nichts, was man sehen oder entscheiden koennte.
//   "Ich schau mir das kurz an."            -> keins  -> Floskel
//   "Ich schau in die Bundesliga-Tabellen." -> Tabellen -> konkret
function nenntEtwasKonkretes(satz) {
  const ohneErstesWort = String(satz).replace(/^\S+\s*/, "");
  return /[A-ZÄÖÜ][a-zäöüß]{2,}/.test(ohneErstesWort) || /\d/.test(satz);
}

// Sagen zwei Saetze im Kern dasselbe? Vergleicht die tragenden Woerter.
function zuAehnlich(a, b) {
  const worte = (s) => new Set(String(s).toLowerCase()
    .replace(/[^\wäöüß\s]/g, " ").split(/\s+/).filter((w) => w.length > 3));
  const A = worte(a), B = worte(b);
  if (!A.size || !B.size) return false;
  let gleich = 0;
  for (const w of A) if (B.has(w)) gleich++;
  return gleich / Math.min(A.size, B.size) >= 0.6;
}

// Aus den Rohzeilen des Modells die brauchbaren Schritte ziehen: keine leeren
// Floskeln, keine Wiederholungen. Lieber zwei gute Saetze als vier, von denen
// drei dasselbe sagen — oder gar keiner, dann bleibt sie still.
function schritteFiltern(zeilen) {
  const raus = [];
  for (const z of zeilen) {
    if (!nenntEtwasKonkretes(z)) continue;
    if (raus.some((v) => zuAehnlich(v, z))) continue;
    raus.push(z);
  }
  return raus;
}
...
async function planBauen(id, auftrag, was = "hermes") {
  // Recherche ist in ~15 s durch, lange Arbeit braucht Minuten. Entsprechend
  // wenige Schritte — bei drei Ansagen fuer eine kurze Suche redet sie mehr,
  // als sie sucht (Rueckmeldung Lukas 25.07.). Und der Abschluss-Satz ("dauert
  // ein paar Minuten") passt nur zu langer Arbeit.
  const recherche = was === "sonnet";
  const anzahl = recherche ? 2 : 4;
  try {
    const roh = await schnell.frage(
      ...
      (recherche
        ? `Gib GENAU ${anzahl} Zeilen aus — deine Rechercheschritte.\n`
      ...
      "\nDas WICHTIGSTE: Sag, WAS du inhaltlich tust — nicht DASS du etwas tust.\n" +
      (recherche
        ? "  GUT     'Ich schau erst, was die grossen Tech-Seiten heute dazu melden.'\n" +
          "          'Jetzt gleich ich ab, ob die Zahlen ueberall gleich sind.'\n" +
          "  SCHLECHT 'Ich recherchiere das.' · 'Ich schau mir das an.' · 'Ich pruefe das.'\n"
        : "  GUT     'Ich fang mit der Gliederung an — drei Kernpunkte reichen.'\n" +
          "          'Jetzt kommen die Beispiele, sonst bleibt es abstrakt.'\n" +
          "  SCHLECHT 'Ich baue das.' · 'Ich kuemmere mich darum.' · 'Ich mach das fertig.'\n") +
      "Jede Zeile muss etwas nennen, das man SIEHT oder ENTSCHEIDET: eine Quelle, einen\n" +
      "Abschnitt, eine Zahl, eine Auswahl. Reine Taetigkeitsworte ohne Inhalt sind wertlos.\n\n" +
    ...
    // Der Abschluss-Satz darf eine Floskel sein — "ich meld mich" IST sein
    // Zweck. Deshalb zuerst abtrennen, dann die Arbeitsschritte filtern.
    // Bei Recherche gibt es keinen: sie ist durch, bevor er faellig waere.
    const abschluss = !recherche && zeilen.length > anzahl ? zeilen[anzahl] : "";
    const schritte = schritteFiltern(zeilen.slice(0, anzahl));

    const e = AUFTRAEGE.get(id);
    if (e && schritte.length) { e.plan = schritte; e.planIndex = 0; e.abschluss = abschluss; }
    else if (e && abschluss) { e.plan = []; e.planIndex = 0; e.abschluss = abschluss; }
    sprachlog.schreiben({
      art: "plan", id, was, schritte, abschluss,
      verworfen: zeilen.slice(0, anzahl).length - schritte.length,
    });
  } catch (e) {
...
module.exports = function (app, { layout }) {
  ...
  app.post("/api/sprache/frage", async (req, res) => {
    ...
    if (antwort.termin) {
      ...
      if (tm.was === "eintragen") {
        ...
        if (!gefunden) {
          ...
          erg = await werkzeuge.terminAendern({ id: gefunden.id, start: tm.start, ende: tm.ende, ort: tm.ort })
            .catch((e) => ({ ok: false, grund: String(e.message).slice(0, 200) }));
          if (erg.ok) {
            sprich = (sprich ? sprich + " " : "") + `Verschoben — ${gefunden.titel} ist jetzt ${zeitWort(erg.start, false)}.`;
            terminFakt = `\n[FAKT: Termin "${gefunden.titel}" verschoben auf ${erg.start}]`;
          } else {
    ...
    history.push({ role: "user", content: text });
    if (sprich) history.push({ role: "assistant", content: sprich + mailFakt + waFakt + terminFakt + laeuft });
  ...
  app.post("/api/sprache/zusage", async (req, res) => {
    ...
    try {
      const z = await schnell.frage(
        ...
        "WICHTIG: Du sprichst DIREKT mit Lukas. Bitte NIE um Kontext, frag NIE zurueck, rede NIE " +
        "ueber 'Lukas' in der dritten Person und antworte NIE als Werkzeug ('ich brauche mehr " +
        "Informationen'). Verstehst du seine Aeusserung nicht oder ist sie unvollstaendig, gib " +
        "einen LEEREN String zurueck — lieber nichts sagen als etwas Sinnloses.",
        text,
        { maxTokens: 40, temp: 0.5, timeoutMs: 6000, model: process.env.SCHNELL_ACK_MODEL || "claude-haiku-4-5" }
      );
      let zusage = String(z).replace(/^["'„»]+|["'"«]+$/g, "").trim().slice(0, 180);

      // Sicherung gegen aus-der-Rolle-Fallen (Fix 25.07.): Am 25.07. um 01:01
      // wurde Lukas vorgelesen: "Ich brauche mehr Kontext. Was hat Lukas dir
      // gerade gesagt? Bitte gib mir seinen S" — eine Rueckfrage ans System,
      // mitten im Wort abgeschnitten. So etwas darf nie gesprochen werden.
      const ausDerRolle = /brauche? mehr (kontext|informationen)|was hat lukas|gib mir (seinen|sein|die)|als (assistent|modell)|ich bin ein\b/i.test(zusage);
      // Abgeschnitten: endet mitten im Satz statt auf einem Satzzeichen.
      const abgeschnitten = zusage.length > 60 && !/[.!?…]$/.test(zusage);
      if (ausDerRolle || abgeschnitten) {
        sprachlog.schreiben({ art: "zusage-verworfen", frage: text.slice(0, 120), zusage, grund: ausDerRolle ? "aus der Rolle" : "abgeschnitten" });
        zusage = "";
      }
...
module.exports.nenntEtwasKonkretes = nenntEtwasKonkretes;
module.exports.zuAehnlich = zuAehnlich;
module.exports.schritteFiltern = schritteFiltern;
