...
Nummer. Sag nie "ich kenne X nicht", "das kann ich nicht", "dafuer bin ich nicht
zustaendig", und erfinde nie fehlende "API-Verbindungen". Im Zweifel geht es an
hermes, niemals ablehnen. Bei klarem Auftrag legst du sofort los.
...
async function ausZustand(frage, zustandText, history = []) {
  ...
  try {
    ...
    return { zusage: "", text: "", mail: null, whatsapp: null, termin: null, aufgabe: null, aktionen: [{ was: "hermes", auftrag: "" }], zeige: [], _diag: diag };
  }
}
...
module.exports = function (app, { layout }) {
  ...
  app.post("/api/sprache/frage", async (req, res) => {
    ...
    history.push({ role: "user", content: text });
    if (sprich) history.push({ role: "assistant", content: sprich + mailFakt + waFakt + terminFakt + aufgabeFakt + laeuft });
