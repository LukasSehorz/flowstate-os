  ...
  const frageBeantworten = async (req, res) => {
    ...
    if (antwort.termin) {
      ...
      if (tm.was === "eintragen") {
        ...
        // EINMAL NACHSEHEN, BEVOR SIE NEIN SAGT (20.08.2026). Der STAND ist bis
        // zu fuenf Minuten alt. Wurde der Termin in der Zwischenzeit direkt im
        // Google-Kalender angelegt — was Lukas staendig tut —, gaebe es ihn hier
        // nicht, und die Antwort waere ein falsches "finde ich nicht". Ein
        // frischer Abruf kostet eine Sekunde und nur dann, wenn es sonst beim
        // Nein bliebe. Gedeckelt wie oben.
        if (!gefunden) {
          await Promise.race([
            zustand.bauen(null, ["kalender"]).catch(() => {}),
      ...
      // sucht.
      //
      // DARAUF WIRD JETZT GEWARTET (Fix 20.08.2026). Vorher lief das Auffrischen
      // nebenher, und am laufenden Server sah das so aus:
      //
      ...
      //
      // Das Warten kostet rund eine Sekunde. Gedeckelt, weil ein haengender
      // Kalenderabruf niemals die Antwort aufhalten darf: Nach zweieinhalb
      // Sekunden geht es ohne ihn weiter, und dann ist es hoechstens wieder so
      // gut wie vorher.
      if (erg?.ok) {
        await Promise.race([
          zustand.bauen(null, ["kalender"]).catch(() => {}),
    ...
    await Promise.all(kurz.map(async (a) => {
      while (Date.now() < bisMs) {
        ...
        if (e?.fertig) {
          ...
          // Was der Chat zeigt, in dieser Reihenfolge:
          //   1. der gesprochene Satz (kurz, menschlich),
          //   2. bei mitDetails zusaetzlich die Liste darunter — drei Fluege
          //      vergleicht man nicht im Kopf (20.08.2026),
          //   3. sonst der rohe reply.
          //
          // KEINE TECHNISCHE FEHLERMELDUNG (Fix 20.08.2026). Vorher stand hier
          ...
          const t = e.gesprochen
            ? (e.mitDetails && e.reply ? e.gesprochen + "\n\n" + e.reply : e.gesprochen)
            : e.reply || (e.hint ? ehrlichGescheitert(e.was) : "");
          if (t) teile.push(t);
          return;
        }
