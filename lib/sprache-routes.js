...
{"zusage": "<1 kurzer Satz: was du parallel anstoesst — oder leer>",
 ...
 "spur": "keine" | "sonnet" | "hermes",
 "auftrag": "<falls spur nicht 'keine': EIN fokussierter Arbeitsauftrag, nur der Arbeitsteil>",
 "zeige": ["kalender"|"zahlen"]}

## DIE DREI SPUREN (Entscheidung Lukas, 21.07.)
"keine"  — Du beantwortest alles selbst aus dem STAND (Termine, Zahlen,
           Reports vorlesen, interne Mail aus dem Stand). Der Normalfall.
"sonnet" — Kurze Denk- oder Recherche-Aufgabe, Sekunden-Sache mit etwas mehr
           Logik: Wetter oder Fakten NACHSCHAUEN (Websuche!), einen Report
           ZUSAMMENFASSEN, CRM-Zahlen AUSWERTEN/vergleichen. Ein schnelles
           Modell mit Websuche erledigt das parallel in wenigen Sekunden.
"hermes" — Lange, echte Arbeit: PowerPoint/Slides bauen, Angebot erstellen,
           Marktrecherche, Website, Routineaufgaben anlegen ("jeden Abend
           Report schreiben"), externe Mails mit Verlauf.

WICHTIG bei "sonnet" und "hermes": KEINE Rueckfrage wie "soll ich nachsehen?"
— einfach machen. Zusage formulieren ("Schau ich kurz nach.") und los.
Wetter/aktuelle Fakten stehen NIE im STAND: immer spur "sonnet", nie raten,
nie rueckfragen.
...
braucht: lass "mail" null, setze spur "hermes" und formuliere den auftrag.
...
AUFTRAG: Gib der gewaehlten Spur NUR den Arbeitsteil als klaren Auftrag —
nicht die ganze Aeusserung. Lukas fragt "was steht morgen an und wie wird das
Wetter" -> du liest die Termine selbst vor (text), auftrag ist NUR "Wie wird
das Wetter morgen in Dorfen?" mit spur "sonnet". So wird nichts doppelt
bearbeitet. Bei Mails via Hermes: im Ton von Lukas verfassen (kein AI-Slop),
als Entwurf vorbereiten, NICHT senden.
...
async function ausZustand(frage, zustandText, history = []) {
  ...
  try {
    ...
    if (!m) return { zusage: "", text: roh.slice(0, 500), mail: null, spur: "keine", auftrag: "", zeige: [] };
    ...
    const mail = p.mail && p.mail.an && p.mail.betreff
      ...
      : null;
    // Spur bestimmen; aeltere Antworten mit braucht_hermes bleiben gueltig.
    const spur = ["keine", "sonnet", "hermes"].includes(p.spur)
      ? p.spur
      : (p.braucht_hermes ? "hermes" : "keine");
    return {
      zusage: String(p.zusage || "").slice(0, 300),
      text: String(p.text || "").slice(0, 900),
      mail,
      spur,
      auftrag: String(p.auftrag || p.hermes_auftrag || "").slice(0, 600),
      zeige: Array.isArray(p.zeige) ? p.zeige : [],
    };
  } catch {
    // Im Zweifel lieber langsam und richtig als schnell und daneben.
    return { zusage: "", text: "", mail: null, spur: "hermes", auftrag: "", zeige: [] };
  }
}
...
function denkAnfrage(text) {
  // Wetter: fester Handgriff (open-meteo, <1 s) statt Websuche (10-33 s).
...
// spur: "sonnet" -> schnelle Recherche (Sekunden), "hermes" -> lange Arbeit.
function auftragStarten(text, history, spur = "hermes") {
  const id = "a" + (++auftragZaehler) + "-" + Date.now().toString(36);
  AUFTRAEGE.set(id, { fertig: false, gestartet: Date.now(), spur });
  const lauf = spur === "sonnet" ? denkAnfrage(text) : hermesAnfrage(text, history);
  lauf.then((r) => {
...
module.exports = function (app, { layout }) {
  ...
  app.post("/api/sprache/frage", async (req, res) => {
    ...
    let mailLauf = null;
    // Spur & Auftrag: was jenseits von Zusage/Vorlesen noch zu tun ist.
    let spur = antwort.spur === "sonnet" || antwort.spur === "hermes" ? antwort.spur : null;
    let auftragText = spur ? (antwort.auftrag || text) : "";
    if (antwort.mail) {
      if (werkzeuge.istIntern(antwort.mail.an)) {
        ...
        // Externe Mail: nie direkt senden — als Entwurf an Hermes (Verlauf +
        // Freigabe). Ueberschreibt eine evtl. leichtere Spur, denn eine externe
        // Mail ist immer Hermes-Arbeit.
        spur = "hermes";
    ...
    // Recherche (Sonnet) oder lange Arbeit (Hermes) — parallel zum Vorlesen.
    // Fokussierter Auftrag statt Roh-Aeusserung: die Spur bearbeitet nur den
    // Arbeitsteil, nicht nochmal den Kalender, den Alexandra schon vorliest.
    if (spur) {
      auftragId = auftragStarten(auftragText, history.slice(-10), spur);
    }
    ...
    res.json({ ok: true, sprich, karten, auftragId, quelle: spur || "zustand" });
  });
  ...
  app.get("/api/sprache/auftrag/:id", async (req, res) => {
    ...
    // Scheitert die Spur (Timeout, Suchlimit), NIE stumm bleiben und nie ein
    // rohes "⚠️ Timeout" sprechen. Ein menschlicher Satz, der ehrlich sagt, dass
    // es nicht geklappt hat — je nach Spur passend.
    if (!a.reply && !a.gesprochen) {
      a.gesprochen = a.spur === "sonnet"
