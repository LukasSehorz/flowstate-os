      ...
      if (tm.was === "eintragen") {
        ...
        if (erg.ok) {
          ...
          try {
            ...
            if (st && Array.isArray(st.kalender) && erg.id) {
              const schon = st.kalender.some((t) => t.id === erg.id);
              if (!schon) {
                st.kalender.push({
                  id: erg.id,
                  start: erg.start,
                ...
                // Nach Startzeit sortiert halten — der STAND wird an mehreren
                // Stellen der Reihe nach vorgelesen.
                st.kalender.sort((a, b) => String(a.start).localeCompare(String(b.start)));
                zustand.schreiben(st);
              }
            }
          } catch { /* Der Termin ist eingetragen; das hier ist nur Beschleunigung. */ }
