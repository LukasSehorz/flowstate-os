...
const steuerVersand = require("./steuer-versand.js");
const gespraechAngebot = require("./gespraech-angebot.js");
const suche = require("./suche.js");
// Die Websuche steht auch weiter unten noch einmal als require() im
// Ausfuehrungszweig. Hier oben wird sie fuer istNachschlagefrage gebraucht —
// das Sicherheitsnetz laeuft VOR den Auftraegen und darf nicht erst laden.
const sucheWeb = require("./suche-web.js");
...
// Zum Abschnitt "IMMER ZUERST ZUSAGEN" weiter unten (19.08.2026): Dort stand
// "Ist raus." als Beispiel fuer eine gute Zusage — und vierzig Zeilen tiefer
// dasselbe "Ist raus." in der FALSCH-Liste. Das Modell hielt sich an die
// empfehlende Stelle, was folgerichtig war: Auf "loesch das Angebot" kam
// "Mach ich, das Angebot ist raus aus dem System", waehrend der Auftrag noch
// lief. Er scheiterte zehn Sekunden spaeter.
//
// Die Begruendung steht hier und nicht im Prompt selbst: Der Prosa-Teil darf
// 9000 Zeichen nicht ueberschreiten (scripts/test-prompt.js prueft das vor
// jedem Push), und jedes Wort mehr kostet bei JEDEM Aufruf Zeit. Regeln
// gehoeren in den Prompt, ihre Vorgeschichte in den Code.
const FORMAT_ANHANG = `
...
"Bin dran." Nur die ABSICHT, nie den Vollzug. Nicht fragen, ob du sollst.
...
function ausAufrufen(aufrufe) {
  ...
  const NACHSCHAUEN = { wetter: "wetter", mail_lesen: "mail", whatsapp_lesen: "wa_lesen",
                        ...
                        // 20.08.2026: beide laufen als KURZER Auftrag, warten also im
                        // Chat mit (bis 45 s) statt sich per Telegram zu melden. Genau
                        // das ist der Unterschied, den man in einer Aufnahme sieht.
                        computer_auftrag: "computer", leads_nachschub: "leads",
                        steuer_versand: "steuer",
                        gespraech_auswerten: "gespraech" };
  ...
  for (const a of aufrufe) {
    ...
    if (NACHSCHAUEN[a.name]) {
      ...
      if (raus.aktionen.length < 4) {
        ...
        // steuer_versand hat wie beleg_erstellen Einzelfelder (Zeitraum,
        // Adresse) statt eines Freitextauftrags — beide gehen als JSON durch.
        raus.aktionen.push(["beleg_erstellen", "steuer_versand", "gespraech_auswerten"].includes(a.name)
          ? { was: NACHSCHAUEN[a.name], auftrag: JSON.stringify(e).slice(0, 2000) }
...
async function ausZustand(frage, zustandText, history = [], onSatz = null) {
  ...
  try {
    ...
    try {
      ...
      // KEIN ZWEITER VERSUCH BEI LEEREM GUTHABEN (20.08.2026).
      //
      // Die Reserve ist ein anderes MODELL, nicht ein anderes Konto. Ist das
      // Guthaben aufgebraucht, scheitert sie mit demselben Fehler — nur eine
      // Sekunde spaeter. Danach greift der Notausgang unten und Alexandra sagt
      // "Mach ich — ich setz mich dran", ohne dass irgendetwas laeuft.
      //
      // Genau so ist es in der Nacht vor den Aufnahmen passiert: ab 02:32 Uhr
      // auf JEDE Frage eine Zusage in 0,4 bis 0,8 Sekunden, und nie ein
      // Ergebnis. Ein Ausfall, der wie Arbeit aussieht, ist schlimmer als
      // einer, der wie ein Ausfall aussieht.
      //
      // Deshalb hier abbrechen und es SAGEN. Der Satz nennt keinen
      // Schluessel und keinen Anbieter — nur, dass gerade nichts geht und
      // woran es liegt.
      if (require("./selbsttest.js").GUTHABEN_LEER.test(diag.fehler)) {
        diag.parse = "guthaben";
        return { text: "Ich komme gerade an gar nichts ran — das Guthaben fürs Denken ist aufgebraucht. " +
                       "Solange das so ist, kann ich dir nichts zusagen. Lad auf, dann bin ich sofort wieder da.",
    ...
    // Auch der aeussere Notausgang darf bei leerem Guthaben nicht an Hermes
    // abgeben — der laeuft ueber dasselbe Konto und schweigt dann ebenfalls.
    if (require("./selbsttest.js").GUTHABEN_LEER.test(diag.fehler)) {
      diag.parse = "guthaben";
      return { text: "Ich komme gerade an gar nichts ran — das Guthaben fürs Denken ist aufgebraucht. " +
                     "Solange das so ist, kann ich dir nichts zusagen. Lad auf, dann bin ich sofort wieder da.",
...
// ---------------------------------------------------------- Merkzettel
//
// Termine, die Alexandra in DIESEM Lauf selbst angelegt oder abgesagt hat.
// Der STAND wird periodisch komplett neu aus Google gebaut und kann beides
// noch nicht wissen: einen Termin von vor drei Sekunden kennt Google noch
// nicht, einen abgesagten fuehrt der alte STAND weiter.
//
// NEUNZIG SEKUNDEN, nicht fuenf Minuten (20.08., zweiter Anlauf).
//
// Der Merkzettel muss genau eine Gespraechsrunde ueberbruecken: eintragen,
// dann im naechsten Satz verschieben oder absagen. Das sind Sekunden. Der
// Neuaufbau im Hintergrund braucht 8,7 s und hat die Wahrheit danach.
//
// Mit fuenf Minuten wurde er selbst zum Problem: Wird ein Termin AUSSERHALB
// geloescht — beim Aufraeumen eines Testlaufs, oder wenn Lukas ihn im Handy
// wegwischt —, fuehrt der Merkzettel ihn weiter. Zwei Laeufe kurz
// hintereinander ergaben dann "Da passen mehrere — ADSTEST Probeaufnahme
// oder ADSTEST Probeaufnahme. Welchen meinst du?", und der zweite war eine
// Leiche aus dem Lauf davor. Je kuerzer er lebt, desto weniger kann er
// falsch liegen.
const MERK_MS = 90 * 1000;
const merkNeu = new Map();     // id -> { termin, bis }
const merkWeg = new Map();     // id -> bis   (abgesagt, aber evtl. noch im STAND)

function merkAufraeumen() {
  const jetzt = Date.now();
  for (const [k, v] of merkNeu) if (v.bis < jetzt) merkNeu.delete(k);
  for (const [k, bis] of merkWeg) if (bis < jetzt) merkWeg.delete(k);
}
function frischerTermin(t) {
  merkAufraeumen();
  if (t && t.id) { merkNeu.set(t.id, { termin: t, bis: Date.now() + MERK_MS }); merkWeg.delete(t.id); }
}
function terminWeg(id) {
  merkAufraeumen();
  if (id) { merkWeg.set(id, Date.now() + MERK_MS); merkNeu.delete(id); }
}
// Der Kalender, wie er WIRKLICH aussieht: STAND plus eigene Neuzugaenge,
// minus eigene Absagen, ohne Doppelte.
function kalenderJetzt() {
  merkAufraeumen();
  const roh = (zustand.lesen().kalender || []).concat([...merkNeu.values()].map((v) => v.termin));
  const gesehen = new Set();
  return roh.filter((t) => {
    if (!t || !t.id || merkWeg.has(t.id) || gesehen.has(t.id)) return false;
    gesehen.add(t.id);
    return true;
...
// Lange Arbeit geht an Hermes — seit Stufe 2 (19.09.2026) ueber lib/hermes.js:
// Runs-API statt Chat-Completion, Auftragsbuch statt .catch(() => {}),
// Ereignisstrom statt 300-Sekunden-Abbruch. melde(teilText) bekommt weiter
// den bisherigen Text, damit Alexandra unterwegs sagen kann, woran sie ist.
//
// history wird NICHT mehr mitgeschickt: Hermes fuehrt den Verlauf selbst unter
// der session_id "sprache" (Rueckfragen und Freigaben beziehen sich darauf).
// Kommt das Ergebnis erst nach der Auftragsfrist (AUFTRAG_FRIST_MS, unten),
// reicht lib/hermes.js es per Telegram nach — nichts geht mehr verloren.
async function hermesAnfrage(text, history = [], melde = null) {
  const hermes = require("./hermes.js");
  if (!hermes.verfuegbar()) return { ok: false, hint: "HERMES_URL fehlt." };
  return hermes.auftrag(text, {
    quelle: "stimme", sessionId: "sprache", melde,
    spaetMelden: Number(process.env.AUFTRAG_FRIST_MS || 120000),
  });
}
...
const GUELTIGE_MODELLE = ["gpt-6-astra", "gpt-5.6-sol", "gpt-5.6-luna", "gpt-5.6-terra"];
...
function markerAufloesen(history, id, gelungen, auftragText = "") {
  ...
  for (const h of history) {
    ...
    // Beim Scheitern reicht es NICHT, den Marker zu ersetzen (19.08.): Der
    // zugesagte Satz steht ja weiter davor im selben Eintrag. Am 19.08. stand
    // dort "Mach ich, das Angebot ist raus aus dem System." — der Auftrag
    // scheiterte, der Marker wurde brav auf FEHLGESCHLAGEN gesetzt, und beim
    // naechsten "Hey Alexandra" las das Modell den Fliesstext und sagte "Das
    // Angebot fuer BK-Dach ist weg". Es hatte recht: So stand es da.
    //
    // Deshalb wird der Widerruf ausdruecklich formuliert, statt sich darauf zu
    // verlassen, dass der Marker den Satz davor entkraeftet.
    const widerruf = gelungen
      ? `[FAKT: erledigt und bestaetigt — ${was}]`
      : `[FAKT: FEHLGESCHLAGEN, nicht erledigt — ${was}. ACHTUNG: Was in DIESER `
        + `Nachricht als getan zugesagt wurde, ist NICHT geschehen. Sag es von `
        + `dir aus, wenn es zur Sprache kommt, und behandle es nie als erledigt.]`;
    h.content = h.content.slice(0, i) + widerruf + h.content.slice(ende + 1);
  }
  return history;
}
  ...
  try {
    require("./telegram.js").push(text, { stimme: true })
      .then((z) => sprachlog.schreiben({
        ...
        // "gelungen" hiess frueher dieses Feld und wurde als ZUSTELL-Erfolg
        // gelesen — gemeint war aber, ob der AUFTRAG etwas geliefert hat.
        // "ZUSTELLUNG: gelungen:false" bei erfolgreich zugestellter
        // Fehlermeldung ist zweimal missverstaendlich (20.08.2026).
        ok: Boolean(z?.ok), auftragHatteErgebnis: Boolean(gelungen), grund: z?.grund || null,
      }))
...
// Ein gescheiterter Auftrag, in Worten statt in Fehlercodes.
//
// EINE Quelle fuer beide Kanaele (20.08.2026). Der Sprachweg hatte diese Saetze
// seit jeher, der getippte Chat nicht — dort stand `Das hat nicht geklappt:
// ${hint}`, und hint ist der ROHE Text aus dem Werkzeug. Im Morning Briefing
// las Lukas deshalb mitten im Absatz "Das hat nicht geklappt: Mail: spawn
// gws-cli ENOENT". Der Grund gehoert ins Sprachprotokoll, wo er beim Suchen
// hilft — nicht in einen Satz, der vorgelesen wird.
function ehrlichGescheitert(was) {
  if (was === "hermes") return "Da bin ich gerade nicht durchgekommen — ich häng mich später nochmal ran.";
  if (was === "mail") return "An deine Mails komm ich gerade nicht ran.";
  if (was === "wa_lesen") return "Auf WhatsApp komm ich gerade nicht drauf.";
  if (was === "computer") return "Die Flugsuche ist mir gerade weggebrochen — sag nochmal, dann probier ich's neu.";
  if (was === "leads") return "Die Leads krieg ich gerade nicht zusammengestellt.";
  return "Das krieg ich gerade nicht geladen — frag mich gleich nochmal, dann schau ich neu.";
}
...
// nutzerId (20.08.2026): Der Lead-Nachschub legt eine Anrufliste an — und die
// gehoert dem, der danach gefragt hat. Ohne diese Angabe fiel sie auf den
// alphabetisch ersten Admin zurueck: Lukas sagte "such mir hundert raus", und
// die Liste landete bei Jannik. Genau so gemessen am 20.08.
function auftragStarten(text, history, was = "hermes", { vonMehreren = false, nutzerId = null } = {}) {
  ...
  const lauf =
    ...
    // Computer Use und Lead-Nachschub (20.08.2026). Beide bewusst HIER und
    // nicht bei Hermes: Als kurzer Auftrag haengt das Ergebnis noch an
    // derselben Antwort, statt Minuten spaeter per Telegram anzukommen.
    was === "computer" ? require("./computer.js").computerAuftrag(text) :
    was === "leads" ? require("./leads-nachschub.js").leadsNachschub(text, { nutzerId }) :
    // Websuche ueber lib/suche-web.js (20.08.2026): Serper-Suche und -News
    // parallel, bis zu vier echte Seiten lesen, EIN Modellaufruf daraus.
    // Gemessen ueber 27 Fragen: Median 2,4 s, langsamste 4,4 s, keine ueber 6.
    //
    // Sie liefert "gesprochen" gleich mit. inStimme() ueberspringt Ergebnisse,
    // die das Feld schon haben — damit entfaellt ein zweiter Modellaufruf, der
    // am 07.08. mit 3,5 s gemessen wurde. Das ist mehr Zeitgewinn als die
    // ganze Suche kostet.
    was === "sonnet" ? require("./suche-web.js").websuche(text, { verlauf: history || [] }) :
  ...
  // JEDER AUFTRAG HAT EINE FRIST (Fix 20.08.2026).
  //
  // Live gemessen: "Schick den Ordner mit den Juli-Rechnungen an die
  // Steuerberaterin" -> "Geb ich weiter — die Juli-Rechnungen werden
  // zusammengestellt und die Mail vorbereitet." Auftrag a12-mt0q1u7x gestartet
  // um 01:27:33. Fuenfzehn Minuten spaeter: immer noch nicht fertig, keine
  // Meldung, kein Fehler. Im Sprachprotokoll steht dann "NIE fertig geworden".
  //
  // Das ist der schlimmste Ausgang von allen — schlimmer als ein Fehler. Sie
  // hat etwas zugesagt, und danach passiert nichts, wovon jemand erfaehrt.
  // Genau das Versprechen ("ich melde mich, wenn es steht") ist der Kern der
  // Anzeige.
  //
  // Die einzelnen Wege bringen zwar eigene Zeitgrenzen mit — aber offenbar
  // nicht alle und nicht zuverlaessig (Hermes haengt trotz AbortSignal, weil
  // undici darunter eigene Grenzen hat). Deshalb hier, an der EINEN Stelle,
  // durch die jeder Auftrag laeuft: Nach der Frist gibt es eine Antwort, und
  // zwar eine ehrliche. Die eigentliche Arbeit laeuft im Hintergrund weiter und
  // schadet nichts; sie kann das Ergebnis nur nicht mehr nachreichen.
  const FRIST_MS = Number(process.env.AUFTRAG_FRIST_MS || (art === "lang" ? 120000 : 60000));
  const abgelaufen = new Promise((r) => {
    const u = setTimeout(() => r({
      ok: false, ueberfaellig: true,
      hint: `Nach ${Math.round(FRIST_MS / 1000)} s keine Antwort`,
    }), FRIST_MS);
    u.unref?.();
  });

  // Meldet sich der eigentliche Lauf NACH der Frist noch mit einem Fehler, ist
  // das kein unbehandelter Fehlschlag mehr — der Auftrag ist laengst
  // abgeschlossen. Ohne diese Zeile wuerde Node ihn als "unhandled rejection"
  // melden und im schlimmsten Fall den Prozess beenden.
  lauf.catch(() => {});

  Promise.race([lauf, abgelaufen]).then((r) => {
    const eintrag = AUFTRAEGE.get(id);
    // Eine abgelaufene Frist ist kein Ergebnis — sie bekommt ihren eigenen,
    // schon fertig formulierten Satz, damit inStimme() nichts mehr daraus
    // machen muss (es gibt ja keinen Inhalt).
    if (r?.ueberfaellig) r = { ...r, gesprochen: ehrlichGescheitert(was) };
...
module.exports = function (app, { layout }) {
  ...
  app.get("/sprache", (req, res) => {
    res.send(layout(AGENT + " — Sprache", "sprache", seite(), req));
  });

  // ------------------------------------------------------------- Die Buehne
  //
  // Nur fuer die Aufnahme (20.08.2026). Ablauf im Werbespot: Jannik kommt
  // herein, klatscht zweimal, das Gehirn faehrt auf, Alexandra begruesst ihn,
  // danach schiebt sich das Command Center dazu.
  //
  // WARUM EINE SEITE UND NICHT ZWEI FENSTER. Der Wunsch war, zwei offene Tabs
  // per Klatschen nach vorn zu holen. Das kann kein Browser: Ein Tab darf sich
  // nur nach einer echten Nutzergeste in den Vordergrund bringen — Klick oder
  // Taste. Ein Geraeusch zaehlt nicht, und die Sperre laesst sich nicht
  // abschalten. Vor der Kamera waere geklatscht worden und nichts passiert.
  //
  // Hier liegt beides auf EINER Seite: die Sprachbuehne, und darunter die
  // Zentrale in einem Rahmen. Sie ist von Anfang an geladen — genau das
  // "schon offen, nur zugeklappt", das gemeint war. Beim Klatschen wird nur
  // noch aufgeblendet, nichts nachgeladen, nichts kann haengen.
  app.get("/buehne", (req, res) => {
    res.send(layout("Flowstate OS", "sprache", seite() + `
<div id="buehne-vorhang" class="bu-vorhang" aria-hidden="true"></div>
<section id="buehne-hud" class="bu-hud" aria-hidden="true">
  <iframe id="buehne-rahmen" title="Command Center" src="/?buehne=1" loading="eager"></iframe>
</section>
<script>
(function () {
  "use strict";
  // body.buehne — daran haengt der Abdunkler vor dem Klatschen. Die Klasse
  // gehoert AUSSCHLIESSLICH hierher: Ohne sie stand die Regel auf
  // body.jarvis und dimmte das Gehirn auch auf /sprache, wo nie geklatscht
  // wird. Es war dort dauerhaft auf 7 % Deckkraft und praktisch unsichtbar.
  document.body.classList.add("buehne");
  // Die Buehne haengt sich an dieselbe Klatsch-Erkennung wie die Sprachseite.
  // public/sprache.js meldet den Weckruf als Ereignis; hier wird daraus die
  // Inszenierung. Ohne dieses Skript verhaelt sich /buehne wie /sprache.
  var vorhang = document.getElementById("buehne-vorhang");
  var hud = document.getElementById("buehne-hud");
  var gestartet = false;

  function auffahren() {
    if (gestartet) return;
    gestartet = true;
    document.body.classList.add("buehne-wach");
    // Das Gehirn zuerst — es ist der Moment, auf den die Kamera wartet.
    // Das Pult kommt zwei Sekunden spaeter, wenn ihr Satz laeuft.
    setTimeout(function () { document.body.classList.add("buehne-hud-an"); }, 2100);
  }

  document.addEventListener("alexandra-geweckt", auffahren);

  // Notausgang fuer den Dreh: Leertaste oder Klick auf den Vorhang tut
  // dasselbe wie zweimal klatschen. Wenn im Studio etwas klemmt, soll
  // niemand am Mikrofon herumraten muessen.
  document.addEventListener("keydown", function (e) {
    if (e.code === "Space" && !gestartet) { e.preventDefault(); auffahren(); }
  });
  if (vorhang) vorhang.addEventListener("click", auffahren);
  ...
  const frageBeantworten = async (req, res) => {
    ...
    // Dasselbe fuer den Steuer-Versand: "ja", "nein", eine Mailadresse oder
    // ein anderer Monat werden hier gefangen, BEVOR das Modell den Satz sieht.
    // Sonst deutete es ein blosses "ja" als neue Frage.
    try {
      const st = await steuerVersand.antwortAuf(text);
      if (st) {
        sprachlog.schreiben({ art: "frage", frage: text.slice(0, 200), quelle: "steuer-rueckfrage", ok: st.ok !== false });
        return antworten({ ok: true, sprich: st.gesprochen || st.reply, text: st.reply, quelle: "steuer" });
      }
    } catch (e) { console.error("Steuer-Rueckfrage:", e.message); }
    ...
    // Der STAND und die Liste der laufenden Auftraege sind ZWEI Dinge, auch
    // wenn sie zusammen an das Modell gehen (20.08.2026, beim Messen gefunden):
    // laufendText() haengt Lukas' eigene Frage woertlich an ("LAEUFT SEIT 3 s:
    // Wie steht der SV Oberbergkirchen in der Tabelle?"). Wer den Gesamttext
    // fragt, ob der STAND einen Namen kennt, bekommt darum ab der zweiten Runde
    // immer "ja" — und das Sicherheitsnetz unten hat genau deshalb nie gegriffen
    // (gemessen: 5 von 20). Der STAND allein ist die ehrliche Auskunft.
    const standText = zustand.alsText();
    const zustandText = standText + laufendText(req.session.laufend);
    ...
    // SICHERHEITSNETZ FUER DIE AUSSENFRAGE (20.08.2026).
    //
    // Gemessen ueber /api/chat, Lauf 2 von 5, woertlich aus dem Sprachprotokoll:
    //
    //   "verstehen": { "aufrufe": [], "tokenDenken": 100 }, "quelle": "zustand",
    //   "gesagt": "Das Ergebnis ist gerade durch, aber bei mir kommt keine
    //              Tabellenplatzierung an. Sag mir die Liga oder Saison, dann
    //              geh ich gezielt ran."
    //
    // `aufrufe: []` — es wurde nie gesucht. Das Modell hat gedacht, sich aus dem
    // Gespraechsverlauf zusammengereimt, die Suche liefere nichts, und das als
    // Tatsache vorgetragen. Vier von fuenf Laeufen waren richtig; dieser eine
    // ist der, der in der Aufnahme steht.
    //
    // Am Prompt ist das nicht dichtzukriegen — dieselbe Erfahrung steht in
    // dieser Datei schon zweimal (Zusage-Netz 07.08., Termin-Netz weiter unten).
    // Deshalb hier eine Pruefung, die nicht vom Modell abhaengt: Hat das
    // Verstehen GAR NICHTS gerufen und kann der STAND die Frage nachweislich
    // nicht beantworten (fremder Name + Frage nach etwas Taeglichem), wird
    // gesucht. Was das Modell stattdessen sagen wollte, faellt weg — es war
    // eine Auskunft ueber eine Suche, die nie stattgefunden hat.
    //
    // Das Netz kann nur BESSER machen: Es greift ausschliesslich dort, wo sonst
    // nichts passiert waere. Die Regeln dafuer stehen in
    // lib/suche-web.js -> istNachschlagefrage.
    if (!(antwort._diag?.aufrufe || []).length && !antwort.mail && !antwort.whatsapp
        && !antwort.termin && !antwort.aufgabe && !antwort.crm
        && sucheWeb.istNachschlagefrage(text, standText, history)) {
      aktionen.push({ was: "sonnet", auftrag: text });
      if (antwort._diag) antwort._diag.netz = "recherche-nachgezogen";
      // Leeren statt ersetzen: Das Zusage-Netz weiter unten baut daraus von
      // selbst "Mach ich — ich recherchiere kurz." Ein zweiter fester Satz an
      // dieser Stelle waere eine dritte Formulierung fuer dieselbe Sache.
      //
      // Im Stromfall ist der Satz schon gesprochen und nicht mehr einzufangen.
      // Dann bleibt er stehen und die Auskunft kommt hinterher — immer noch
      // besser als eine Ausrede ohne Suche.
      if (!strom) antwort.text = "";
    }
    ...
    for (const a of aktionen) {
      ...
      if (a.was === "steuer") {
        try {
          const r = await steuerVersand.vorbereiten(req.session.crm, { ...JSON.parse(a.auftrag), text });
          const satz = r.gesprochen || r.reply;
          sofort = sofort ? sofort + " " + satz : satz;
        } catch (e) {
          console.error("Steuer-Versand:", e.message);
          sofort = "Das Paket fuer die Steuerberaterin konnte ich nicht zusammenstellen.";
        }
        continue;
      }
      if (a.was === "beleg" || a.was === "gespraech") {
        try {
          // "gespraech" holt Leistung, Argumente und Preis erst aus dem CRM
          // und uebergibt dann an denselben Belegweg — die Rueckfrage "soll
          // ich es raussenden?" bleibt dadurch an EINER Stelle.
          const r = a.was === "gespraech"
            ? await gespraechAngebot.ausGespraech(JSON.parse(a.auftrag))
            : await belegErstellen.erstellen(JSON.parse(a.auftrag));
          // gesprochen, nicht reply (20.08.): reply traegt den ganzen
          // Angebotstext fuer den Chat. Hier wurde er bisher mitgesprochen —
          // 828 Zeichen Aufzaehlung am Stueck. beleg-erstellen.js liefert
          // laengst ein kurzes "gesprochen"; genutzt wurde es nur nicht.
          const satz = r.gesprochen || r.reply;
          sofort = sofort ? sofort + " " + satz : satz;
        } catch (e) {
      ...
      auftraege.push(auftragStarten(a.auftrag || text, history.slice(-10), a.was,
        { vonMehreren: (antwort.aktionen || []).length > 1, nutzerId: req.session.crm?.id || null }));
    }
    ...
    if (!sprich && auftraege.length) {
      const NENNEN = {
        ...
        // 20.08.2026: Die Flugsuche dauert rund zwanzig Sekunden. Ohne diesen
        // Satz steht Lukas so lange in der Stille — und das ist die Einstellung,
        // die man in der Aufnahme wiederholen muesste.
        computer: "mach den Browser auf und such die Fluege",
        leads: "stell dir die Leads zusammen",
      };
    ...
    if (antwort.termin) {
      ...
      // KEINE ZUSAGE VOR EINEM MISSERFOLG (Fix 20.08.2026).
      //
      // Der Prompt verlangt, dass Alexandra zuerst zusagt und dann arbeitet —
      // sonst hoert Lukas sekundenlang nichts. Der Preis dafuer wurde live
      // sichtbar, als er einen Termin absagen liess, den es nicht gibt:
      //
      //   "Mach ich, der Kamera-Check morgen fliegt raus.
      //    Ich finde keinen Termin, der auf 'Kamera-Check' passt —
      //    sag mir kurz, welchen du meinst."
      //
      // Zwei von zwei Laeufen. Der Zuschauer hoert "fliegt raus" und danach das
      // Gegenteil; im Video ist das schlimmer als eine Sekunde Stille.
      //
      // Am Prompt ist das nicht zuverlaessig zu loesen — die Regel "nur die
      // ABSICHT, nie den Vollzug" steht laengst drin, und das Modell hat sie
      // trotzdem gebogen. Also hier: Ging der Termin schief, faellt die Zusage
      // WEG und es bleibt bei dem, was der Server sicher weiss.
      //
      // Nur wenn sonst nichts lief. Hat dieselbe Runde noch Mails geholt oder
      // eine Nachricht vorbereitet, steht in sprich auch deren Antwort — die
      // darf ein misslungener Termin nicht mitreissen.
      const sonstNichts = () => !auftraege.length && !antwort.mail && !antwort.whatsapp
        && !antwort.aufgabe && !antwort.crm && !sofort;
      const melden = (satz, gelungen) => {
        sprich = (!gelungen && sonstNichts()) ? satz : (sprich ? sprich + " " : "") + satz;
      };

      if (tm.was === "eintragen") {
        erg = await werkzeuge.terminEintragen(tm).catch((e) => ({ ok: false, grund: String(e.message).slice(0, 200) }));
        if (erg.ok) {
          // Den frischen Termin SOFORT in den STAND legen (20.08.2026).
          //
          // Warum nicht einfach neu einlesen: Gemessen im Container braucht
          // zustand.bauen(["kalender"]) 8,7 Sekunden — der Deckel weiter unten
          // liegt bei 2,5. Das Wettrennen verliert also immer, und der STAND
          // bleibt alt. Genau daran ist die Drehbuchszene gescheitert:
          // "Trag mir 16 Uhr Probeaufnahme ein" (klappt), zwei Sekunden
          // spaeter "sag den Termin wieder ab" -> "Ich finde keinen Termin,
          // der darauf passt". Der Termin STAND im Kalender; nur der STAND
          // wusste es noch nicht.
          //
          // terminFinden() liest ausschliesslich aus dem STAND. Ein Eintrag,
          // den wir selbst gerade angelegt haben, muss dort nicht erst ueber
          // Google zurueckgeholt werden — wir kennen ihn ja. Das kostet
          // nichts und wirkt sofort.
          // Der Eintrag geht in den MERKZETTEL, nicht in den STAND (20.08.).
          //
          // In den STAND zu schreiben hat zweimal nicht getragen, und beide
          // Male anders:
          //   - Der periodische zustand.bauen() ueberschreibt die Datei
          //     komplett. Laeuft er zwischen "eintragen" und "absagen", ist
          //     der Eintrag weg — Google kennt den Termin da noch nicht.
          //     Gemessen: "Trag mir morgen um neun ein" klappt, "schieb ihn"
          //     drei Sekunden spaeter -> "Ich finde keinen Termin".
          //   - Umgekehrt blieben abgesagte Termine als Leichen liegen, weil
          //     das Aufraeumen sie bei Google loescht, nicht im STAND. Nach
          //     ein paar Laeufen stand derselbe Termin vierfach da, und die
          //     Antwort war "Da passen mehrere — X oder X oder X oder X".
          //
          // Der Merkzettel liegt daneben und gehoert uns: Was wir selbst
          // angelegt haben, wissen wir sicher; was wir abgesagt haben, auch.
          // Beides ueberlebt jeden Neuaufbau, und nach fuenf Minuten ist es
          // ohnehin ueber die regulaere Auffrischung im STAND.
          if (erg.id) frischerTermin({
            id: erg.id, start: erg.start, ende: erg.ende || "",
            titel: tm.titel, ort: tm.ort || "",
          });

          melden(`Steht — ${tm.titel}, ${zeitWort(erg.start, tm.ganztags)}.`, true);
          terminFakt = `\n[FAKT: Termin "${tm.titel}" eingetragen: ${erg.start}]`;
        } else {
          melden("Beim Eintragen hat's gehakt — der Termin ist nicht drin.", false);
          terminFakt = `\n[FAKT: Termin "${tm.titel}" NICHT eingetragen: ${erg.grund}]`;
        }
      } else {
        // Verschieben/Absagen: erst den gemeinten Termin im Kalender finden.
        // kalenderJetzt() statt des rohen STAND: enthaelt, was wir gerade
        // selbst angelegt haben, und nicht mehr, was wir abgesagt haben.
        gefunden = terminFinden(tm.suche, kalenderJetzt(), tm.tag);
        // HIER STAND EIN NACHSCHLAG, DER NIE FUNKTIONIEREN KONNTE (raus am
        // 20.08.2026). Die Idee war: Findet sich nichts im STAND, einmal frisch
        // einlesen und nochmal suchen — gedeckelt auf 2,5 Sekunden, damit ein
        // haengender Abruf die Antwort nicht aufhaelt.
        //
        // Nachgemessen im Container braucht zustand.bauen(["kalender"]) aber
        // 8,7 Sekunden. Das Wettrennen ging also IMMER an den Deckel: Der STAND
        // blieb unveraendert, gesucht wurde ein zweites Mal in denselben Daten,
        // und ein "finde ich nicht" kam 2,5 Sekunden spaeter als noetig. Kosten
        // ohne jeden Nutzen, und im Gespraech ist das die Pause, in der Lukas
        // denkt, es haenge.
        //
        // Der Fall, den es abfangen sollte — Termin gerade selbst angelegt —
        // ist eine Ebene hoeher geloest: Der Eintrag wandert direkt in den
        // STAND ("Den frischen Termin SOFORT in den STAND"). Bleibt der Fall
        // "in Google von Hand angelegt"; den faengt die regulaere Auffrischung
        // ab, nicht ein Abruf mitten im Satz.
        if (!gefunden) {
          melden(`Ich finde keinen Termin, der auf „${tm.suche || "das"}“ passt — sag mir kurz, welchen du meinst.`, false);
          ...
          melden(`Da passen mehrere — ${liste}. Welchen meinst du?`, false);
          ...
          if (erg.ok) {
            // Der Merkzettel bekommt den NEUEN Stand. Sonst sucht der naechste
            // Zug ("und sag ihn wieder ab") nach der alten Uhrzeit.
            frischerTermin({
              id: gefunden.id, start: erg.start || gefunden.start,
              ende: erg.ende || gefunden.ende || "",
              titel: neuerName || gefunden.titel, ort: gefunden.ort || "",
            });
            ...
            melden("Das Verschieben hat nicht geklappt — der Termin steht noch wie vorher.", false);
          ...
          erg = await werkzeuge.terminAbsagen({ id: gefunden.id })
            .catch((e) => ({ ok: false, grund: String(e.message).slice(0, 200) }));
          if (erg.ok) {
            // Auf den Merkzettel: Der STAND fuehrt den Termin noch, bis die
            // naechste Auffrischung laeuft. Ohne das taucht ein gerade
            // abgesagter Termin im naechsten Zug wieder auf.
            terminWeg(gefunden.id);
            melden(`Abgesagt — ${gefunden.titel} ist raus.`, true);
            terminFakt = `\n[FAKT: Termin "${gefunden.titel}" abgesagt.]`;
          } else {
            melden("Das Absagen hat nicht geklappt — der Termin steht noch.", false);
            terminFakt = `\n[FAKT: Termin "${gefunden.titel}" NICHT abgesagt: ${erg.grund}]`;
          }
        }
      }

      // Hat sich etwas geaendert, ist der Kalender im STAND veraltet — und der
      // STAND ist die einzige Quelle, aus der terminFinden() im NAECHSTEN Zug
      // sucht.
      //
      // DER STAND MUSS SOFORT STIMMEN (20.08.2026). Am laufenden Server sah es
      // vorher so aus:
      //
      //   "Trag mir morgen um neun ADSTEST Kamera-Check ein."
      //   -> "Steht — ADSTEST Kamera-Check, Freitag, 21. August um 09:00."
      //   "Schieb ADSTEST Kamera-Check auf halb elf."      (2,4 s spaeter)
      //   -> "Ich finde keinen Termin, der auf ADSTEST Kamera-Check passt."
      //
      // Der Termin stand wirklich im Kalender. Der STAND wusste es nur noch
      // nicht. Genau diese Abfolge steht als Szene im Drehbuch (Creative 2:
      // Termine ansehen, einen loeschen, einen neuen eintragen) — sie waere in
      // der Aufnahme auseinandergefallen.
      //
      // NACHGEZOGEN STATT NEU EINGELESEN (20.08.2026, zweiter Anlauf).
      //
      // Erster Anlauf war ein gedeckeltes Warten auf zustand.bauen(). Das
      // konnte nicht aufgehen: Der Neuaufbau braucht gemessene 8,7 Sekunden,
      // der Deckel lag bei 2,5 — das Rennen ging immer an den Deckel, der
      // STAND blieb alt, und die Antwort kam trotzdem 2,5 Sekunden spaeter.
      //
      // Beim Eintragen loest der Code das laengst richtig: Der neue Termin
      // wird direkt in den STAND gelegt, weil wir ihn ja kennen. Fuers
      // Verschieben und Absagen gilt dasselbe — auch da wissen wir genau, was
      // sich geaendert hat. Kein Netzaufruf, kein Warten, sofort wirksam.
      if (erg?.ok && gefunden?.id) {
        try {
          const st = zustand.lesen();
          if (st && Array.isArray(st.kalender)) {
            if (tm.was === "absagen") {
              st.kalender = st.kalender.filter((t) => t.id !== gefunden.id);
            } else {
              const t = st.kalender.find((x) => x.id === gefunden.id);
              if (t) {
                if (erg.start) t.start = erg.start;
                if (tm.ende) t.ende = tm.ende;
                if (tm.titel && tm.titel !== gefunden.titel) t.titel = tm.titel;
                if (tm.ort) t.ort = tm.ort;
              }
              st.kalender.sort((a, b) => String(a.start).localeCompare(String(b.start)));
            }
            zustand.schreiben(st);
          }
        } catch { /* Die Aenderung IST durch; das hier ist nur Beschleunigung. */ }
      }
      // Im Hintergrund trotzdem sauber neu einlesen — dann stimmen auch die
      // Felder, die wir hier nicht kennen. Bewusst OHNE await: Es dauert 8,7
      // Sekunden und niemand wartet darauf.
  ...
  app.post("/api/chat", async (req, res) => {
    ...
    // Karten werden ANGEZEIGT, nicht vorgelesen. Sie sammeln sich hier und
    // gehen als eigenes Feld raus — nie als Anhang an den Sprechtext.
    const karten = [...(ergebnis.karten || [])];
    ...
    await Promise.all(kurz.map(async (a) => {
      while (Date.now() < bisMs) {
        ...
        if (e?.fertig) {
          ...
          // WAS GESAGT WIRD UND WAS NUR ZU SEHEN IST (20.08.2026).
          //
          // Früher wurde hier bei "mitDetails" die ganze Liste an den
          // Sprechtext gehängt. Aus 37 gesprochenen Wörtern wurden dadurch 152:
          // derselbe Flug ein zweites Mal, "eins. zwei. drei.", die
          // Google-Adresse samt Abfrage, die Absage zweimal. Der Text war für
          // den Chat gedacht — vorgelesen wurde er trotzdem.
          //
          // Jetzt strikt getrennt: gesprochen/reply ist der Satz, alles
          // Nachschlagbare kommt als KARTE und wird nie gesprochen.
          //
          // KEINE TECHNISCHE FEHLERMELDUNG. Vorher stand hier
          // `Das hat nicht geklappt: ${e.hint}`, und im Morning Briefing las
          // Lukas mitten im Text "Das hat nicht geklappt: Mail: spawn gws-cli
          // ENOENT". Der Sprachweg macht es seit jeher richtig (siehe
          // /api/sprache/auftrag/:id weiter unten) — der Chat zog nicht mit.
          if (e.karte) karten.push(e.karte);
          const t = e.gesprochen || e.reply || (e.hint ? ehrlichGescheitert(e.was) : "");
          if (t) teile.push(t);
          return;
        }
    ...
    res.json({ ok: true, reply: reply || "Fertig.", karten });
  });
  ...
  app.get("/api/sprache/auftrag/:id", async (req, res) => {
    ...
    if (!a.reply && !a.gesprochen) a.gesprochen = ehrlichGescheitert(a.was);
  ...
  app.get("/api/sprache/status", (req, res) => {
    ...
    res.json({
      ...
      agent: AGENT,
      // Welches Creative im Dreh laeuft. Kommt vom SERVER, nicht aus dem Link.
      //
      // WARUM (20.08.2026): Der Modus haengt bisher an ?drehbuch=… Beim Test
      // wurde /sprache in einem neuen Tab geoeffnet — Frage weg, Modus aus, und
      // Jannik sprach mit dem echten Agenten. Im Protokoll steht dann eine
      // "frage" mit Modell-Aufruf statt eines Zuges. Am Bildschirm sieht man
      // den Unterschied nicht.
      drehbuch: process.env.DREH_CREATIVE || null,
...
// Cache-Stempel, dasselbe Muster wie v() in lib/schale.js: Der Browser cacht
// /lib/... aggressiv, und ohne wechselnde URL sieht nach einem Deploy die halbe
// Firma noch das alte Gehirn. Der Zeitstempel der Datei genuegt dafuer.
function stempel(datei) {
  try {
    const st = fs.statSync(path.join(__dirname, "..", "public", datei.replace(/^\//, "")));
    return `${datei}?v=${Math.round(st.mtimeMs).toString(36)}`;
  } catch {
    return datei;   // Datei fehlt: ohne Stempel ausliefern, nicht abstuerzen
  }
}

// ---------------------------------------------------------------- Seite
//
// DIE SPRACHBUEHNE (19.08.2026)
//
// Vorher stand hier eine SVG-Kugel mit drei gestrichelten Ringen und 64
// Pegelbalken. Sie war sauber gebaut, aber sie war ein Symbol fuer eine
// Assistentin, kein Bild von ihr — und diese Seite wird abgefilmt und als
// Meta-Anzeige geschaltet. Aus zwei Metern Entfernung ist ein rotierender
// Ring eine Ladeanzeige.
//
// Jetzt steht dort ein GEHIRN aus rund 13.000 Fadenpunkten auf einem Canvas.
// Gezeichnet wird es von public/lib/jarvis-gehirn.js; dort steht auch, wie es
// technisch funktioniert. Diese Funktion liefert nur noch die Buehne drumherum
// und — wichtig — jeden Haken, an dem public/sprache.js haengt:
//
//   #kugel (data-zustand, Klick, Tastatur, ARIA), #kugel-wort, #zustand-text,
//   #kugel-hinweis, #btn-wake, #wake-punkt, #btn-pause, #btn-stop,
//   #btn-frisch, #stimme-info, #karten, #verlauf und die .pegel-Linien.
//
// An public/sprache.js wurde KEINE Zeile geaendert. Das Gehirn liest den
// Zustand ueber einen MutationObserver am data-zustand und den Mikrofon- bzw.
// Stimmpegel aus denselben .pegel-Linien, die die Bedienlogik ohnehin Bild
// fuer Bild beschriftet — sie sind hier nur unsichtbar (0x0 px). Die Leitung
// bleibt, nur die Anzeige daran ist neu.

function seite() {
  // Der Pegelgeber. public/sprache.js sucht in #kugel nach ".pegel" und
  // schreibt dort das Frequenzband hin. 40 Linien reichen: der Analyser
  // liefert 64 Baender, und die oberen tragen bei Sprache fast nichts.
  const pegelLinien = Array.from({ length: 40 }, (_, i) =>
    `<line class="pegel" data-i="${i}" x1="0" y1="0" x2="1" y2="0"/>`).join("");

  // Die Signalanzeige unten links. Reine Ablesung des echten Pegels — ohne
  // Ton stehen alle Striche flach, es wird nichts erfunden.
  const pegelBalken = Array.from({ length: 28 }, () => "<span></span>").join("");

  // Die vier Eckwinkel. Gezeichnete SVG, kein Unicode-Zeichen — dasselbe
  // Erkennungszeichen wie .j-panel in public/jarvis.css, nur um die ganze
  // Buehne statt um eine Kachel.
  const winkel = `<svg viewBox="0 0 26 26" fill="none" stroke="currentColor" stroke-width="1"
      aria-hidden="true" focusable="false"><path d="M.5 8.5V.5H8.5"/></svg>`;

  return `
<!--
DIREKTIONSVERTRAG — Sprachseite (Alexandra)

THESIS: A voice assistant is not a button you press. It is a mind you
interrupt. Show the mind.

OWN-WORLD: Instrument black, one blue. Everything drawn in thread and light:
no fills, no cards, no panel around the subject.

STORY: It wakes out of a scatter cloud, breathes, leans its fibres outward
when spoken to, races while thinking, pushes rings from the core while
answering.

FIRST VIEWPORT: A brain of 13000 thread points, sixty percent of the height,
lit from inside. Name upper left, state upper right, one cue and four
controls below.

FORM: Canvas 2D, additive. Every reading is real: audio, state, speech
engine. No invented numbers.

FINISH: unreviewed and undocumented is unfinished; this build ends with the finish review, the verdict, and DESIGN.md
-->
<script>
// Zwei Kleinigkeiten, die vor dem ersten Malen passieren muessen.
//
// 1) Die body-Klassen. lib/schale.js vergibt sie nach dem Muster
//    crm-dashboard-design (dort: eine Liste von Seitennamen), setzt aber
//    "jarvis" noch nicht. Bis das dort nachgezogen ist, setzt die Seite ihre
//    Klassen selbst — hier, ganz am Anfang des Inhalts, damit es vor dem
//    ersten Bild geschieht und die helle Fassung nicht aufblitzt.
//      .jarvis        aktiviert die gemeinsame HUD-Sprache (public/jarvis.css)
//      .jarvis-buehne aktiviert NUR diese Seite (public/jarvis-gehirn.css) und
//                     haelt sie fest dunkel, egal was der Hell/Dunkel-Schalter
//                     des OS sagt.
//
// 2) Der Direktionsvertrag muss der erste Kindknoten des <body> sein. Der
//    Rumpf der Seite kommt aus lib/schale.js und faengt mit <div class="shell">
//    an — die Datei gehoert einem anderen Bauabschnitt und wird hier nicht
//    angefasst. Also wandert der Kommentar nach dem Parsen an seinen Platz.
(function () {
  document.body.classList.add("jarvis", "jarvis-buehne");
  var k = document.currentScript ? document.currentScript.previousSibling : null;
  while (k && k.nodeType !== 8) k = k.previousSibling;
  if (k) document.body.insertBefore(k, document.body.firstChild);
})();
</script>

<section class="gh" id="gh-buehne" data-zustand="ruhe">
  <canvas class="gh-canvas" id="gh-canvas" aria-hidden="true"></canvas>
  <div class="gh-raster" aria-hidden="true"></div>
  <div class="gh-vignette" aria-hidden="true"></div>
  <div class="gh-rahmen" aria-hidden="true">${winkel}${winkel}${winkel}${winkel}</div>

  <div class="gh-kopf">
    <div class="gh-marke">
      <!-- Ueber der Ueberschrift stand bis 19.08. ein Kicker ("SPRACHKERN").
           Der ist in den Qualitaetsvorgaben ausdruecklich verboten — die
           Ueberschrift traegt sich selbst, und ALEXANDRA tut das sowieso.
           Ersatzlos gestrichen, keine kleinere Fassung davon. -->
      <!-- #kugel-wort schreibt public/sprache.js: in Ruhe "ALEXANDRA", sonst
           das, was gerade passiert. Bisher war die Zeile hidden — jetzt ist
           sie die Anzeige der Seite. -->
      <span id="kugel-wort">${AGENT.toUpperCase()}</span>
      <div class="gh-spur" aria-hidden="true"></div>
    </div>
    <div class="gh-block rechts">
      <span class="j-label">Zustand</span>
      <span class="gh-lampe"><i aria-hidden="true"></i><b id="zustand-text" aria-live="polite">bereit</b></span>
    </div>
  </div>

  <div class="gh-mitte">
    <!-- Der Klickbereich ueber dem Gehirn. Rolle, Tastaturbedienung und
         Beschriftung sind unveraendert; public/lib/jarvis-gehirn.js passt nur
         seine Groesse an das an, was man tatsaechlich leuchten sieht. -->
    <div id="kugel" data-zustand="ruhe" role="button" tabindex="0"
         aria-label="Sprechen — klicken oder „Hey ${AGENT}“ sagen">
      <svg class="gh-pegelquelle" viewBox="0 0 2 2" aria-hidden="true" focusable="false">${pegelLinien}</svg>
    </div>
  </div>

  <div class="gh-fuss">
    <!-- Zweimal klatschen steht ZUERST (20.08.): Es ist der Weg, der in der
         Anzeige gezeigt wird, und der einzige, der ganz ohne Beruehrung und
         ohne Stichwort auskommt.
         Aber nur im DREH (31.08.): Im Alltag horcht sie nicht mehr von selbst
         mit — das Mikrofon geht erst beim Klick auf die Kugel an. Ein Hinweis,
         der zum Klatschen auffordert, waehrend niemand zuhoert, waere die
         unfreundlichste Art, das zu erklaeren. -->
    <div id="kugel-hinweis" aria-live="polite">${process.env.DREH_CREATIVE
      ? `Zweimal klatschen — oder „Hey ${AGENT}“ sagen`
      : "Zum Sprechen die Kugel anklicken"}</div>

    <div class="gh-leiste" role="group" aria-label="Sprachsteuerung">
      <!-- Die Beschriftung von Pause und Neu einlesen schreibt
           public/sprache.js zur Laufzeit um (textContent). Ein <svg> im Knopf
           wuerde dabei geloescht — deshalb steckt das Symbol in einer
           CSS-Maske (public/jarvis-gehirn.css). Gezeichnet ist es trotzdem. -->
      <button id="btn-wake" class="vx-tool" type="button"
              title="Dauerhaft auf „Hey ${AGENT}“ lauschen">Wake<span class="vx-dot" id="wake-punkt"></span></button>
      ...
      <button id="btn-frisch" class="vx-tool" type="button" title="Kalender & Zahlen neu einlesen">Neu einlesen</button>
    </div>

    <div class="gh-fussleiste">
      <div class="gh-block">
        <span class="j-label">Signal</span>
        <div class="gh-pegel" aria-hidden="true">${pegelBalken}</div>
      </div>
      <div class="gh-block rechts">
        <span class="j-label">Ausgabe</span>
        <span class="vx-voice" id="stimme-info">wird geprüft …</span>
      </div>
    </div>
  </div>

  <!-- Unsichtbare Haken fuer die Bedienlogik: Karten und Verlauf werden von
       public/sprache.js gefuellt, auf der Buehne aber nicht gezeigt. Auf einer
       Seite, die man ANSPRICHT, ist ein mitlaufendes Protokoll Beiwerk. -->
  <div id="karten" hidden></div><div id="verlauf" hidden></div>
</section>

<!-- Das Gehirn selbst. Eigene Datei, weil es fast 600 Zeilen Mathematik sind
     und der Browser sie zwischen zwei Deploys cachen soll. -->
<script src="${stempel("/lib/jarvis-gehirn.js")}" defer></script>
`;
}
