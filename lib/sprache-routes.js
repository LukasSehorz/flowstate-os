  ...
  app.post("/api/sprache/frage", async (req, res) => {
    ...
    if (req.session.pendingWa) {
      ...
      if (/\b(ja|jo|jep|jup|passt|genau|schick|abschick\w*|senden?|raus|mach|los|ok|okay|jawohl)\b/i.test(text)) {
        ...
        const sp = r.ok ? `Ist raus — die WhatsApp an ${p.name}.` : "Bei der WhatsApp hakt's gerade — ist die Verbindung gekoppelt?";
        history.push({ role: "user", content: text });
        history.push({ role: "assistant", content: sp + (r.ok ? `\n[FAKT: WhatsApp an ${p.name} gesendet: "${p.text.slice(0, 150)}"]` : "") });
