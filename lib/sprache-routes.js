...
function modellHinweis(auftrag) {
  ...
  const a = auftrag
    .replace(/gpt[-\s]?\d[.,]?\d?[-\s]?sol\w*/gi, "gpt-5.6-sol")
    .replace(/gpt[-\s]?\d[.,]?\d?[-\s]?luna\w*/gi, "gpt-5.6-luna")
    .replace(/gpt[-\s]?\d[.,]?\d?[-\s]?terra\w*/gi, "gpt-5.6-terra")
    .replace(/\bsol(?:ana|ar|a)?\b/gi, "gpt-5.6-sol")
    .replace(/\b(luna|mond)\b/gi, "gpt-5.6-luna")
    .replace(/\b(terra|erde)\b/gi, "gpt-5.6-terra");
  return a +
...
function auftragStarten(text, history, was = "hermes") {
  ...
  const lauf =
    ...
    hermesAnfrage(modellHinweis(text), history);   // Modellnamen absichern
  lauf.then((r) => {
