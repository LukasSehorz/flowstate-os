...
Empfaenger-Namen (oder der Nummer) und dem Nachrichtentext im Ton von Lukas.
Der Server prueft selbst, ob der Kontakt intern ist (dann direkt) oder extern
(dann Rueckfrage) und loest den Namen ueber die Kontakte auf. Du verschickst NIE
an viele auf einmal (kein Serienversand).
    ...
    if (antwort.whatsapp && antwort.whatsapp.text) {
      ...
      if (!ziel || !ziel.nummer) {
        ...
        const r = await Promise.race([
          whatsapp.senden({ an: ziel.nummer, text: antwort.whatsapp.text }).catch((e) => ({ ok: false, grund: String(e.message) })),
          new Promise((ok) => setTimeout(() => ok(null), 8000)),
        ]);
        if (r && r.ok) {
          sprich = (sprich ? sprich + " " : "") + `Die WhatsApp an ${ziel.name} ist raus.`;
          waFakt = `\n[FAKT: WhatsApp an ${ziel.name} wurde gesendet.]`;
        } else if (r && !r.ok) {
