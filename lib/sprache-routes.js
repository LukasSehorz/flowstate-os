...
async function planBauen(id, auftrag) {
  try {
    const roh = await schnell.frage(
      ...
      "Gib GENAU 4 Zeilen aus, eine pro Arbeitsschritt, in der Reihenfolge deines Vorgehens:\n" +
      ...
      "- Jede Zeile anders — nicht viermal dieselbe Formulierung.",
      "AUFTRAG: " + String(auftrag).slice(0, 400),
      { maxTokens: 160, temp: 0.8, timeoutMs: 8000, model: process.env.SCHNELL_ACK_MODEL || "claude-haiku-4-5" }
    );
    const schritte = String(roh).split("\n")
      .map((z) => z.replace(/^\s*(?:[-*•]|\d+[.)])\s*/, "").replace(/^["'„»]+|["'“«]+$/g, "").trim())
      .filter((z) => z.length > 8 && z.length < 160)
      .slice(0, 4);
    const e = AUFTRAEGE.get(id);
    if (e && schritte.length) { e.plan = schritte; e.planIndex = 0; }
    sprachlog.schreiben({ art: "plan", id, schritte });
  } catch (e) {
    sprachlog.schreiben({ art: "plan", id, schritte: [], fehler: String(e.message).slice(0, 150) });
  }
}
...
function auftragStarten(text, history, was = "hermes") {
  ...
  // Schritt sei fertig.
  if (was === "hermes") planBauen(id, text);
...
module.exports = function (app, { layout }) {
  ...
  const ERZAEHL_ABSTAND_MS = 12000;   // nicht haeufiger reden als alle 12 s
  const ERZAEHL_ZUWACHS = 180;        // und nur bei echtem neuem Inhalt
