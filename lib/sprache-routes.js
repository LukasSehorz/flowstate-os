...
Du KANNST NICHT: beliebige PRIVATE WhatsApp-Chats durchsuchen/mitlesen (z. B. "was
hat meine Mutter geschrieben"). Nur EINGEHENDE geschaeftliche WhatsApp fliessen
ins Gehirn. Sag das ehrlich — erfinde KEINE "WhatsApp nicht verbunden"-Ausrede.
...
woertlich abgeschickt). Der Server loest den Namen auf, prueft intern/extern
und formuliert aus. Du verschickst NIE an viele auf einmal (kein Serienversand).
...
module.exports = function (app, { layout }) {
  ...
  app.post("/api/sprache/frage", async (req, res) => {
    ...
    // WhatsApp senden — Leitplanken (Entscheidung Lukas 22.07.): intern (Jannik,
    // Team) ohne Rueckfrage; extern erst nach Freigabe; nie Serienversand (das
    // Modell liefert genau EINEN Empfaenger). Bekannte Nummer noetig.
    let waFakt = "";
    if (antwort.whatsapp && antwort.whatsapp.text) {
      const ziel = kontakte.finde(antwort.whatsapp.an);
      if (!ziel || !ziel.nummer) {
        sprich = (sprich ? sprich + " " : "") +
          `Für ${antwort.whatsapp.an || "den Kontakt"} hab ich keine WhatsApp-Nummer hinterlegt — gib sie mir einmal, dann schick ich's.`;
        waFakt = `\n[FAKT: WhatsApp an ${antwort.whatsapp.an} nicht gesendet — keine Nummer im Kontaktbuch.]`;
      } else if (!ziel.intern) {
        sprich = (sprich ? sprich + " " : "") +
          `${ziel.name} ist nicht als intern hinterlegt. Soll ich die WhatsApp wirklich rausschicken? Sag kurz ja.`;
        // v1: extern nicht automatisch senden (Freigabeschleife folgt).
      } else {
        // Sonnet formuliert die Nachricht in Lukas' Ton aus (nicht woertlich!).
        const nachricht = await komponiere("WhatsApp", ziel.name, antwort.whatsapp.text);
        const r = await Promise.race([
          whatsapp.senden({ an: ziel.nummer, text: nachricht }).catch((e) => ({ ok: false, grund: String(e.message) })),
          new Promise((ok) => setTimeout(() => ok(null), 12000)),
        ]);
        if (r && r.ok) {
          sprich = (sprich ? sprich + " " : "") + `Die WhatsApp an ${ziel.name} ist raus.`;
          waFakt = `\n[FAKT: WhatsApp an ${ziel.name} gesendet, Wortlaut: "${nachricht.slice(0, 200)}"]`;
        } else if (r && !r.ok) {
          sprich = (sprich ? sprich + " " : "") + "Bei der WhatsApp hakt's gerade — ist die Verbindung gekoppelt?";
          waFakt = `\n[FAKT: WhatsApp an ${ziel.name} fehlgeschlagen: ${r.grund || "unbekannt"}]`;
        }
      }
    }
