...
async function hermesStream(url, headers, koerper, melde) {
  ...
  if (!r.ok || !r.body) throw new Error("Streaming nicht moeglich (" + r.status + ")");
...
async function hermesAnfrage(text, history = [], melde = null) {
  ...
  if (melde && process.env.HERMES_STREAM !== "aus") {
    try {
      const reply = await hermesStream(url, headers, koerper, melde);
      return { ok: true, reply };
    } catch (e) {
      // Kein Streaming moeglich -> ganz normal weiter. Einmal vermerken, damit
      // im Protokoll steht, warum keine Zwischenstaende kamen.
