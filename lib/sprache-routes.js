...
MAIL: Bittet Lukas um eine Mail, deren Inhalt komplett aus dem STAND kommt
(z. B. Terminuebersicht an ihn selbst), fuellst du "mail" direkt aus — der
Server verschickt sie. "an mich" heisst lukas.sehorz@hotmail.com. Fuer Texte
in Lukas' Namen gelten die STILPROBEN unten (natuerlich, direkt, kein
AI-Slop: keine formelhaften Doppelpunkt-Strukturen, kein "nicht X, sondern Y").
Braucht die Mail Recherche oder geht sie an Externe, lass mail null und setze
braucht_hermes.
...
async function ausZustand(frage, zustandText, history = []) {
  ...
  try {
    ...
    return { zusage: "", text: "", mail: null, braucht_hermes: true, zeige: [] };
  }
}
...
module.exports = function (app, { layout }) {
  ...
  app.post("/api/sprache/frage", async (req, res) => {
    ...
    // Mail, deren Inhalt komplett aus dem Stand kommt (z. B. Terminuebersicht
    // an Lukas selbst): direkt ueber den Werkzeugkasten senden — kein Hermes.
    // Laeuft PARALLEL zum Vorlesen: Wir warten hier nicht darauf.
    let mailLauf = null;
    if (antwort.mail) {
      mailLauf = werkzeuge.mailSenden(antwort.mail)
        .then((r) => ({ ok: true, ...r }))
        .catch((e) => ({ ok: false, fehler: String(e.message).slice(0, 200) }));
    }
    ...
    if (mailLauf) {
      ...
      if (erg?.ok) sprich = (sprich ? sprich + " " : "") + "Die Mail an dich ist raus.";
      else if (erg && !erg.ok) sprich = (sprich ? sprich + " " : "") + "Bei der Mail hakt's gerade, ich kümmere mich.";
    ...
    if (sprich) history.push({ role: "assistant", content: sprich });
