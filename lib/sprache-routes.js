...
{"zusage": "<1 kurzer Satz: was du parallel anstoesst — oder leer>",
 ...
 "termin": {"titel": "<kurz>", "start": "<JJJJ-MM-TTTHH:MM>", "ende": "<oder weglassen>", "ganztags": true/false, "ort": "<oder leer>"} oder null,
...
wa_lesen — einen WhatsApp-Chat lesen. auftrag = NUR der Kontaktname ("Mama").
           Er kennt nur Nachrichten seit dem Koppeln. Erfinde keine "WhatsApp
           nicht verbunden"-Ausrede — nutze die Aktion.
...
## Termine eintragen
Soll etwas in den Kalender ("trag mir morgen 10 Uhr Sport ein", "Freitag 14 Uhr
Termin mit Mueller"), fuellst du "termin" aus — der Server traegt ihn SOFORT ein
(etwa eine Sekunde). Dafuer KEINE Aktion an hermes, das dauerte frueher fast eine
Minute.
- "start" als JJJJ-MM-TTTHH:MM in Lukas' Ortszeit. Rechne relative Angaben aus
  HEUTE im STAND aus ("morgen", "Freitag", "in zwei Wochen").
- "ende" nur, wenn er eine Dauer nennt — sonst weglassen (dann eine Stunde).
- Ganztaegig ("den ganzen Tag", "Urlaub am 5."): "ganztags": true, "start" dann
  nur JJJJ-MM-TT.
- Fehlt die Uhrzeit voellig und es ist kein Ganztagstermin, frag EINMAL kurz nach.
Der Server bestaetigt danach; erst dann darfst du sagen, dass er steht.
...
async function ausZustand(frage, zustandText, history = []) {
  ...
  try {
    ...
    const termin = p.termin && p.termin.titel && p.termin.start
      ? {
...
module.exports = function (app, { layout }) {
  ...
  app.post("/api/sprache/frage", async (req, res) => {
    ...
    let terminFakt = "";
    if (antwort.termin) {
      const t = await werkzeuge.terminEintragen(antwort.termin)
        .catch((e) => ({ ok: false, grund: String(e.message).slice(0, 200) }));
      const wann = new Date(t.ok ? t.start : antwort.termin.start);
      const wannText = isNaN(wann) ? "" : wann.toLocaleString("de-DE", {
        timeZone: "Europe/Berlin", weekday: "long", day: "numeric", month: "long",
        ...(antwort.termin.ganztags ? {} : { hour: "2-digit", minute: "2-digit" }),
      });
      if (t.ok) {
        sprich = (sprich ? sprich + " " : "") +
          `Steht — ${antwort.termin.titel}${wannText ? ", " + wannText : ""}.`;
        terminFakt = `\n[FAKT: Termin "${antwort.termin.titel}" wurde eingetragen: ${t.start}${t.ganztags ? " (ganztaegig)" : ""}.]`;
        // Der eigene Kalender im STAND ist damit veraltet — sofort nachziehen,
        // sonst antwortet die naechste Frage aus dem alten Stand.
        zustand.bauen(null, ["kalender"]).catch(() => {});
      } else {
        sprich = (sprich ? sprich + " " : "") +
          "Beim Eintragen hat's gehakt — ich hab den Termin nicht reinbekommen.";
        terminFakt = `\n[FAKT: Termin "${antwort.termin.titel}" NICHT eingetragen: ${t.grund}]`;
      }
      sprachlog.schreiben({
        art: "termin", ok: t.ok, titel: antwort.termin.titel,
        start: t.ok ? t.start : antwort.termin.start, ganztags: antwort.termin.ganztags,
        grund: t.grund || null,
      });
    }
