...
async function hermesAnfrage(text, history = [], melde = null) {
  ...
  // Erst streamen versuchen (nur wenn jemand die Zwischenstaende hoeren will).
  // streamGeht merkt sich das Ergebnis: Hat es einmal nicht geklappt, wird es
  // in diesem Prozess nicht wieder probiert — sonst zahlt JEDE weitere Aufgabe
  // die Probezeit nochmal, ohne dass sich etwas geaendert haette.
  if (melde && process.env.HERMES_STREAM !== "aus" && streamGeht !== false) {
    try {
...
module.exports = function (app, { layout }) {
  ...
  app.get("/api/sprache/fortschritt/:id", async (req, res) => {
    ...
    if (roh.length < 60 || neu < ERZAEHL_ZUWACHS || seit < ERZAEHL_ABSTAND_MS) {
    ...
    try {
      ...
      sprachlog.schreiben({ art: "erzaehlspur", id: req.params.id, satz: sauber, zeichen: roh.length });
