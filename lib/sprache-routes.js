...
## Das Ergebnis NICHT selbst sagen — beim Tun wie beim Nachsehen
TUN (termin_*, aufgabe_*, crm_*, *_senden): Server bestaetigt in einer Sekunde.
Sag gar nichts — ausser was er NICHT sagt (Warnung, Kollision).

NACHSEHEN (daten_fragen, nachschlagen, neuigkeiten, recherchieren, mail_lesen,
whatsapp_lesen, gehirn_suchen, wetter): Hier sagst du IMMER zuerst EINEN kurzen
Satz, WAS du nachsiehst — und ZWAR BEVOR du das Werkzeug aufrufst, als erstes
in deiner Antwort. Er wird sofort gesprochen, waehrend der Aufruf laeuft;
laesst du ihn weg, hoert Lukas sekundenlang gar nichts und denkt, es haengt.
In deinen Worten, passend zur Frage: "Moment, ich zaehl die Leads bei Ioannis
nach." Keine leere Floskel, nie zweimal derselbe Satz, nie das Ergebnis
vorwegnehmen oder raten.
  ...
  for (const a of aufrufe) {
    ...
    if (NACHSCHAUEN[a.name]) {
      ...
      if (raus.aktionen.length < 4) {
        ...
        raus.aktionen.push({ was: NACHSCHAUEN[a.name], auftrag: s(e.auftrag || e.frage, 600) });
      }
      continue;
    }
...
module.exports = function (app, { layout }) {
  ...
  app.post("/api/sprache/frage", async (req, res) => {
    ...
    const antwort = await ausZustand(text, zustandText, history.slice(-6), strom ? sendeSatz : null);
