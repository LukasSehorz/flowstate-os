...
Unten steht der aktuelle Stand der Firma. Beantworte die Frage NUR daraus.

Regeln:
- Was nicht im Stand steht, weißt du nicht. Rate nie, rechne nichts hoch.
- Reicht der Stand nicht für die Frage, setze braucht_hermes auf true und lass text leer.
- Braucht die Frage eine Handlung (Mail schreiben, Termin eintragen, recherchieren,
  jemanden anrufen), setze ebenfalls braucht_hermes auf true.
...
{"text": "<was du sagst, leer wenn braucht_hermes>", "braucht_hermes": false, "zeige": ["kalender"|"zahlen"]}`;
...
module.exports = function (app, { layout }) {
  ...
  app.post("/api/sprache/frage", async (req, res) => {
    ...
    if (auftragId) {
      sprich = sprich
        ? sprich + " Den Rest setze ich auf, ich sag dir gleich Bescheid."
  ...
  app.get("/api/sprache/status", (req, res) => {
    res.json({
      ...
      wakeWord: process.env.WAKE_WORD || "alexandra",
    });
  });
};
...
(() => {
  ...
  const wort = document.getElementById("kugel-wort");
  const zustandText = document.getElementById("zustand-text");
  const hinweis = document.getElementById("kugel-hinweis");
  const karten = document.getElementById("karten");
  const verlauf = document.getElementById("verlauf");
  const btnWake = document.getElementById("btn-wake");
  const wakePunkt = document.getElementById("wake-punkt");
  const btnStop = document.getElementById("btn-stop");
  const stimmeInfo = document.getElementById("stimme-info");

  let konfig = { elevenlabs: false, hermes: false, wakeWord: "alexandra" };
  let zustand = "ruhe";
  let audio = null;
  let wakeAn = false;
  let erkennung = null, wakeErkennung = null;
  let audioCtx = null, analyser = null, mikroStrom = null;

  fetch("/api/sprache/status").then(r => r.json()).then(k => {
    konfig = k;
    stimmeInfo.textContent = k.elevenlabs
      ? "Stimme: ElevenLabs"
      : "Stimme: Browser (ElevenLabs noch nicht eingerichtet)";
  }).catch(() => {});

  function setzeZustand(z, text) {
    zustand = z;
    kugel.dataset.zustand = z;
    const worte = { ruhe: "bereit", lauschen: "hört zu", denken: "denkt nach", sprechen: "spricht" };
    zustandText.textContent = text || worte[z] || z;
    wort.textContent = z === "ruhe" ? "ALEXANDRA" : (worte[z] || "").toUpperCase();
  }

  // ---- Mikrofon-Pegel: die Striche im Ring folgen der echten Lautstaerke
  async function pegelStarten() {
    try {
      if (!audioCtx) audioCtx = new (window.AudioContext || window.webkitAudioContext)();
      if (!mikroStrom) mikroStrom = await navigator.mediaDevices.getUserMedia({ audio: true });
  ...
  function pegelSchleife() {
    ...
    const striche = kugel.querySelectorAll(".pegel");
    const tick = () => {
      if (!analyser || (zustand !== "lauschen" && zustand !== "sprechen")) {
  ...
  if (!SR) {
    hinweis.textContent = "Dieser Browser kann keine Spracherkennung. Nimm Chrome oder Edge.";
    kugel.style.opacity = ".5";
  }
  ...
  function hoeren() {
    ...
    let letzter = "";
    erkennung.onstart = () => { setzeZustand("lauschen"); pegelStarten(); };
    erkennung.onresult = (ev) => {
      letzter = Array.from(ev.results).map(r => r[0].transcript).join("");
      hinweis.textContent = letzter || "…";
    };
    erkennung.onerror = () => setzeZustand("ruhe", "nichts gehört");
    erkennung.onend = () => {
      hinweis.textContent = "Klick auf die Kugel oder sag „Hey Alexandra“";
      const text = letzter.trim();
      if (text) fragen(text); else setzeZustand("ruhe");
    };
    erkennung.start();
  }

  // ---- Wake-Word: eigener Dauerlauscher, der nur auf den Namen wartet
  function wakeUmschalten() {
    wakeAn = !wakeAn;
    wakePunkt.classList.toggle("an", wakeAn);
    if (wakeAn) wakeStarten(); else wakeErkennung?.stop();
  }
  function wakeStarten() {
    if (!SR || !wakeAn) return;
    wakeErkennung = new SR();
    wakeErkennung.lang = "de-DE";
    wakeErkennung.continuous = true;
    wakeErkennung.interimResults = true;
    wakeErkennung.onresult = (ev) => {
    ...
    // Chrome beendet Dauererkennung nach ~60 s von selbst — neu starten.
    wakeErkennung.onend = () => { if (wakeAn && zustand === "ruhe") setTimeout(wakeStarten, 250); };
    wakeErkennung.onerror = () => { if (wakeAn) setTimeout(wakeStarten, 900); };
    try { wakeErkennung.start(); } catch {}
  }
  ...
  async function fragen(text) {
    ...
    try {
      ...
      const d = await r.json();
      if (!d.ok) { zeile("sie", "⚠️ " + (d.hint || "Fehler")); setzeZustand("ruhe"); return; }

      (d.karten || []).forEach(karteZeigen);
      // Zeigt beim Testen, ob die schnelle Spur gegriffen hat oder Hermes ran musste.
      if (d.sprich) {
        const el = zeile("sie", d.sprich);
        const dauer = ((performance.now() - begonnen) / 1000).toFixed(1);
        const marke = document.createElement("span");
        marke.className = "sv-quelle";
        marke.textContent = (d.quelle === "hermes" ? "Hermes" : "Zustand") + " · " + dauer + " s";
        el.appendChild(marke);
        await sprich(d.sprich);
      }

      // Der Hintergrundauftrag lief die ganze Zeit mit — jetzt nachfragen.
      if (d.auftragId) await auftragVerfolgen(d.auftragId);
      setzeZustand("ruhe");
      if (wakeAn) wakeStarten();
    } catch (e) {
  ...
  async function auftragVerfolgen(id) {
    ...
    for (let i = 0; i < 200; i++) {
      ...
      warte.textContent = "…arbeitet noch (" + Math.round(i * 0.9 + 1) + " s)";
    }
    warte.textContent = "Das dauert länger als erwartet — schau später im Chat nach.";
  }
    ...
    if (konfig.elevenlabs) {
      try {
        ...
        if (r.ok) {
          ...
          await abspielen(URL.createObjectURL(blob));
          return;
        }
      } catch { /* faellt unten auf die Browser-Stimme zurueck */ }
    }
    await browserStimme(text);
  }

  function abspielen(url) {
    return new Promise((fertig) => {
      audio = new Audio(url);
      if (audioCtx && !analyser) { /* Pegel optional */ }
      audio.onended = audio.onerror = () => { URL.revokeObjectURL(url); fertig(); };
  ...
  function zeile(wer, text, warte = false) {
    ...
    el.className = "sv " + wer + (warte ? " warte" : "");
    el.textContent = text;
  ...
  function karteZeigen(k) {
    ...
    if (k.art === "kalender") {
      const zeilen = (k.termine || []).map(t =>
        "<tr><td class='zeit'>" + (t.von ? t.von + (t.bis ? "–" + t.bis : "") : "—") +
        "</td><td>" + esc(t.titel) + "</td></tr>").join("");
      el.innerHTML = "<h3>" + esc(k.titel) + " · " + esc(k.label) + "</h3>" +
        (zeilen ? "<table>" + zeilen + "</table>" : "<div class='leer'>Keine Termine eingetragen.</div>");
    } else if (k.art === "zahlen") {
      const w = k.werte || {};
      el.innerHTML = "<h3>Kennzahlen</h3><table>" +
        (w.offen != null ? "<tr><td class='zeit'>Offen</td><td>" + fmt(w.offen) + " €</td></tr>" : "") +
        (w.monat != null ? "<tr><td class='zeit'>Monat</td><td>" + fmt(w.monat) + " €</td></tr>" : "") +
        (w.leads != null ? "<tr><td class='zeit'>Leads</td><td>" + w.leads + "</td></tr>" : "") +
        "</table>";
    } else if (k.art === "briefing") {
      el.innerHTML = "<h3>Briefing" + (k.alterMin != null ? " · vor " + k.alterMin + " Min." : "") +
        "</h3><div class='leer' style='white-space:pre-wrap;color:var(--text-secondary)'>" +
        esc(String(k.text).slice(0, 1200)) + "</div>";
    }
  ...
  // ---- Bedienung
  kugel.addEventListener("click", () => zustand === "ruhe" ? hoeren() : stoppen());
  kugel.addEventListener("keydown", (e) => { if (e.key === " " || e.key === "Enter") { e.preventDefault(); kugel.click(); } });
  btnWake.addEventListener("click", wakeUmschalten);
