...
// DIE 300-SEKUNDEN-WAND — warum hier nicht fetch() steht.
//
// Am 26.07. fiel auf, dass lange Auftraege exakt bei 301 Sekunden mit "fetch
// failed" abbrechen, obwohl AbortSignal.timeout auf 15 Minuten stand. Ursache
// ist nicht der Server: Node baut sein fetch auf undici, und dessen
// headersTimeout steht standardmaessig auf 300 Sekunden. Es zaehlt die Zeit
// BIS ZUM ERSTEN Byte der Antwort — und genau die ist hier lang, weil das
// Modell erst Python schreibt, ausfuehrt, das Ergebnis prueft und dann
// antwortet. Der eigene Timeout kommt gar nicht mehr zum Zug.
//
// Damals dokumentiert und nicht behoben, weil nur Hermes betroffen war und der
// ohnehin ersetzt werden sollte. Am 05.08. lief der Ersatz in dieselbe Wand:
// Erster echter Bau-Auftrag, 301 Sekunden, "fetch failed".
//
// undici laesst sich nicht direkt konfigurieren (nicht als Modul verfuegbar),
// deshalb hier node:https. Kein Header-Timeout, nur eine Gesamtgrenze, die wir
// selbst setzen. Das ist mehr Code als fetch, aber es ist der einzige Weg, der
// nicht nach fuenf Minuten aufgibt.
function langerPost(url, headers, koerper, timeoutMs) {
  return new Promise((fertig, fehler) => {
    const daten = Buffer.from(JSON.stringify(koerper));
    const u = new URL(url);
    const anfrage = https.request({
      hostname: u.hostname,
      path: u.pathname + u.search,
      method: "POST",
      headers: { ...headers, "content-length": daten.length },
    }, (antwort) => {
      const stuecke = [];
      antwort.on("data", (s) => stuecke.push(s));
      antwort.on("end", () => {
        const roh = Buffer.concat(stuecke).toString("utf-8");
        try { fertig({ status: antwort.statusCode, json: JSON.parse(roh), roh }); }
        catch { fertig({ status: antwort.statusCode, json: null, roh }); }
      });
    });
    // EINE Grenze fuer den ganzen Vorgang, nicht fuers erste Byte.
    const wecker = setTimeout(() => { anfrage.destroy(new Error("Zeitgrenze erreicht")); }, timeoutMs);
    anfrage.on("error", (e) => { clearTimeout(wecker); fehler(e); });
    anfrage.on("close", () => clearTimeout(wecker));
    anfrage.end(daten);
  });
}
...
async function bauen(auftrag, { sorte, timeoutMs = 600000, modell } = {}) {
  ...
  try {
    const r = await langerPost("https://api.anthropic.com/v1/messages", KOPF(), {
    ...
    if (r.status !== 200) {
      return { ok: false, hint: `Dokument ${r.status}: ${String(r.roh || "").slice(0, 150)}` };
    }
    const d = r.json || {};
