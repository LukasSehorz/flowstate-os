...
<style>
 ...
 /* DIE BUEHNE. Hier laufen die Inhalte, ohne dass ein einziges Fenster
    aufgehen muss — siehe die Begruendung unten im Skript. */
 #buehne{position:fixed;inset:0;width:100%;height:100%;border:0;display:none;background:#fff}
 #buehne.da{display:block}
</style></head><body>
<iframe id="buehne" title="Drehmappe"></iframe>
<div id="hilfe">
...
(function(){
  ...
  // ---------------------------------------------------------------------
  // INHALTE OHNE POP-UP (22.08.2026)
  //
  // Bis heute schob die Sprachseite jeden Inhalt per window.open als eigenen
  // Tab in dieses Fenster. Das braucht die dauerhafte Pop-up-Erlaubnis: Der
  // Aufruf kommt aus dem Ablauf des Drehbuchs, nicht aus einem Klick, und ohne
  // Geste laesst Chrome ihn nur mit Erlaubnis durch. Am Set hiess das: Der
  // Take laeuft, Telegram und WhatsApp kommen an — und kein Dokument geht auf.
  // Zweimal passiert, beide Male erst im Schnitt gesehen.
  //
  // Jetzt schickt die Sprachseite nur noch eine Nachricht, und dieses Fenster
  // legt den Inhalt selbst in einen Rahmen. Ein Rahmen ist kein Fenster —
  // daran ist kein Blocker beteiligt.
  //
  // WhatsApp Web geht weiterhin als eigener Tab: Es verbietet die Einbettung
  // (X-Frame-Options), und es soll ohnehin schon angemeldet danebenstehen.
  var buehne = document.getElementById("buehne");
  window.addEventListener("message", function(e){
    if (e.origin !== location.origin) return;
    var d = e.data || {};
    if (d.typ !== "zeigen" || !d.url) return;
    hilfe.classList.add("weg");
    buehne.classList.add("da");
    // Beim Neuladen desselben Inhalts muss sich die Adresse unterscheiden,
    // sonst passiert nichts — der Kalender zeigte sonst den Stand von vorhin.
    buehne.src = d.url;
