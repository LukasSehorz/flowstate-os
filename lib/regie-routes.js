  ...
  const TATEN = {
    ...
    gym: async () => {
      ...
      const a = await werkzeuge.terminEintragen({
        name: "Gym", von: `${heute}T19:00:00`, bis: `${heute}T20:00:00`, ort: "Fitnessstudio",
      });
      const b = await werkzeuge.terminEintragen({
        name: "Privat — Abend mit den Eltern", von: `${heute}T20:00:00`, bis: `${heute}T22:30:00`,
        beschreibung: "Geblockt",
      });
