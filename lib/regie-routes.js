...
<style>
 :root{--bg:#0b1220;--rand:#1f2b45;--text:#e5e7eb;--grau:#8b97ad;--blau:#4b8df8}
 *{box-sizing:border-box}
 html,body{height:100%;margin:0;background:var(--bg);color:var(--text);
   font:14px system-ui,-apple-system,Segoe UI,sans-serif}
 body{display:flex;flex-direction:column}
 #reiter{display:flex;gap:6px;padding:8px 10px;background:var(--bg);
   border-bottom:1px solid var(--rand);overflow-x:auto;flex:0 0 auto}
 #reiter button{background:#141d31;color:var(--grau);border:1px solid var(--rand);
   border-radius:8px;padding:7px 14px;font-size:13px;cursor:pointer;white-space:nowrap}
 #reiter button.an{background:var(--blau);border-color:var(--blau);color:#fff;font-weight:600}
 #leer{flex:1;display:flex;align-items:center;justify-content:center;color:var(--grau)}
 iframe{flex:1;width:100%;border:0;background:#fff;display:none}
</style></head><body>
<div id="reiter"></div>
<div id="leer">Drehmappe bereit — hier erscheinen Angebot, Creatives und Kalender.</div>
<iframe id="rahmen" allow="clipboard-read; clipboard-write"></iframe>
<script>
(function(){
  var reiter = document.getElementById("reiter");
  var rahmen = document.getElementById("rahmen");
  var leer = document.getElementById("leer");
  var offen = [];   // { schluessel, titel, url, knopf }

  function huebsch(url){
    try {
      if (/calendar\.google\.com/.test(url)) return "Kalender";
      if (/whatsapp/.test(url)) return "WhatsApp";
      var name = decodeURIComponent(url.split("?")[0].split("/").pop() || "Inhalt");
      name = name.replace(/\.(pdf|html|xlsx|docx|png|jpg)$/i, "").replace(/[-_]+/g, " ");
      return name.length > 34 ? name.slice(0, 32) + "…" : name;
    } catch(e){ return "Inhalt"; }
  }
  function schluesselVon(url){ return String(url).split("?")[0]; }

  function zeigen(eintrag){
    offen.forEach(function(o){ o.knopf.classList.toggle("an", o === eintrag); });
    leer.style.display = "none";
    rahmen.style.display = "block";
    rahmen.src = eintrag.url;
  }

  function aufnehmen(url){
    var k = schluesselVon(url);
    var da = offen.filter(function(o){ return o.schluessel === k; })[0];
    if (da) { da.url = url; return zeigen(da); }   // gleiche Seite: neu laden
    var knopf = document.createElement("button");
    knopf.textContent = huebsch(url);
    var eintrag = { schluessel: k, url: url, knopf: knopf };
    knopf.addEventListener("click", function(){ zeigen(eintrag); });
  ...
  window.addEventListener("message", function(e){
    if (e.origin !== location.origin) return;      // nur aus dem eigenen Haus
    var d = e.data || {};
    if (d.typ === "zeigen" && d.url) aufnehmen(String(d.url));
  });
  // Dem Oeffner sagen, dass die Mappe steht — er schickt dann Wartendes nach.
