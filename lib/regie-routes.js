...
<title>Drehmappe</title><meta name="viewport" content="width=device-width,initial-scale=1">
<style>
 :root{--bg:#0b1220;--karte:#131c2f;--rand:#22304d;--text:#e5e7eb;--grau:#8b97ad;--blau:#4b8df8}
 *{box-sizing:border-box}
 body{margin:0;min-height:100vh;background:var(--bg);color:var(--text);
   font:15px/1.6 system-ui,-apple-system,Segoe UI,sans-serif;
   display:flex;align-items:center;justify-content:center;padding:32px}
 .karte{max-width:620px;width:100%;background:var(--karte);border:1px solid var(--rand);
   border-radius:16px;padding:28px 30px}
 h1{margin:0 0 6px;font-size:20px;color:var(--blau);letter-spacing:.02em}
 p{margin:0 0 14px;color:var(--grau)}
 ol{margin:0 0 18px;padding-left:20px;color:var(--grau)}
 li{margin-bottom:6px}
 button{background:var(--blau);color:#fff;border:0;border-radius:10px;
   padding:12px 22px;font-size:15px;font-weight:600;cursor:pointer}
 button[disabled]{background:#243350;color:var(--grau);cursor:default}
 #stand{margin-top:14px;font-size:13px;color:var(--grau);white-space:pre-line}
 ul.inhalt{margin:14px 0 0;padding-left:18px;font-size:13px;color:var(--grau)}
</style></head><body>
<div class="karte">
  <h1>Drehmappe · ${esc(c.titel || "")}</h1>
  <p>Dieses Fenster gehört auf den zweiten Bildschirm.</p>
  <ol>
    <li>Diesen Tab aus der Leiste ziehen und auf den rechten Bildschirm legen.</li>
    <li>Sobald es drüben liegt, lädt es die Inhalte von selbst.</li>
    <li>Fertig — im Dreh kommt jeder Inhalt sofort nach vorn, ohne Ladezeit.</li>
  </ol>
  <button id="laden">Alles vorladen (${dateien.length})</button>
  <div id="stand"></div>
  <ul class="inhalt">${dateien.map((d) => `<li>${esc(d.length > 64 ? d.slice(0, 62) + "…" : d)}</li>`).join("")}</ul>
</div>
<script>
(function(){
  ...
  function laden(){
    ...
    // Zum Schluss wieder hierher, damit die Mappe vorn liegt.
    try { window.focus(); } catch(e){}
    stand.textContent = offen + " Tabs geladen"
      + (blockiert ? "\n" + blockiert + " blockiert — Pop-ups für diese Seite erlauben." : "")
      + "\nDie Sprachseite holt sie im Dreh nach vorn.";
  ...
  //
  // Der Knopf war ein Handgriff zu viel. Vorher darf aber nicht geladen
  // werden: window.open landet in dem Fenster, aus dem es kommt — solange
  // dieser Tab noch auf dem Laptop haengt, kaemen alle Inhalte dort an.
  //
  // Ein Tab, den man aus der Leiste zieht, wird zu einem eigenen Fenster.
  // Genau das ist an screenX zu sehen: Der Wert springt. Sobald er sich
  // deutlich bewegt hat und kurz ruhig bleibt, wird geladen.
  var startX = window.screenX, ruhe = 0, schonGeladen = false;
  setInterval(function(){
    if (schonGeladen) return;
    if (Math.abs(window.screenX - startX) < 250) { ruhe = 0; return; }
    ruhe++;
    if (ruhe >= 2) {            // ~600 ms still an der neuen Stelle
      schonGeladen = true;
      stand.textContent = "Fenster verschoben — lade die Inhalte …";
      laden();
    }
  }, 300);
