...
- UMSATZ heisst in diesem Haus: gewonnene Deals.
    sum(deals.wert) where deals.status='gewonnen', datiert nach
    deals.geschlossen_am (NICHT nach erstellt).
  Genau so rechnet die Zentrale (crm.kennzahlen), und nur so stimmt die Zahl
  mit dem ueberein, was Lukas auf dem Dashboard sieht.
  ...
  (art: 'einnahme' | 'ausgabe'). Am 20.08. lief "wie viel Umsatz im Juli"
  gegen buchungen und lieferte "null Euro" — in den gewonnenen Deals standen
  5.000. Eine Null, die keine ist, ist die teuerste Antwort.
...
async function datenFragen(frage, { nutzerId, vonMehreren = false } = {}) {
  ...
  while (versuche < 2) {
    ...
    const roh = await schnell.frage(
      ...
      { maxTokens: 700, temp: 0, timeoutMs: 22000,
