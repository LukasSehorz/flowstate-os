...
// Aus einem SQL-Bezeichner wird ein deutsches Wort (20.08.2026).
//
// Der Zweig fuer mehrere Zaehlspalten hat die Spaltennamen woertlich
// vorgelesen. Live gehoert:
//
//   "Umsatz und gewonnene Deals im Juli 2026: 5.000 € umsatz, 5 anzahl deals."
//
// Zweimal "Umsatz", einmal davon klein und als Datenbankfeld, dazu "anzahl
// deals" — das ist die Ausgabe eines Werkzeugs, nicht die Antwort einer Person.
// Sichtbar wurde es erst, seit Geldspalten ueberhaupt in diesem Zweig landen.
//
// putzen() nimmt das fuehrende "anzahl" weg (dieselbe Regel wie bei der
// Beschriftung), der Rest wird gross geschrieben — ein deutsches Substantiv
// sieht anders aus als ein Bezeichner, und genau das ist der Zweck.
const SPALTEN_WORT = {
  umsatz: "Umsatz", gesamtumsatz: "Umsatz", summe: "Summe", wert: "Wert",
  gewonnen: "gewonnene Deals", verloren: "verlorene Deals", offen: "offene Deals",
  leads: "Leads", kunden: "Kunden", firmen: "Firmen", deals: "Deals",
  anrufe: "Anrufe", erstgespraeche: "Erstgespräche", aufgaben: "Aufgaben",
};

function spaltenWort(spalte) {
  const roh = String(spalte || "").replace(/_/g, " ").trim();
  if (SPALTEN_WORT[roh.toLowerCase().replace(/\s+/g, "")]) {
    return SPALTEN_WORT[roh.toLowerCase().replace(/\s+/g, "")];
  }
  const ohneAnzahl = putzen(roh);
  const w = SPALTEN_WORT[ohneAnzahl.toLowerCase().replace(/\s+/g, "")];
  if (w) return w;
  // Gross geschrieben wird das SUBSTANTIV, also das letzte Wort — "neue_firmen"
  // heisst "neue Firmen", nicht "Neue firmen". Ein Eigenschaftswort am Anfang
  // bleibt klein, so wie im Deutschen ueblich.
  const teile = ohneAnzahl.toLowerCase().split(/\s+/).filter(Boolean);
  if (!teile.length) return ohneAnzahl;
  teile[teile.length - 1] = teile[teile.length - 1].charAt(0).toUpperCase() + teile[teile.length - 1].slice(1);
  return teile.join(" ");
}
  ...
  if (zeilen.length === 1 && rest.length > 1 && rest.every((s) => istZahl(zeilen[0][s]))) {
    const teile = rest
      ...
      .map((s) => (GELD_SPALTE.test(s)
        ? `${wertText(zeilen[0][s], s)} ${spaltenWort(s)}`
        : `${zahlDe(zeilen[0][s])} ${spaltenWort(s)}`));
    if (!teile.length) return `Keine Treffer — ${th}.`;
    // Mit "und" statt Komma am Ende: "5.000 Euro Umsatz und 5 Deals" ist ein
    // Satz, "5.000 Euro Umsatz, 5 Deals" eine Aufzaehlung. Vorgelesen ist das
    // der Unterschied zwischen Auskunft und Datenbankausgabe.
