...
function wertText(v) {
  ...
  if (typeof v === "number") return zahlDe(v);
...
function satzBauen(thema, spalten, zeilen) {
  ...
  if (zeilen.length === 1 && rest.length > 1 && rest.every((s) => istZahl(zeilen[0][s]))) {
    const teile = rest
      ...
      .map((s) => `${zahlDe(zeilen[0][s])} ${s.replace(/_/g, " ")}`);
  ...
  if (anzahlSpalte && !einheitlich && rest.length >= 1) {
    ...
    const teile = zeilen
      ...
      .slice(0, 6)
      .map((z) => `${zahlDe(z[anzahlSpalte])} ${wertText(z[label])}`);
    return teile.length ? `${th}: ${teile.join(", ")}.` : `Keine Treffer — ${th}.`;
  }
  ...
  // Zahl plus Beispiele. Bevorzugt eine Namensspalte; gibt es keine, tut es
  // die erste — besser ein grober Hinweis als eine Aufzaehlung von Spaltennamen.
  const nameSpalte = rest.find((s) => /name|titel|firma|betreff/i.test(s)) || rest[0];
  const namen = [...new Set(zeilen.map((z) => z[nameSpalte])
    .filter((v) => v !== null && v !== undefined && v !== "")
    .map(wertText))].slice(0, 5);
  const n = gesamt ?? zeilen.length;
  return `${zahlDe(n)} ${th}` + (namen.length ? `, darunter ${namen.join(", ")}` : "") + ".";
}
...
// richtige.
async function satzSprechen(frage, thema, spalten, zeilen) {
  ...
  try {
    const satz = await schnell.frage(
      "Du bist Alexandra. Formuliere den SATZ unten so um, wie man es einem Kollegen " +
      "sagt — gesprochen, hoechstens zwei Saetze. Du darfst NUR die Formulierung " +
...
// --------------------------------------------------------------------------
async function datenFragen(frage, { nutzerId } = {}) {
  ...
  while (versuche < 2) {
    ...
    const roh = await schnell.frage(
      ANWEISUNG,
      `HEUTE IST ${heute}.\n\nTABELLEN:\n${tabellen}\n\nFRAGE: ${frage}${zusatz}`,
      { maxTokens: 500, temp: 0, timeoutMs: 12000,
        model: process.env.DATEN_SQL_MODEL || "claude-sonnet-5" }
    ).catch((e) => { letzterFehler = String(e.message).slice(0, 120); return ""; });

    const j = jsonAus(roh);
    if (!j) { letzterFehler = "Antwort war kein JSON"; continue; }
    ...
    if (r.ok) {
      return {
        ok: true,
        reply: await satzSprechen(frage, thema, r.spalten, r.zeilen),
