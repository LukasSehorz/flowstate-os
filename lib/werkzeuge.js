...
async function terminAbsagen({ id }) {
  if (!id) return { ok: false, grund: "keine Termin-ID" };
  try {
    const out = await gws(["calendar", "delete", String(id)], 20000);
...
async function wetter(ort, tag = 1) {
  ...
  // Wettercode und Regenwahrscheinlichkeit sind zwei UNABHAENGIGE Werte der
  // API. Frueher wurden sie stumpf aneinandergehaengt — am 26.07. kam dabei
  // heraus: "Schauer, 20 bis 25 Grad, trocken." Beides stimmte fuer sich, die
  // Kombination war Unsinn.
  //
  // Der Code nennt die vorherrschende Wetterart des Tages, die Wahrscheinlich-
  // keit sagt, ob es ueberhaupt nass wird. Unter 20 Prozent regnet es
  // praktisch nicht — dann darf "Schauer" nicht als Tageslage dastehen.
  const nass = code >= 51;              // ab Niesel aufwaerts ist Niederschlag
  let lageSatz;
  if (typeof regen !== "number") lageSatz = lage;
  else if (regen >= 50) lageSatz = `${lage}, Regen wahrscheinlich, ${regen} Prozent`;
  else if (regen >= 20) lageSatz = `${lage}, geringe Regenchance`;
  else if (nass) lageSatz = `ueberwiegend trocken, hoechstens vereinzelt ${lage}`;
  else lageSatz = `${lage} und trocken`;

  return { ok: true, reply: `${wann} in ${k.name}: ${lageSatz}, ${tmin} bis ${tmax} Grad.` };
}
