...
// In welchen Kalender geschrieben wird (18.09.2026).
//
// Mit kalenderId: genau dorthin — so landen Janniks CRM-Termine in seinem
// Kalender und nicht in Lukas'. Ohne: der bisherige Weg (KALENDER_ID oder der
// Hauptkalender des Kontos). Damit bleibt jeder Aufrufer, der die Angabe nicht
// kennt, unveraendert — Sprache, Belege, Drehkulisse.
const zielArgs = (kalenderId) => {
  const id = String(kalenderId || "").trim();
  return id ? ["-c", id] : kalenderArgs();
};
...
// kalenderId (18.09.2026): In WESSEN Kalender der Termin gehoert. Leer =
// Hauptkalender wie bisher. Die Wahl trifft der Aufrufer (lib/crm.js ueber
// kalenderFuer), nicht diese Funktion — hier wird nur angehaengt, was
// mitgegeben wurde.
async function terminEintragen({ titel, start, ende, ganztags = false, ort = "", beschreibung = "", kalenderId = "" }) {
...
//
// Zusaetzlich seit 05.09.2026 (Kalender <-> Kundenakte): `beschreibung` wird
// als -d mitgegeben — dieselbe Option, die `calendar create` seit dem 25.07.
// nimmt. Gebraucht, weil der Akte-Link ("Kundenakte: …") in der Beschreibung
// steht und beim Umhaengen auf eine andere Firma mitwandern muss. Nur gesetzt,
// wenn der Aufrufer sie ausdruecklich uebergibt — ein Verschieben per Sprache
// (sprache-routes.js) schickt weiter keine und faesst die Beschreibung nicht an.
// `ganztags` wird beim Normalisieren beachtet: ein ganztaegiger Termin bekommt
// reine Tage als Start/Ende, keine Uhrzeiten.
async function terminAendern({ id, start, ende, titel, ort, beschreibung, ganztags = false, kalenderId = "" }) {
  ...
  if (start) {
    ...
    const bis = ende ? zeitNormal(ende, { ganztags }) : (ganztags ? von : stundeSpaeter(von));
    if (bis) args.push("--end", bis);
  } else if (ende) {
    const bis = zeitNormal(ende, { ganztags });
    if (bis) args.push("--end", bis);
  }
  if (ort) args.push("-l", String(ort).slice(0, 200));
  if (typeof beschreibung === "string") args.push("-d", beschreibung.slice(0, 500));
...
async function terminAbsagen({ id, kalenderId = "" }) {
  if (!id) return { ok: false, grund: "keine Termin-ID" };
  try {
    const out = await gws(["calendar", "delete", String(id), ...zielArgs(kalenderId)], 20000);
