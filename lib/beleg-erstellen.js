...
const webseite = require("./website-lesen.js");
...
// Wohin die fertige PDF zum Ansehen geht. Wird von aussen gesetzt (server.js),
// damit dieses Modul nichts von Telegram wissen muss — telegram.js laedt
// umgekehrt schon dieses hier, und zwei Module, die sich gegenseitig laden,
// sind eine Fehlerquelle, die man sich sparen kann.
let zusteller = null;
function zustellerSetzen(fn) { zusteller = typeof fn === "function" ? fn : null; }
...
async function erstellen(auftrag = {}) {
  ...
  let firma = String(auftrag.firma || auftrag.empfaenger?.firma || "").trim();
  if (!firma) return { ok: false, reply: `Für welche Firma soll ich die ${wort === "Angebot" ? "das Angebot" : "Rechnung"} schreiben?` };

  // AUS DEM CRM ERGAENZEN (Lukas, 07.08.: "Man soll entweder Adresse checken
  // oder wenn die Webseite hinterlegt ist, dann auf diese gehen").
  //
  // Nach einem Erstgespraech sitzt Lukas im Auto. Anschrift, Ansprechpartner
  // und Mailadresse stehen im CRM, nicht in seinem Kopf — und dort abzulesen
  // heisst anhalten.
  //
  // Ergaenzt wird nur, was er NICHT gesagt hat: Er sitzt beim Kunden, das CRM
  // kann alt sein.
  let quelle = null, dazu = [], mehrdeutig = null;
  try {
    const e = await firmaNach.ergaenzen({ ...auftrag, firma });
    if (e.grund === "mehrdeutig") mehrdeutig = e.namen;
    auftrag = e.auftrag;
    firma = String(auftrag.firma || firma).trim();
    quelle = e.quelle;
    dazu = e.dazu || [];
  } catch (e) { console.error("CRM-Abgleich:", e.message); }

  // Mehrere Firmen passen — dann wird gefragt. Ein Angebot an den falschen
  // Kunden ist schlimmer als eine Rueckfrage.
  if (mehrdeutig) {
    return { ok: false, reply: `Da hab ich mehrere: ${mehrdeutig.join(", ")}. Welche meinst du?` };
  }
  ...
  let gelesen = null;
  if (art === "angebot") {
    // Die Website des Kunden lesen, wenn eine hinterlegt ist. Sie sagt genauer,
    // was der Betrieb macht, als jedes CRM-Feld. Klappt es nicht, laeuft es
    // ohne weiter — ein Angebot ohne Websitetext ist immer noch ein Angebot.
    if (quelle?.website) {
      const w = await webseite.lesen(quelle.website);
      if (w.ok) gelesen = w;
      else console.log(`Website ${quelle.website} nicht gelesen: ${w.grund}`);
    }
    try {
      inhalt = await angebotText.schreiben({
        ...
        crm: firmaNach.alsText(quelle), website: gelesen,
      });
    } catch (e) {
  ...
  // Der geschriebene Angebotstext steht im CHAT, wird aber nicht mehr
  // vorgelesen (Lukas, 19.08.: "das ganze Angebot auf einmal vorgelesen —
  // macht gar keinen Sinn, das schau ich mir persoenlich an").
  //
  // Bis dahin galt die umgekehrte Regel, und sie hatte ihren Grund: Er hat den
  // Text nicht diktiert und sollte hoeren, was rausgeht. Im Gespraech vom
  // 19.08. waren das dann 828 gesprochene Zeichen am Stueck — eine
  // Aufzaehlung mit Spiegelstrichen, vorgetragen wie ein Prospekt. Zum
  // Pruefen taugt das nicht: Wer eine Formulierung nachlesen will, braucht
  // sie vor Augen, nicht im Ohr.
  //
  // Deshalb jetzt getrennt: "reply" behaelt den vollen Text fuer den Chat,
  // "gesprochen" sagt nur, dass er da ist. Sehen statt vorlesen.
  const gezeigt = inhalt
    ? `\n\n${inhalt.posten_titel}\n` + inhalt.vorteile.map((v) => `· ${v}`).join("\n")
    : "";

  // Woher die Angaben kamen, gehoert dazu. Lukas muss wissen, ob die Anschrift
  // von ihm oder aus einem womoeglich alten CRM-Eintrag stammt — sonst prueft
  // er sie nicht.
  const herkunft = [
    dazu.length ? `${dazu.join(", ")} aus dem CRM` : null,
    gelesen ? "Website gelesen" : (quelle?.website ? "Website nicht erreichbar" : null),
  ].filter(Boolean);
  const woher = herkunft.length ? `\n(${herkunft.join(", ")}.)` : "";

  // Die PDF hinterherschicken, sobald sie fertig ist (Lukas, 07.08.). Er soll
  // sehen, was er freigibt — die Aufzaehlung im Chat sagt nichts darueber, ob
  // die Anschrift stimmt oder das Layout sitzt. Das laeuft NEBENHER: Die
  // Rueckfrage darf nicht auf den Wandler warten.
  if (zusteller) {
    pdfLaeuft.then((x) => {
      if (x.pdf) return zusteller(x.pdf, `${basis}.pdf`, `${wort} ${n.nummer} — ${firma}`);
      console.error(`Beleg ${n.nummer}: keine PDF zum Ansehen (${x.hint}).`);
    }).catch((e) => console.error("Beleg zustellen:", e.message));
  }

  return {
    ok: true, nummer: n.nummer, datei: `${basis}.docx`, inhalt,
    reply: `${kopf}${gezeigt}${woher}\n\nSoll ich sie raussenden? Soll ich eine Mail dafür aufsetzen?`,
    // Was die Stimme sagt. Ohne dieses Feld spricht sprache-routes.js die
    // ganze "reply" — mitsamt Aufzaehlung.
    gesprochen: art === "angebot" && inhalt
      ? `${kopf} Den Angebotstext hab ich dir in den Chat gelegt, schau ihn dir an. Soll ich es raussenden?`
      : `${kopf} Soll ich sie raussenden?`,
  };
}
...
function mailTexten(b) {
  ...
  return {
    ...
    text: `${anrede}\n\n${koerper}\n\nViele Grüße\nLukas Sehorz\nsvh Consulting – Sehorz & vom Hofe GbR`,
  };
}
...
async function senden() {
  ...
  try {
    ...
    // ES GEHT NUR EINE PDF RAUS (Lukas, 07.08.: "Das Angebot muss immer als PDF
    // versendet werden, nicht als Textform.").
    //
    // Vorher fiel der Versand auf die Word-Datei zurueck, wenn der Wandler
    // nicht ansprang — mit einem Hinweis hinterher. Das war falsch herum
    // gedacht: Der Hinweis kam, NACHDEM die Datei beim Kunden war. Eine
    // Word-Datei kann der Kunde aendern, sie sieht je nach seiner Word-Version
    // anders aus, und sie ist nicht das, was Lukas seit jeher verschickt.
    //
    // Lieber gar nicht senden. Der Beleg liegt fertig, die Nummer steht, und
    // Lukas kann es erneut versuchen — das ist in jedem Fall besser, als etwas
    // Falsches beim Kunden zu haben.
    if (nurWord) {
      offen = merk;                       // Vorgang bleibt offen fuer den zweiten Versuch
      merk.schritt = "mail-freigeben";
      merk.seit = Date.now();
      return { ok: false, reply: `Ich schicke nichts raus — der PDF-Wandler hat nicht angeschlagen (${hint}), und als Word-Datei geht es nicht zum Kunden. ${merk.wort} ${merk.nummer} liegt fertig. Sag Bescheid, dann versuche ich es nochmal.` };
    }
    ...
    return { ok: true, datei, reply: `Raus an ${merk.mailAn} — als PDF. ${merk.wort} ${merk.nummer} über ${euro(merk.betrag)}${merk.art === "rechnung" ? `, fällig ${dKurz(merk.frist)}` : ""}.` };
  } catch (e) {
...
module.exports = { erstellen, mailAufsetzen, senden, antwortAuf, vergessen, wasOffen, mailTexten, kurzName, anhangVon, zustellerSetzen, AUSGANG };
