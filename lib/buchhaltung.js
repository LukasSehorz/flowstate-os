...
// SAMMELBUCHUNG (18.09.2026). Lukas: "alle Rechnungen … automatisch
// einbuchen". Von selbst gebucht wird weiterhin nichts — aber die Rueckschau
// ueber Monate legt hunderte Belege in den Eingang, und "Passt" hundertmal
// zu druecken ist keine Pruefung mehr, das ist Fleissarbeit. Darum: EIN Klick
// fuer alles, was der Leser SAUBER gelesen hat. Sauber heisst:
//   - Ausgabe (eine Einnahme braucht den Zahltag, den kennt niemand hier),
//   - gelesen, mit Betrag, Datum, Gegenstelle und Kategorie,
//   - OHNE Lesehinweis. Der Hinweis sammelt alles, was das Modell unklar fand
//     ("schlecht lesbar", "sieht nach Kundenrechnung aus") — so ein Beleg
//     bleibt fuer die Einzelpruefung liegen.
// Was hier nicht durchkommt, steht weiter unter "zu pruefen".
const SAUBER_WO = `b.status = 'neu' and b.art = 'ausgabe' and b.gelesen is not null
      and b.betrag > 0 and b.datum is not null
      and nullif(b.gegenstelle, '') is not null and nullif(b.kategorie, '') is not null
      and b.lese_hinweis is null`;

module.exports.belegeSauber = async function (user) {
  return alsNutzer(user.id, async (q) => {
    const { rows } = await q(
      `select b.id, b.laufnummer, b.betrag, b.datum, b.faellig, b.gegenstelle
         from belege b where ${SAUBER_WO} order by b.laufnummer`);
    return rows.map((r) => ({ ...r, betrag: Number(r.betrag) }));
  });
};

// Bezahlt oder offen? Der Bon ist der Zahlungsnachweis — ausser die Rechnung
// traegt ein Zahlungsziel, das noch nicht vorbei ist: Dann ist sie mit
// einiger Sicherheit noch nicht ueberwiesen und landet unter "Noch zu zahlen",
// statt mit einem erfundenen Zahltag in einem Monatsordner.
function nochOffen(faellig, heute = tagText(new Date())) {
  const f = tagText(faellig);
  return Boolean(f && f >= heute);
}

// Bucht alle sauberen Belege — jeden einzeln ueber belegBuchen, damit jede
// Regel von dort (Zahltag, Kundenakte, Fixkosten) unveraendert gilt. ids
// (optional) grenzt auf eine Auswahl ein; ohne ids: alle sauberen.
module.exports.belegeSammelBuchen = async function (user, ids = null) {
  const liste = await module.exports.belegeSauber(user);
  const auswahl = Array.isArray(ids) && ids.length
    ? liste.filter((b) => ids.map(String).includes(String(b.id)))
    : liste;
  let gebucht = 0, offen = 0;
  const nicht = [];
  for (const b of auswahl) {
    const istOffen = nochOffen(b.faellig);
    let r;
    try { r = await module.exports.belegBuchen(user, b.id, null, { offen: istOffen }); }
    catch (e) { r = { ok: false, grund: String(e.message).slice(0, 80) }; }
    if (r.ok) { gebucht += 1; if (istOffen) offen += 1; }
    else nicht.push({ id: b.id, laufnummer: b.laufnummer, grund: r.grund || "unbekannt" });
  }
  return { ok: true, gebucht, offen, nicht, gesamt: auswahl.length };
};
...
module.exports.rechnen = { nochOffen, quelleAus, naechsteFaelligkeit, fixkostenJeMonat, offeneSummen, monatsStatus, tagText };
