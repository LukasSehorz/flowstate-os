    ...
    const { rows } = await q(
      `select extract(year from bu.bezahlt_am)::int as jahr,
              ...
              count(*) filter (where not exists
                (select 1 from belege b where b.buchung_id = bu.id and b.status = 'gebucht' and b.daten is not null))::int as ohne_beleg
...
module.exports.monatsExport = async function (user, jahr, monat) {
  ...
  // Scheitert der Excel-Bau, geht das ZIP trotzdem raus — ohne das Blatt,
  // mit Meldung im Protokoll.
  let excel = null;
...
module.exports.monatsExcelBauen = function (d, offene = []) {
  ...
  const zeilen = d.zeilen.map((z) => [
    ...
    z.notiz || "",
    z.belegnummer || "",
    z.hat_datei ? archivName(z) : "keine Datei",
  ]);
  const uebersicht = {
    name: "Übersicht",
    kopf: [
...
module.exports.monatsExcel = async function (user, jahr, monat) {
  ...
  const datei = module.exports.monatsExcelBauen(d, offene);
  ...
  return {
    ...
    anzahl: d.zeilen.length, offene: offene.length,
  };
};
