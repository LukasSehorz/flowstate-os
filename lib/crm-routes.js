...
module.exports = function (app) {
  ...
  app.get("/crm", async (req, res, next) => {
    try {
      ...
      const gew = await crm.gewonneneDeals(u, sparte);
      const verlauf = [];
      for (let i = 5; i >= 0; i--) {
      ...
      const pkt = verlauf.map((m, i) => [(i / (verlauf.length - 1 || 1)) * 780 + 10, 165 - (m.wert / maxV) * 140]);
      const linie = "M" + pkt.map((p) => p[0].toFixed(1) + "," + p[1].toFixed(1)).join(" L");
      const flaeche = linie + ` L${pkt[pkt.length - 1][0].toFixed(1)},175 L10,175 Z`;
      ...
      res.send(rahmen(u, "start", "Dashboard", "", reiter, `
        ...
        <div class="karte" style="margin-bottom:16px">
          <div class="karte-kopf"><div><h2>Umsatzentwicklung</h2>
            <div class="sub">Gewonnene Deals je Monat · letzte 6 Monate</div></div>
            <div style="text-align:right"><div class="kachel-zahl" style="font-size:26px">${geld(verlauf.reduce((a,m)=>a+m.wert,0))}</div>
              <div class="caption">Summe im Zeitraum</div></div></div>
          <div style="position:relative;height:180px">
            <svg viewBox="0 0 800 180" preserveAspectRatio="none" style="width:100%;height:100%">
              <defs><linearGradient id="fl" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stop-color="var(--blau-500)" stop-opacity=".28"/>
                <stop offset="100%" stop-color="var(--blau-500)" stop-opacity="0"/></linearGradient></defs>
              ${[0,1,2,3].map(i=>`<line x1="0" y1="${i*45+15}" x2="800" y2="${i*45+15}" stroke="var(--border)" stroke-width="1" stroke-dasharray="3 5"/>`).join("")}
              <path d="${flaeche}" fill="url(#fl)"/>
              <path d="${linie}" fill="none" stroke="var(--blau-600)" stroke-width="2.5" stroke-linejoin="round"/>
              ${verlauf.map((m,i)=>`<circle cx="${(i/(verlauf.length-1||1))*780+10}" cy="${165-(m.wert/maxV)*140}" r="4" fill="#fff" stroke="var(--blau-600)" stroke-width="2.5"/>`).join("")}
            </svg>
          </div>
          <div style="display:flex;justify-content:space-between;margin-top:8px">
            ${verlauf.map(m=>`<span class="caption">${e(m.label)}</span>`).join("")}</div>
        </div>
