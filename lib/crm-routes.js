...
function zeilenZuLeads(zeilen) {
  ...
  return zeilen.map((z) => baue(z, fest)).filter((l) => l.name);
}
  ...
  app.post("/crm/leads/neu", async (req, res, next) => {
    try {
      ...
      if (b.link) {
        ...
        const roh = String(b.liste).replace(/\r/g, "");
        const zeilen = roh.includes("\t")
          ? roh.split("\n").map((z) => z.split("\t").map((x) => x.trim()))
              .filter((z) => z.some(Boolean))
          : csvZerlegen(roh);
        leads = zeilenZuLeads(zeilen);
      } else {
