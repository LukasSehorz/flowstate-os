...
module.exports = function (app) {
  ...
  app.get("/crm/leads", async (req, res, next) => {
    try {
      ...
      const listen = await crm.leadListen(u, { besitzer: nurVon || "" });
      ...
      res.send(rahmen(u, "leads", "Leads", `Cold-Calling-Liste · ${e(spartenName)}`, reiter, `
        ...
        <form class="filter" method="get">
          ...
          ${/* Hochgeladene Liste. Steht nur da, wenn es ueberhaupt welche gibt —
               ein Auswahlfeld mit einem einzigen Eintrag ist kein Filter,
               sondern Beiwerk in einer Leiste, die schon voll ist. */""}
          ${listen.length > 1 ? `<select name="hochgeladen" class="knapp" onchange="this.form.submit()">
