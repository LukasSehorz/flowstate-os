  ...
  app.get("/crm/pipeline", async (req, res, next) => {
    try {
      ...
      res.send(seite(u, "pipeline", "Pipeline", `
        ...
        ${!gesamt ? `<div class="karte leer" style="margin-top:16px"><div class="leer-icon">${ICON.pipeline}</div><h3>Keine offenen Deals in ${e(SPARTEN[sparte])}</h3>
  ...
  app.get("/crm/firma/:id", async (req, res, next) => {
    try {
      ...
      res.send(seite(u, f.status === "kunde" ? "kunden" : "leads", f.name, `
        <div class="kopf">
          <div><h1>${e(f.name)}</h1>
            <p><span class="badge ${f.status === "kunde" ? "b-gruen" : f.status === "verloren" ? "b-rot" : "b-blau"}">${e(f.status)}</span>
