  ...
  app.get("/crm/leads", async (req, res, next) => {
    try {
      ...
      const zeile = (f) => {
        ...
        ${/* Ohne Webseite steht hier ein Strich und KEIN Link (06.08., Wunsch
             Lukas). Vorher stand dort "keine ✨" als gruene Marke — gemeint war
             "kein Auftritt, bestes Verkaufsargument". Wer aber "keine" in der
             Spalte stehen hatte, bekam einen Link auf eine Fehlerseite. Ein
             Strich kann man nicht anklicken, und genau darum ging es. */""}
        <td>${webseite(f.website)
