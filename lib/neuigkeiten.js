...
async function crm(seit) {
  ...
  // ZAEHLEN UND AUFZAEHLEN SIND ZWEI FRAGEN (Fix 07.08.2026).
  //
  // Vorher stand hier "limit 6" und die Zeilenzahl wurde als ANZAHL vorgelesen.
  // Damit konnte die Antwort nie ueber 6 hinauskommen: Am 05.08. sagte
  // Alexandra "6 neue Firmen" — tatsaechlich waren an dem Tag 273 angelegt
  // worden, davon 107 fuer Ioannis und 102 fuer Jannik. Die Zahl klang
  // plausibel und war um den Faktor 45 zu klein.
  //
  // "count(*) over ()" liefert die echte Gesamtzahl in derselben Abfrage, in
  // der die Beispiele geholt werden — eine Runde zur Datenbank, keine zwei
  // Zahlen, die auseinanderlaufen koennen.
  const neueFirmen = await zaehle(
    "select count(*) over ()::int as gesamt, name from firmen " +
    "where erstellt >= $1 order by erstellt desc limit 2", [seit]);
  if (neueFirmen?.length) {
    const gesamt = neueFirmen[0].gesamt;
    const namen = aufzaehlen(neueFirmen.map((f) => sprechName(f.name)));
    teile.push(gesamt > 2
      ? `${gesamt} neue Firmen, darunter ${namen}`
      : `${einsMehr(gesamt, "neue Firma", "neue Firmen")}: ${namen}`);
  }

  // Die Summe muss ueber ALLE neuen Deals gehen, nicht ueber die angezeigten
  // zwei — sonst nennt Alexandra einen Betrag, der zu niedrig ist.
  const neueDeals = await zaehle(
    "select count(*) over ()::int as gesamt, sum(d.wert) over () as summe, " +
    "d.titel, f.name as firma from deals d left join firmen f on f.id = d.firma_id " +
    "where d.erstellt >= $1 order by d.erstellt desc limit 2", [seit]);
  if (neueDeals?.length) {
    const gesamt = neueDeals[0].gesamt;
    const summe = Number(neueDeals[0].summe) || 0;
    const wo = aufzaehlen(neueDeals.map((d) => sprechName(d.firma || d.titel)));
    teile.push(`${einsMehr(gesamt, "neuer Deal", "neue Deals")} bei ${wo}` +
      (gesamt > 2 ? " und weiteren" : "") +
      (summe ? `, zusammen ${Math.round(summe).toLocaleString("de-DE")} Euro` : ""));
  }

  const gewonnen = await zaehle(
    "select count(*) over ()::int as gesamt, f.name as firma " +
    "from deals d left join firmen f on f.id = d.firma_id " +
    "where d.status = 'gewonnen' and d.geschlossen_am >= $1 order by d.geschlossen_am desc limit 4", [seit]);
  if (gewonnen?.length) {
    const gesamt = gewonnen[0].gesamt;
    const namen = gewonnen.map((d) => d.firma).filter(Boolean).join(", ");
    teile.push(`${einsMehr(gesamt, "Deal gewonnen", "Deals gewonnen")}: ${namen}` +
      (gesamt > gewonnen.length ? " und weitere" : ""));
  }
  ...
  const WORT = {
    ...
    // Mit echten Umlauten (07.08.): Als "weitergerueckt" buchstabierte die
    // Stimme das Wort hoerbar aus. Was gesprochen wird, wird geschrieben wie
    // gesprochen — die ae/oe/ue-Schreibweise gilt fuer Code, nicht fuer Text,
    // der aus dem Lautsprecher kommt.
    stufenwechsel: ["Deal weitergerückt", "Deals weitergerückt"],
    uebergabe: ["Übergabe", "Übergaben"],
...
async function projekte(seit) {
  const teile = [];
  // Gleicher Fix wie im CRM: die echte Anzahl, nicht die des Ausschnitts.
  const p = await zaehle(
    "select count(*) over ()::int as gesamt, name from projekte where erstellt >= $1 " +
    "order by erstellt desc limit 3", [seit]);
  if (p?.length) {
    teile.push(`${einsMehr(p[0].gesamt, "neues Projekt", "neue Projekte")}: ` +
      p.map((x) => x.name).join(", ") + (p[0].gesamt > p.length ? " und weitere" : ""));
  }
