...
async function crm(seit) {
  ...
  if (neueFirmen?.length) {
    const namen = neueFirmen.slice(0, 3).map((f) => f.name).join(", ");
    teile.push(neueFirmen.length > 3
  ...
  if (neueDeals?.length) {
    ...
    const wo = neueDeals.slice(0, 2).map((d) => d.firma || d.titel).join(" und ");
  ...
  const akt = await zaehle(
    "select art, count(*)::int as n from aktivitaeten where zeit >= $1 group by art order by n desc limit 4", [seit]);
  if (akt?.length) {
    teile.push(akt.map((a) => `${a.n} mal ${a.art}`).join(", "));
  }

  const erledigt = await zaehle(
    "select count(*)::int as n from aufgaben where erledigt >= $1", [seit]);
  if (erledigt?.[0]?.n) teile.push(`${erledigt[0].n} Aufgaben abgehakt`);
