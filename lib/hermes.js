...
//   instructions  eigene Anweisung statt SOUL.md (Stufe 3: Denkspur des OS)
//   model         Modell nur fuer diesen Lauf, z. B. gpt-5.6-terra (ueber das Abo)
async function auftrag(text, { quelle = "web", nutzerId = null, sessionId = null, melde = null,
  fristMs = 30 * 60 * 1000, spaetMelden = 0, instructions = null, model = null } = {}) {
  ...
  try {
    const r = await fetch(`${basis()}/v1/runs`, {
      ...
      body: JSON.stringify({ input: String(text), ...(sessionId ? { session_id: String(sessionId) } : {}),
        ...
        ...(model ? { model: String(model), provider: process.env.HERMES_PROVIDER || "openai-codex" } : {}) }),
...
// ---------------------------------------------------------------- Kurzaufruf (Schnellspur)

// EIN kurzer Modellaufruf ueber Hermes, ohne Auftragsbuch (Stufe 3.1, 19.09.2026).
//
// Die Schnellspur des OS (lib/schnell.js frage()) formuliert Saetze, entnimmt
// Felder, klassifiziert — hunderte Male am Tag, mit 120 bis 400 Token. Dafuer
// ist kein Auftrag mit Nummer noetig, wohl aber derselbe Weg: die Runs-API
// mit eigener Anweisung (statt SOUL.md), dem kleinen Modell ueber das Abo und
// ohne Sitzung. Gemessen am 19.09.: Luna 2,0 bis 2,8 s, Terra 2,4 s.
//
// Kommt trotz Anweisung der JSON-Block aus SOUL.md zurueck, wird sein
// "ergebnis" (sonst "zusammenfassung") als Text genommen — nie das JSON selbst.
function klartext(output) {
  const s = String(output || "").trim();
  const erg = ergebnisLesen(s);
  if (erg && (typeof erg.ergebnis === "string" || typeof erg.zusammenfassung === "string")) {
    const e = String(erg.ergebnis || "").trim();
    return e || String(erg.zusammenfassung || "").trim() || s;
  }
  return s;
}

async function kurz(system, nutzer, { model = null, timeoutMs = 20000 } = {}) {
  if (!verfuegbar()) throw new Error("HERMES_URL fehlt.");
  const modell = model || process.env.SCHNELL_MODEL_HERMES || "gpt-5.6-luna";
  const anweisung = [String(system || "").trim(),
    "Antworte in Klartext, ohne JSON, ohne Werkzeuge, ohne Rueckfragen."].filter(Boolean).join("\n\n");
  const start = Date.now();
  const r = await fetch(`${basis()}/v1/runs`, {
    method: "POST", headers: kopf(),
    body: JSON.stringify({ input: String(nutzer), instructions: anweisung, model: modell,
      provider: process.env.HERMES_PROVIDER || "openai-codex" }),
    signal: AbortSignal.timeout(Math.min(timeoutMs, 20000)),
  });
  if (!r.ok) throw new Error(`Hermes ${r.status}: ${(await r.text().catch(() => "")).slice(0, 160)}`);
  const runId = (await r.json()).run_id;
  if (!runId) throw new Error("Hermes gab keine Lauf-Kennung zurueck.");
  const output = await ereignisseLesen(runId, { fristMs: timeoutMs }).catch(async (e) => {
    if (/kein Ende/.test(e.message)) await stoppen(runId);
    throw e;
  });
  const text = klartext(output);
  sprachlog.schreiben({ art: "hermes-kurz", modell, dauerMs: Date.now() - start, laenge: text.length });
  if (!text) throw new Error("Hermes: leere Antwort");
  return text;
}
...
module.exports = { auftrag, feuern, kurz, liste, laufende, stoppen, gesund, verfuegbar, basis,
  sseZeile, ergebnisLesen, antwortText, klartext };
