...
//      Zeile "<Firma>: <Titel>" mit Link in die Akte in den Kunden-Block auf
//      der Tafel des VERANTWORTLICHEN geschrieben — gibt es dort noch keinen
//      Block, den der Anleger beschreiben darf, entsteht einer am festen
//      Kunden-Platz. Die Zeile traegt "aufgabe": aufgaben.id; das ist der
//      einzige Faden zwischen beiden Welten.
...
// Der feste Kunden-Platz auf der Tafel (2400 x 1400) — dieselben Zahlen wie
// platzVon(0) im Client (public/lib/whiteboard.js: ORD_X[0], ORD_Y[0],
...
// In welchen Block auf der Tafel? Den JUENGSTEN lebenden Kunden-Textblock,
// den der Anleger aendern darf (Autor oder Tafel-Besitzer — dieselbe Regel
// wie im Speicher und in der RLS) und der noch Platz hat. Haftnotizen nie:
// sie sind Gegenstaende, keine Listen (siehe zusammenfuehrZiel im Client).
function zielBlock(elemente, user) {
  let bester = null;
  for (const el of elemente) {
    if (el.art !== "text" || !el.inhalt || el.inhalt.kategorie !== "kunden") continue;
...
// Wohin mit einem neuen Block? Auf den Kunden-Platz — und wenn dort schon
// etwas liegt (gleich welcher Kategorie), darunter: unter den untersten
// Block, der die Spalte des neuen Blocks waagrecht ueberdeckt, mit LUFT
// Abstand. Was seitlich daneben steht, schiebt nicht. Striche zaehlen nicht
// (wie blockAufPlatz im Client). Und nie unten aus der Tafel hinaus: liegt
// die Spalte voll, wird geklemmt — ueberlappen ist dann das kleinere Uebel
// als ein Block, den niemand mehr sieht.
function platzFinden(elemente, hoehe, breite = BLOCK_BREITE) {
...
async function aufgabeAufTafel(user, aufgabe, { speicher } = {}) {
  try {
    ...
    return await s.gesperrt(user, tafel, async (t) => {
      // ---- Anhaengen an den bestehenden Kunden-Block — bei einem
      ...
      for (let versuch = 0; versuch < VERSUCHE; versuch++) {
        ...
        const ziel = zielBlock(elemente, user);
        if (!ziel) return neuAnlegen(t, tafel, zeile, elemente, pruefeElement);
        ...
        if (r.grund !== "veraltet") return neuAnlegen(t, tafel, zeile, await t.vonTafel(tafel), pruefeElement);
      }
      ...
      return neuAnlegen(t, tafel, zeile, await t.vonTafel(tafel), pruefeElement);
    });
  } catch (fehler) {
...
// Neuer Block am Kunden-Platz (bzw. darunter). besitzer setzt der Speicher auf
// den Anleger; auf einer fremden Tafel zeigt der Client dann die Fahne "von X".
async function neuAnlegen(t, tafel, zeile, elemente, pruefeElement) {
  const hoehe = hoeheSchaetzen([zeile], BLOCK_BREITE, SCHRIFT);
  const lage = platzFinden(elemente, hoehe);
  const p = pruefeElement({
    art: "text", x: lage.x, y: lage.y, breite: BLOCK_BREITE, hoehe,
    inhalt: { liste: "check", farbe: "schwarz", groesse: SCHRIFT, kategorie: "kunden", zeilen: [zeile] },
  });
  if (!p.ok) return { ok: false, grund: "pruefung:" + p.grund };
