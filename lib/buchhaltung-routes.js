...
const FEHLERTEXTE = {
  ...
  "rueckschau-laeuft": `<b>Die Rückschau läuft schon.</b> Warte, bis sie fertig ist — der Stand steht oben auf der Seite.`,
  "rueckschau-seit": `<b>Ohne Startdatum keine Rückschau.</b> Trag den Tag ein, ab dem das Postfach durchsucht werden soll.`,
  "sauber-leer": `<b>Nichts zu sammelbuchen.</b> Es liegt kein Ausgabebeleg im Eingang, den der Leser ohne Hinweis
     gelesen hat — die übrigen bitte einzeln öffnen und prüfen.`,
};
...
const HINWEISTEXTE = {
  ...
  "rueckschau-gestartet": (q) => `<b>Rückschau gestartet — seit ${e(String(q.seit || "").split("-").reverse().join("."))}.</b>
     Das dauert je nach Menge einige Minuten. Der Stand steht oben; lad die Seite zwischendurch neu.`,
  "sauber-gebucht": (q) => {
    const n = Number(q.n) || 0, o = Number(q.offene) || 0, x = Number(q.nicht) || 0;
    return `<b>${n} ${n === 1 ? "Beleg" : "Belege"} gebucht${o ? `, davon ${o} offen (Zahlungsziel noch nicht vorbei)` : ""}.</b>${
      x ? ` ${x} ${x === 1 ? "ließ" : "ließen"} sich nicht buchen und ${x === 1 ? "liegt" : "liegen"} weiter im Eingang.` : ""}`;
  },
};
...
const MELDUNGS_PARAMETER = ["fehler", "hinweis", "gebucht", "offen", "text", "neu", "uebersprungen", "tage", "an", "n", "doppelt", "seit", "nicht", "offene"];
...
// Die Postfach-Zeile der Belege-Seite (18.09.2026): welche Postfaecher
// haengen dran, die Rueckschau ab einem Tag, und der Stand einer laufenden
// oder gerade beendeten Rueckschau. Vorgabe fuer "seit": der 26.07.2026 —
// Beginn dieser Buchfuehrung (Verfahrensdokumentation, Abschnitt 8). Frueher
// ist Lexware massgeblich; wer trotzdem weiter zurueck will, aendert das Datum.
const RUECKSCHAU_VORGABE = "2026-07-26";
function postfachZeile({ gmailBereit, listeUrl }) {
  let mailBelege = null, weitere = [];
  try {
    mailBelege = require("./mail-belege.js");
    weitere = require("./gmail-direkt.js").postfaecher().map((k) => k.name);
  } catch { mailBelege = null; }
  const konten = [gmailBereit ? "Hauptkonto" : null, ...weitere].filter(Boolean);
  const st = mailBelege ? mailBelege.rueckschauStatus() : null;
  const seitText = (t) => String(t || "").split("-").reverse().join(".");
  let stand = "";
  if (st && (st.laeuft || (st.fertig && Date.now() - st.fertig.getTime() < 6 * 3600 * 1000))) {
    const s = st.stand || {};
    const konten2 = (s.konten || []).map((k) => `${e(k.name)}: ${k.fehler ? "nicht erreichbar" : `${k.neu} neu von ${k.mails}`}`).join(" · ");
    stand = st.laeuft
      ? `<div class="os-hinweis os-hinweis--info">${ICON.uhr}<div><b>Rückschau läuft</b> (seit ${e(seitText(st.seit))}): ${s.geprueft || 0} Mails geprüft, ${s.neu || 0} neue Belege${s.uebersprungen ? `, ${s.uebersprungen} nicht verarbeitet` : ""}.${konten2 ? " " + konten2 + "." : ""} Seite neu laden für den aktuellen Stand.</div></div>`
      : st.ergebnis?.ok
        ? `<div class="os-hinweis os-hinweis--erfolg">${ICON.check}<div><b>Rückschau fertig</b> (seit ${e(seitText(st.seit))}, ${zeitpunkt(st.fertig)}): ${st.ergebnis.neu} neue Belege aus ${st.ergebnis.geprueft} Mails${st.ergebnis.uebersprungen ? `, ${st.ergebnis.uebersprungen} nicht verarbeitet` : ""}.${konten2 ? " " + konten2 + "." : ""}</div></div>`
        : `<div class="os-hinweis os-hinweis--fehler">${ICON.warnung}<div><b>Rückschau abgebrochen.</b> ${e(st.ergebnis?.hint || "")}</div></div>`;
  }
  if (!gmailBereit && !weitere.length) return stand;
  return `${stand}<div class="buch-postfach">
    <span>${ICON.mail} Postfächer: ${konten.map((k) => pille(e(k), "grau")).join(" ")}</span>
    <form method="post" action="/buchhaltung/mail-rueckschau" class="buch-postfach-form"
      onsubmit="return confirm('Alle Rechnungen seit dem gewählten Tag aus allen Postfächern holen? Das läuft im Hintergrund und kann einige Minuten dauern.')">
      <input type="hidden" name="zurueck" value="${e(listeUrl)}">
      <label>Alle Rechnungen seit <input type="date" name="seit" value="${RUECKSCHAU_VORGABE}" max="${heuteFeld()}" required></label>
      <button type="submit" class="sekundaer klein" ${st?.laeuft ? "disabled" : ""}>${ICON.wiederholen} holen</button>
    </form>
  </div>`;
}
...
module.exports = function (app) {
  ...
  app.get("/buchhaltung/belege", nurAdmin, async (req, res, next) => {
    try {
      ...
      const [liste, zaehler, firmen, beleg, sauber] = await Promise.all([
        buch.belegeListe(u, filter),
        buch.belegZaehler(u),
        crm.firmenListe(u, { limit: 300 }),
        /^\d+$/.test(String(q.beleg || "")) ? buch.belegEinzeln(u, q.beleg) : null,
        buch.belegeSauber(u).catch(() => []),
      ]);
      const kunden = [...(firmen || [])].sort((a, b) => a.name.localeCompare(b.name, "de"));
      ...
      const postfachBlock = postfachZeile({ gmailBereit, listeUrl });
      ...
      res.send(seite(req, {
        ...
        <div class="os-werkzeugleiste">
          ...
          <div class="os-werkzeugleiste-aktionen">
            ...
            ${sauber.length ? `<form method="post" action="/buchhaltung/belege/sauber-buchen" style="display:contents" onsubmit="return confirm('${sauber.length} sauber gelesene Ausgabebelege jetzt buchen? Belege mit Lesehinweis bleiben zur Einzelprüfung liegen.')">
              <input type="hidden" name="zurueck" value="${e(listeUrl)}">
              <button type="submit" class="sekundaer" title="Bucht alle Ausgabebelege, die der Leser mit Betrag, Datum, Lieferant und Kategorie und ohne Hinweis gelesen hat">${ICON.check} ${sauber.length} sauber gelesene buchen</button>
            </form>` : ""}
  ...
  const MAIL_TAGE = 7;
  // Das "dash" fuer einen Postfachlauf unter den Rechten des angemeldeten
  // Nutzers — geteilt von "Postfach jetzt pruefen" und der Rueckschau.
  const dashFuer = (u) => async (pfad, body, kopf = {}) => {
    let ergebnis;
  ...
  const postfachDa = () => { try { return require("./gmail-direkt.js").bereit(); } catch { return false; } };

  app.post("/buchhaltung/mail-import", nurAdmin, async (req, res, next) => {
    const ziel = zurueckZiel(req, "/buchhaltung/belege");
    try {
      if (!postfachDa()) return res.redirect(mitParam(ziel, "fehler", "mail-kein-zugang"));
      const mailBelege = require("./mail-belege.js");
      const r = await mailBelege.laufen({ dash: dashFuer(req.nutzer), tage: MAIL_TAGE });
  ...
  // Die Rueckschau (18.09.2026): alle Rechnungen seit einem Tag, aus allen
  // angeschlossenen Postfaechern, im Hintergrund. Die Antwort kommt sofort;
  // den Stand zeigt die Belege-Seite (postfachZeile).
  app.post("/buchhaltung/mail-rueckschau", nurAdmin, async (req, res, next) => {
    const ziel = zurueckZiel(req, "/buchhaltung/belege");
    try {
      if (!postfachDa()) return res.redirect(mitParam(ziel, "fehler", "mail-kein-zugang"));
      const seit = String((req.body && req.body.seit) || "").trim();
      const mailBelege = require("./mail-belege.js");
      let melden = null;
      try { const tg = require("./telegram.js"); if (tg.hatOwner?.()) melden = (t) => tg.push(t); } catch { melden = null; }
      const r = mailBelege.rueckschauStarten({ dash: dashFuer(req.nutzer), seit, melden });
      if (!r.ok) return res.redirect(mitParam(ziel, "fehler", "rueckschau-" + r.grund));
      res.redirect(mitParam(mitParam(ziel, "hinweis", "rueckschau-gestartet"), "seit", seit));
    } catch (err) { next(err); }
  });

  // Sammelbuchung (18.09.2026): alles, was der Leser sauber gelesen hat, in
  // einem Klick — Regeln in buchhaltung.js (belegeSammelBuchen).
  app.post("/buchhaltung/belege/sauber-buchen", nurAdmin, async (req, res, next) => {
    const ziel = zurueckZiel(req, "/buchhaltung/belege");
    try {
      const r = await buch.belegeSammelBuchen(req.nutzer);
      if (!r.gesamt) return res.redirect(mitParam(ziel, "fehler", "sauber-leer"));
      res.redirect(`${mitParam(ziel, "hinweis", "sauber-gebucht")}&n=${r.gebucht}&offene=${r.offen}&nicht=${r.nicht.length}`);
    } catch (err) { next(err); }
  });
