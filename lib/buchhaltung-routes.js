...
// lib/kalender-routes.js:221 und ist hier bewusst dasselbe.
const FEHLERTEXTE = {
...
const fehlerText = (code) => {
  if (!code) return "";
  return FEHLERTEXTE[code]
    || `<b>Das hat nicht geklappt.</b> Gespeichert wurde nichts. Meldung: <code>${e(code)}</code>`;
};
...
module.exports = function (app) {
  ...
  // springt der Belegzaehler von 33 auf 34, die Ausgaben im Juli-Ordner um
  // 37,30 Euro hoch. Ohne das muesste jemand F5 druecken, und genau das sieht
  // man einem Video an.
  //
  // Die Wache holt sich alle zwei Sekunden nur eine Zahl. Aendert sie sich,
  // laedt sie die Seite EINMAL neu und tauscht die beiden Stellen aus, an denen
  // die Zahl steht — den Belegzaehler und die Monatsordner. Warum die ganze
  // Seite holen statt die Zahl selbst zu setzen: So steht dort exakt das, was
  // der Server auch beim naechsten Laden schreiben wuerde. Zwei Stellen, die
  // Geldbetraege formatieren, waeren zwei Stellen, die auseinanderlaufen.
  //
  // Ohne DREH_REGIE=1 gibt dieser Aufruf einen leeren Text zurueck, und im
  // Betrieb steht nichts davon in der Seite.
  const drehWache = () => process.env.DREH_REGIE !== "1" ? "" : `
    <style>
      @keyframes dreh-blitz {
        0%   { background: rgba(255, 214, 10, .55); }
    ...
    (function () {
      ...
      function hervorheben(el) {
        if (!el) return;
        el.classList.remove("dreh-frisch");
        void el.offsetWidth;              // Neustart der Animation erzwingen
        el.classList.add("dreh-frisch");
      }
      async function nachsehen() {
        ...
        try {
          ...
          var html = await (await fetch("/buchhaltung", { cache: "no-store" })).text();
          var neu = new DOMParser().parseFromString(html, "text/html");
          [".beleg-mehr-titel", ".bh-ordner"].forEach(function (wahl) {
            var alt = document.querySelector(wahl), frisch = neu.querySelector(wahl);
...
Ausgaben ${geld(v.ausgaben)}">
      ...
      const kanzleiZeile = `<div style="display:flex;align-items:center;justify-content:space-between;
        ...
        <p class="caption" style="margin:0;flex:1 1 240px">
          ${kanzleiDa
            ? `${AGENT} schickt den Ordner an <b>${e(kanzleiName || einst.steuer_mail)}</b>
               <span class="caption">· ${e(einst.steuer_mail)}</span>`
            : `<b>Noch keine Steuerkanzlei hinterlegt.</b> Solange hier nichts steht,
               fragt ${AGENT} jedes Mal im Gespräch nach der Adresse, statt sie zu raten.`}
        </p>
      ...
      // Ein Schreibversuch, der schiefging (siehe FEHLERTEXTE oben). Steht ganz
      // oben, weil er die Antwort auf das ist, was Lukas gerade getan hat —
      // unter den Dauerhinweisen wuerde er untergehen.
      const fehlerCode = String(req.query.fehler || "").slice(0, 60);
      const fehlerHinweis = fehlerCode ? `
        <div class="hinweis warn" style="margin-bottom:16px">${ICON.warnung}<div>
          ${fehlerText(fehlerCode)}</div></div>` : "";
      ...
      res.send(schale({
        ...
        <!-- Steuerkanzlei (20.08.2026). Einmal eintragen statt jedes Mal
             diktieren: ${AGENT} liest die Adresse hier heraus, wenn sie den
        ...
        <dialog id="dlg-kanzlei"><form method="post" action="/buchhaltung/kanzlei">
          ...
          <p class="caption" style="margin:-4px 0 12px">Anrede und Name stehen getrennt, weil
            ${AGENT} den Satz ausspricht: „Soll ich sie an <b>Frau Meier</b> schicken?“
