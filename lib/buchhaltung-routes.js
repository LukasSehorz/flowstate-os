...
module.exports = function (app) {
  ...
  app.get("/buchhaltung", nurAdmin, async (req, res, next) => {
    try {
      ...
      const [d, firmen, belegeAlle, kasse, monate, exporte, erinnerung] = await Promise.all([
        ...
        ${d.offen.map((o) => {
          ...
          return `<div class="liste-zeile">
            ...
            <form method="post" action="/buchhaltung/bezahlt" class="lz-tat lz-bezahlt">
              ...
              <input type="date" name="bezahlt_am" value="${heuteFeld()}" max="${heuteFeld()}"
                title="Wann ist das Geld eingegangen? Danach richtet sich der Monatsordner — bei einer Zahlung im Mai gehört sie in den Mai."
                aria-label="Zahltag">
                ...
                <div class="beleg-felder">
                  ...
                  <label>USt ${uSaetze.length === 1
                    ...
                    : `<select name="steuersatz">
                        ${uSaetze.map((s) => `<option value="${s}" ${Number(b.steuersatz) === s || (b.steuersatz === null && s === uSaetze[0]) ? "selected" : ""}>${s} %</option>`).join("")}
                       </select>`}</label>
