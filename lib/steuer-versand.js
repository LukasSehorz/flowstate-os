...
//
// WIEDERHOLEN IST KEINE ESKALATION (Fehler gefunden am 20.08.2026).
//
// Das Muster war `\b(schick|…)\b|\braus\b` — und damit hat der HAEUFIGSTE Satz
// ueberhaupt eskaliert: der wiederholte Auftrag. Gemessen:
//
//   LUKAS> Schick den Juli-Ordner an die Steuerberaterin   -> Entwurf liegt
//   LUKAS> Schick den Juli-Ordner an die Steuerberaterin   -> Entwurf GELOESCHT
//   LUKAS> ja                                              -> RAUS an die Kanzlei
//
// Ebenso "Ja, ist die schon raus?" — eine FRAGE, die nur wissen will, wo die
// Mail steht, loeste denselben Weg aus. Damit war der ganze Umbau ("Ein
// Entwurf verlaesst das Haus nicht") ausgehebelt: Zwei harmlose Saetze und ein
// Ja, und die komplette Buchhaltung war beim Empfaenger.
//
// Drei Regeln seitdem:
//   1. Ein Satz mit Fragezeichen ist nie ein Befehl. "Ist die schon raus?"
//      fragt nach dem Stand, es befiehlt nichts.
//   2. `raus` allein reicht nicht mehr. Es braucht entweder ein eindeutiges
//      Hinausschicken-Wort (rausschicken, abschicken, …) oder "schick … raus".
//   3. Nennt der Satz wieder, WAS geschickt werden soll (Ordner, Rechnungen,
//      Steuerberaterin, Monat), ist er die Wiederholung des Auftrags — und nur
//      dann eine Eskalation, wenn ausdruecklich Nachdruck dabeisteht
//      ("schick den Ordner jetzt WIRKLICH raus").
const FRAGESATZ = /\?/;
const NACHDRUCK = /\b(wirklich|tats[äa]chlich|direkt|sofort|jetzt|gleich|trotzdem|doch|ruhig|selber|selbst)\b|\bvon hier\b/;
const HINAUS = /\b(rausschicken|rausschick\w*|raushauen|abschicken|abschick\w*|absenden|rausgehen|verschick\w*|versend\w*)\b|\braus damit\b/;
// Woran man den wiederholten Auftrag erkennt: Er nennt die SACHE, nicht das
// Hinausgehen.
const AUFTRAG_WIEDERHOLT = /\b(ordner|rechnungen|belege|buchhaltung|unterlagen|steuerberaterin|steuerberater|kanzlei|monatsordner)\b/;

// Der Satz, der den Stand nennt. An zwei Stellen gebraucht — beim wiederholten
// Auftrag und bei der Frage nach dem Stand — und deshalb hier einmal.
function entwurfLiegtSatz(b) {
  const wer = anredeKurz(b.name);
...
function sendewunsch(t) {
  if (FRAGESATZ.test(t)) return false;
  if (AUFTRAG_WIEDERHOLT.test(t) && !NACHDRUCK.test(t)) return false;
  if (HINAUS.test(t)) return true;
  // "schick sie raus", "schick sie wirklich direkt weg"
  return /\bschick\w*\b/.test(t) && (/\braus\b/.test(t) || NACHDRUCK.test(t));
}
...
async function antwortAuf(text) {
  ...
  // Eine FRAGE nach dem Stand ("ist die schon raus?", "ist die schon
  // rausgegangen?") ist weder Ja noch Nein und lief bisher am Modell vorbei
  // ins Blaue. Solange ein Entwurf liegt, ist die Antwort hier bekannt — und
  // eine bekannte Antwort gehoert nicht geraten (20.08.2026).
  if (b.schritt === "entwurf-liegt" && FRAGESATZ.test(roh)
      && /\b(raus|rausgegangen|rausgeschickt|abgeschickt|verschickt|gesendet|unterwegs)\b/.test(t)) {
    return entwurfLiegtSatz(b);
  }
  ...
  if (b.schritt === "entwurf-liegt") {
    if (!sendewunsch(t)) return entwurfLiegtSatz(b);
...
module.exports = {
  ...
  MODUS_STANDARD,
  // Fuer den Test: die Frage "ist das ein ausdruecklicher Sendewunsch?" muss
  // einzeln pruefbar sein, ohne dass ein Vorgang offen sein muss.
  sendewunsch,
};
