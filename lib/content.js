...
//
// Genau diese vier, nicht mehr: YouTube, TikTok, Instagram, X. Eine kurze Liste
// ist hier die Aussage — wer ueberall gleichzeitig sein will, ist nirgends.
const KANAELE = ["YouTube", "TikTok", "Instagram", "X"];

// Formate je Kanal. Ein Reel gibt es auf YouTube nicht, einen Thread nur auf X —
// die Auswahl im Dialog richtet sich darum nach dem gewaehlten Kanal.
const FORMATE_JE_KANAL = {
  YouTube:   ["Kurzvideo", "Langvideo", "Livestream"],
  TikTok:    ["Kurzvideo", "Foto-Karussell", "Livestream"],
  Instagram: ["Reel", "Karussell", "Story", "Einzelbild"],
  X:         ["Beitrag", "Thread", "Video"],
};
// Alle Formate zusammen — fuer die Pruefung beim Speichern.
const FORMATE = [...new Set(Object.values(FORMATE_JE_KANAL).flat())];
...
// Filter auf eine Marke. Ueberall dieselbe Bedingung, damit Kacheln, Plan und
// Ausstoss nie verschiedene Teilmengen zeigen. $1 ist immer die Marke (oder null
// fuer "alle") — so bleibt die Parameternummer in allen Abfragen gleich.
const MARKE_FILTER = (alias = "p") => `($1::uuid is null or ${alias}.marke = $1)`;
...
module.exports.redaktionsplan = async function (user, marke = null) {
  return alsNutzer(user.id, async (q) => {
    const { rows } = await q(
      `select p.*, pr.name as wer, mk.name as marke_name,
        ...
        where p.status <> 'veroeffentlicht' and ${MARKE_FILTER()}
        order by p.geplant_am nulls last, p.erstellt
        limit 60`, [markeOderNull(marke)]);
    return rows;
  });
};

module.exports.letzteVeroeffentlicht = async function (user, anzahl = 12, marke = null) {
  return alsNutzer(user.id, async (q) => {
    const { rows } = await q(
      `select p.*, pr.name as wer, mk.name as marke_name from content_posts p
         ...
         left join profiles mk on mk.id = p.marke
        where p.status = 'veroeffentlicht' and ${MARKE_FILTER()}
        order by p.veroeffentlicht_am desc nulls last, p.id desc limit $2`,
      [markeOderNull(marke), anzahl]);
    return rows;
  });
};
...
module.exports.massenZaehler = async function (user, marke = null) {
  ...
  return alsNutzer(user.id, async (q) => {
      ...
      `with w as (
       ...
       select w.start, count(p.id)::int as anzahl
         ...
         left join content_posts p
          ...
          and ${MARKE_FILTER()}
        group by w.start order by w.start`, [m]);
    ...
    const { rows: [summe] } = await q(
      `select
       ...
       from content_posts p where status = 'veroeffentlicht' and ${MARKE_FILTER()}`, [m]);
    ...
    const { rows: [offen] } = await q(
      `select
       ...
       from content_posts p where ${MARKE_FILTER()}`, [m]);
  ...
  return alsNutzer(user.id, async (q) => {
    const { rows } = await q(
      `select pr.id, pr.name,
              count(*) filter (where p.status = 'veroeffentlicht'
                and p.veroeffentlicht_am >= date_trunc('week', current_date))::int as woche,
              count(*) filter (where p.status <> 'veroeffentlicht')::int as in_arbeit,
              count(*) filter (where p.status <> 'veroeffentlicht'
...
module.exports.FORMATE_JE_KANAL = FORMATE_JE_KANAL;
