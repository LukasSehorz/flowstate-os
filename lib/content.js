  ...
  return alsNutzer(user.id, async (q) => {
    const { rows } = await q(
      ...
      [titel.slice(0, 200),
       ...
       String(d.ueberschrift || titel).trim().slice(0, 80) || null]);
...
// Die Zahlen fuer die Karte auf der Zentrale — ueber alle Marken und Kanaele.
//
// Zwei Zaehlweisen, und der Unterschied ist Absicht:
//   Ausstoss   je Stueck UND Kanal (cross join unnest). Muss so sein, weil die
//              Wochenziele je Kanal gesetzt sind — ein Kurzvideo auf TikTok und
//              YouTube erfuellt zwei Ziele, nicht eins. Genauso zaehlt summen().
//   Produktion je Stueck. Ein Video, das noch geschnitten werden muss, ist EIN
//              Stueck Arbeit, egal auf wie vielen Kanaelen es spaeter landet.
module.exports.kennzahlen = async function (user) {
  return alsNutzer(user.id, async (q) => {
    const { rows: [raus] } = await q(
      `select
        count(*) filter (where p.veroeffentlicht_am >= date_trunc('week', current_date))::int as woche,
        count(*) filter (where p.veroeffentlicht_am >= date_trunc('month', current_date))::int as monat
       from content_posts p cross join lateral unnest(p.kanaele) as k
       where p.status = 'veroeffentlicht'`);
    const { rows: [offen] } = await q(
      `select
        count(*) filter (where status in ('idee','entwurf','aufnehmen','schneiden','hochladen'))::int as produktion,
        count(*) filter (where status = 'geplant')::int as geplant,
        count(*) filter (where veroeff_status = 'faellig' and status <> 'veroeffentlicht')::int as faellig
       from content_posts`);
    // Kein einziges Ziel gesetzt heisst "noch nicht festgelegt", nicht "Ziel 0".
    // Darum null statt einer Zahl — die Karte zeigt dann keinen Fortschritt an,
    // statt "0 von 0 geschafft" zu behaupten.
    const { rows: [z] } = await q(
      `select coalesce(sum(ziel), 0)::int as ziel, count(*)::int as zeilen from content_ziele`);
    return {
      woche: raus.woche, monat: raus.monat,
      ziel_woche: z.zeilen ? z.ziel : null,
      produktion: offen.produktion, geplant: offen.geplant, faellig: offen.faellig,
    };
  });
};
...
module.exports.summen = async function (user, marke = null, kanal = null) {
  ...
  return alsNutzer(user.id, async (q) => {
    ...
    const { rows } = await q(
      `with letzte as (
       ...
       select ${bau("p.veroeffentlicht_am >= date_trunc('week', current_date)", "woche")},
              ...
              -- "true" statt eines Datumsfilters: alles, was wir je gemessen haben.
              -- Das ist NICHT der Kanalstand — aeltere Videos und alles vor
              -- unserer Erfassung fehlen darin. Steht auf der Seite auch so dabei.
              ${bau("true", "gesamt")},
    ...
    return {
      ...
      gesamt: Object.fromEntries(KENNZAHLEN.filter((k) => k.art === "ganz")
        .map((k) => [k.id, zahl(r[`${k.id}_gesamt`])])),
      stuecke_woche: r.stuecke_woche || 0,
      stuecke_monat: r.stuecke_monat || 0,
      stuecke_gesamt: r.stuecke_gesamt || 0,
    };
  });
};
...
module.exports.tafel = async function (user, marke = null, kanal = null) {
  ...
  return alsNutzer(user.id, async (q) => {
    const { rows } = await q(
      `select p.id, p.titel, p.ueberschrift, p.headline, p.status, p.sorte, p.kanaele, p.rubrik, p.funnel,
...
// ------------------------------------------------- Der Kanalstand
//
// Abonnenten und Gesamtaufrufe des KANALS — nicht die Summe unserer Videos.
// Von Hand eingetragen, solange kein Plattformzugang besteht.
//
// Geliefert wird immer der neueste Stand UND der davor: der Wert allein sagt
// wenig ("4.210 Abos"), erst die Veraenderung ist die Aussage ("+310 seit
// letzter Woche").
module.exports.kanalstand = async function (user, marke = null, kanal = null) {
  return alsNutzer(user.id, async (q) => {
    const { rows } = await q(
      `select distinct on (kanal) kanal, gemessen_am, abonnenten, views_gesamt,
              likes_gesamt, videos_gesamt, notiz,
              -- Der Eintrag davor, um daraus das Wachstum zu rechnen. lag()
              -- ueber das Fenster je Kanal, damit auch bei Luecken der
              -- tatsaechlich vorherige Eintrag genommen wird und nicht
              -- "vor genau sieben Tagen", den es vielleicht nicht gibt.
              vor_abonnenten, vor_views, vor_likes, vor_am
         from (
           select k.*,
                  lag(abonnenten)   over w as vor_abonnenten,
                  lag(views_gesamt) over w as vor_views,
                  lag(likes_gesamt) over w as vor_likes,
                  lag(gemessen_am)  over w as vor_am
             from content_kanalstand k
            where ($1::uuid is null or k.marke = $1)
              and ($2::text is null or k.kanal = $2)
           window w as (partition by k.kanal order by k.gemessen_am)
         ) s
        order by kanal, gemessen_am desc`,
      [markeOderNull(marke), kanalOderNull(kanal)]);
    return rows;
  });
};

module.exports.standEintragen = async function (user, d) {
  if (!KANAELE.includes(d.kanal)) return { ok: false, grund: "kanal" };
  const tag = /^\d{4}-\d{2}-\d{2}$/.test(d.gemessen_am || "") ? d.gemessen_am : null;
  return alsNutzer(user.id, async (q) => {
    await q(
      // Zweimal am selben Tag heisst korrigiert, nicht zweimal gewachsen —
      // darum ueberschreiben statt eine zweite Zeile anzulegen.
      `insert into content_kanalstand (marke, kanal, gemessen_am, abonnenten,
                                       views_gesamt, likes_gesamt, videos_gesamt,
                                       notiz, erfasst_von)
       values ($1,$2,coalesce($3::date, current_date),$4,$5,$6,$7,$8,$9)
       on conflict (marke, kanal, gemessen_am) do update set
         abonnenten = excluded.abonnenten, views_gesamt = excluded.views_gesamt,
         likes_gesamt = excluded.likes_gesamt, videos_gesamt = excluded.videos_gesamt,
         notiz = excluded.notiz, erfasst_von = excluded.erfasst_von`,
      [markeOderNull(d.marke) || user.id, d.kanal, tag,
       zuGanz(d.abonnenten), zuGanz(d.views_gesamt), zuGanz(d.likes_gesamt),
       zuGanz(d.videos_gesamt), String(d.notiz || "").trim().slice(0, 300) || null,
       user.id]);
    return { ok: true };
  });
};
