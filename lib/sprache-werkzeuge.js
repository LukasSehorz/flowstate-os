...
const WERKZEUGE = [
  ...
  w("whatsapp_senden",
    "WhatsApp vorbereiten. Der Server liest sie vor und holt die Freigabe — abgeschickt wird erst danach. Kein Serienversand an viele.",
    {
      ...
      text: txt("die FERTIGE Nachricht, wortwoertlich abzuschicken — nicht der Auftrag an dich. 'schreib Jannik, wann er wieder da ist' -> 'Hey, wann bist du wieder im Buero?'. Auch bei duennem Auftrag ('schreib Jannik wegen morgen') NICHT nachfragen, sondern aus STAND und Verlauf ausformulieren und vorlegen — Lukas hoert sie und kann sie in einem Satz aendern. Die einzige Rueckfrage ist die Freigabe."),
  ...
  // 20.08.2026: Hier stand "die Frage, nur dieser Teil" — und genau das hat das
  // Verstehen-Modell als Einladung gelesen, die Frage in SUCHBEGRIFFE
  // umzuschreiben. Gemessen: "Wie steht der SV Oberbergkirchen in der Tabelle?"
  // ging als "Aktueller Tabellenstand SV Oberbergkirchen Fussball Saison
  // 2026/27" hinaus — damit fand die Websuche nichts, obwohl derselbe Aufruf
  // direkt auf lib/suche-web.js 5 von 5 mal richtig lag. Das Nachbarwerkzeug
  // daten_fragen sagt "die Frage woertlich", und dort passiert es nicht.
  w("recherchieren", "Kurze Websuche: Fakten, Preise, News, Zahlen. Dauert Sekunden.",
    { auftrag: txt("die Frage WÖRTLICH, so wie Lukas sie gestellt hat — nicht in Suchbegriffe umschreiben. Die Suche baut sich ihre Begriffe selbst.") },
    ["auftrag"]),
  ...
  w("gespraech_auswerten",
    "Erstgespraech/Gespraechsnotizen zu EINER Firma auswerten und daraus ein Angebot machen. " +
    "'Werte das Erstgespraech mit X aus und erstell ein Angebot'. Leistung, Argumente und Preis " +
    "holt der Server selbst aus dem CRM — frag NIE nach der Firma, wenn ihr Name im Satz steht.",
    {
      firma: txt("Firma, wie Lukas sie nennt — auch ungenau ('Zahnarztpraxis Bergmann')"),
      betrag: { type: "number", description: "nur wenn er den Preis JETZT nennt. Sonst weglassen — der Server sucht ihn." },
      sparte: { type: "string", enum: ["Website", "SocialMedia", "Performance Marketing"] },
      nur_auswerten: { type: "boolean", description: "true, wenn er nur hoeren will, was im Gespraech stand" },
    }, ["firma"]),

  w("steuer_versand",
    "Buchhaltungsordner an die Steuerberaterin schicken (Monatsordner als ZIP per Mail). " +
    "Der Server stellt zusammen und legt die Mail als ENTWURF in Lukas' Postfach; abgeschickt " +
    "wird sie dort von ihm. Nur wenn er ausdruecklich sagt, dass sie direkt rausgehen soll, " +
    "holt der Server vorher die Freigabe. " +
    "Fehlt der Zeitraum, NICHT nachfragen: der Server nimmt den letzten abgeschlossenen Monat und nennt ihn.",
    {
      zeitraum: txt("wie Lukas ihn gesagt hat: 'vom Juli', 'letzten Monat', 'das Quartal'. Weglassen, wenn er nichts gesagt hat."),
      an: txt("Mailadresse, nur falls er sie ausdruecklich nennt"),
    }),
  ...
  // --- Am Rechner arbeiten (20.08.2026). Ein echter Browser, keine Websuche:
  //     Formular ausfuellen, Ergebnis lesen. Bis zum Warenkorb ja, kaufen nie
  //     (REGELN.md, Zone SCHWARZ) — das steht in lib/computer.js und in jeder
  //     Antwort, die von dort kommt.
  w("computer_auftrag",
    "Flug suchen und vergleichen — echter Browser, echte Preise, auf Wunsch mit dem Kalender abgestimmt. Auch wenn Lukas 'buch mir' sagt: gesucht und vorgeschlagen wird, gebucht nie. Dauert bis zu einer Minute.",
    { auftrag: txt("der Reisewunsch woertlich, mit Ziel und Zeitraum") }, ["auftrag"]),

  // --- Nachschub fuer die Anrufliste (20.08.2026). Frueher lief das ueber
  //     lange_arbeit und meldete sich per Telegram — im Gespraech passierte
  //     sichtbar nichts. Jetzt steht die Liste, bevor der Satz zu Ende ist.
  w("leads_nachschub",
    "Unbearbeitete Leads zu einer neuen Anrufliste buendeln. 'such nochmal hundert weitere raus', 'wir brauchen mehr Leads'. Holt aus dem Bestand, sucht nichts im Internet.",
    { auftrag: txt("woertlich, mit Anzahl, Ort/Branche und fuer wen, soweit genannt") }, ["auftrag"]),
