...
function pruefeElement(roh, { inhaltNoetig = true } = {}) {
  ...
  if (roh.art === "strich") {
    ...
    const zeilen = i.zeilen.map((z, nr) => {
      const zeile = {
      ...
      // Unterpunkt (17.09.2026): ebene 1 = Stichpunkt UNTER der Aufgabe
      // darueber — eingerueckt, mit kleinem Kreis statt Kaestchen, zaehlt nicht
      // als eigene Aufgabe. Es gibt genau eine Stufe; alles andere faellt weg.
      // Die erste Zeile kann keiner sein (ueber ihr steht nichts), und wie der
      // Aufgaben-Bezug MUSS das Feld durchkommen, sonst rueckte jeder
      // Tastendruck im Block alle Stichpunkte wieder aus.
      if (nr > 0 && ganz(z && z.ebene, 1, 1) === 1) zeile.ebene = 1;
