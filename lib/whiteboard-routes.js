...
// "whiteboard_elemente" (Migration 0056). Nichts wandert automatisch von der
// Tafel in die Aufgabenliste oder zurueck.
//
...
// (31.08.2026): Kunden gehen immer vor, danach Vertrieb, dann das Interne,
// zuletzt Content und Wissen. Wer eine Aufgabe einordnet, muss sie nicht mehr
// von Hand an die richtige Stelle schieben; ohne Angabe bleibt ein Block
// liegen, wo er liegt. Die Namen stehen hier, damit der Server nichts
// durchlaesst, was die Oberflaeche nicht einsortieren kann.
const KATEGORIEN = ["kunden", "vertrieb", "intern", "content"];
const GRENZEN = {
...
// und mailto — javascript:-Adressen und alles andere fliegen raus, BEVOR sie
// in der Datenbank landen: Ein Klick auf einen Link darf nie Code ausfuehren.
const LINK = /^(https?:\/\/|mailto:)\S{1,500}$/i;
...
module.exports = function (app, optionen = {}) {
  ...
  app.post("/api/whiteboard/aendern", angemeldetApi, async (req, res) => {
    try {
      ...
      if (!p.ok) return res.status(400).json({ ok: false, grund: p.grund });
      const r = await speicher.aendern(req.nutzer, {
  ...
  app.post("/api/whiteboard/wiederherstellen", angemeldetApi, async (req, res) => {
    try {
      ...
      const ids = idListe(req.body.ids);
      if (!ids) return res.status(400).json({ ok: false, grund: "ids" });
      res.json(await speicher.wiederherstellen(req.nutzer, ids));
    } catch (fehler) {
