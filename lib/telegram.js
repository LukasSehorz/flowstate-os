...
async function verfolge(chat, a) {
  const runden = a.art === "lang" ? 300 : 45;
  for (let i = 0; i < runden; i++) {
    await sleep(a.art === "lang" ? 2000 : 1400);
    let d;
    try { d = await (await dash("/api/sprache/auftrag/" + a.id)).json(); } catch { continue; }
    if (d.unbekannt) return;
    if (d.fertig) {
      if (d.reply) { await sendeText(chat, d.reply); sendeStimme(chat, d.reply); insGehirn(d.reply, "Alexandra"); }
      return;
    }
  }
}
...
async function behandle(msg) {
  ...
  let antwort;
  try { antwort = await (await dash("/api/sprache/frage", { text })).json(); }
  catch { await sendeText(chat, "Verbindung hakt gerade — sag's gleich nochmal."); return; }

  if (antwort.sprich) {
    await sendeText(chat, antwort.sprich);
    await sendeStimme(chat, antwort.sprich);
    insGehirn(antwort.sprich, "Alexandra");
  }
  for (const a of (antwort.auftraege || [])) verfolge(chat, a).catch(() => {});
}
