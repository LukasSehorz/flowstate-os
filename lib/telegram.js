...
// Sprachausgabe (sendeStimme, mp3ZuOgg) am 19.09.2026 entfernt: Telegram antwortet nur noch in Text.
...
async function verfolgeLang(chat, a) {
  for (let i = 0; i < 300; i++) {
    ...
    if (d.fertig) {
      if (d.reply) { await sendeText(chat, d.reply); insGehirn(d.reply, "Alexandra"); }
      return;
    }
  }
}
...
async function behandle(msg) {
  ...
  // Auftragsbuch auf Zuruf (Stufe 2, 19.09.2026): "/auftraege" zeigt die letzten fuenf.
  if (/^\/auftr(ae|ä)ge\b/i.test(text.trim())) {
    try {
      const d = await (await dash("/api/agent/auftraege?n=5")).json();
      const zeilen = (d.auftraege || []).map((a) =>
        `#${a.id} ${a.status}${a.dauer_ms ? " (" + Math.round(a.dauer_ms / 1000) + " s)" : ""}: ${a.text}` +
        (a.antwort ? "\n   → " + String(a.antwort).slice(0, 160) : ""));
      await sendeText(chat, zeilen.length ? zeilen.join("\n") : "Noch keine Aufträge.");
    } catch { await sendeText(chat, "Auftragsbuch gerade nicht erreichbar."); }
    return;
  }
...
function starten() {
  ...
  // NUR SENDEN, NICHT ZUHOEREN (21.08.2026).
  //
  // Seit die Drehkulisse in einem eigenen Container laeuft, gibt es diesen
  // Dienst ZWEIMAL — und beide haetten denselben Bot-Ausweis. Telegram gibt
  // ein eingehendes getUpdates aber nur EINEM Abholer: Wer zuerst zieht, hat
  // die Nachricht, der andere sieht sie nie. Im Ergebnis wuerde Lukas' Bot im
  // Alltag zufaellig Nachrichten verlieren, und ein im Dreh fotografierter
  // Beleg landete womoeglich in der ECHTEN Buchhaltung.
  //
  // Mit TELEGRAM_LAUSCHEN=aus laeuft der Container nur als Absender. Das
  // reicht fuer Creative 2, wo der Vertrag hinausgeht und nichts
  // hereinkommt. Wer im Dreh auch Belege einlesen will, braucht einen
  // eigenen Bot (zweiter Ausweis) — nicht diesen Schalter.
  if (/^(aus|nein|0|false)$/i.test(String(process.env.TELEGRAM_LAUSCHEN || ""))) {
    console.log("Telegram: nur Senden — dieser Container holt keine Nachrichten ab.");
    return;
  }
  schleife();
}
