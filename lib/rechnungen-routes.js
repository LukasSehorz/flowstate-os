...
const ICON = {
  ...
  // 05.09.2026 (Bereich F): der Weg ueber das Gespraech — Sprechblase.
  gespraech: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M21 11.5a8.4 8.4 0 0 1-9 8.4 8.9 8.9 0 0 1-4-.9L3 21l1.9-4.9A8.4 8.4 0 0 1 12 3a8.4 8.4 0 0 1 9 8.5z"/></svg>',
    ...
    try {
      ...
      const tabelle = liste.length ? `<div class="os-tabelle-huelle"><table class="os-tabelle">
        ...
        : `<div class="os-leer"><h3>${filter.art || filter.status || filter.firma || filter.suche ? "Nichts gefunden" : "Noch keine Rechnungen oder Angebote"}</h3>
            ...
            <p class="rg-sub">Anlegen geht oben rechts — <b>Im Chat erstellen</b>; das <b>Formular</b> steht daneben, wenn du jede Zeile selbst tippen willst.</p></div>`;
      res.send(seite(u, "Rechnungen & Angebote", "Ausgangsrechnungen, Angebote und Zahlstand", `
        <div class="os-kopf"><div><div class="os-kopf-titel">Rechnungen &amp; Angebote</div>
          ...
          <div class="os-kopf-aktionen">
            <a class="rg-knopf sekundaer" href="/buchhaltung/rechnungen/neu">${ICON.stift} Formular</a>
            <a class="rg-knopf dunkel" href="/buchhaltung/rechnungen/chat">${ICON.gespraech} Im Chat erstellen</a></div></div>
  ...
  app.get("/buchhaltung/rechnungen/:id(\\d+)", nurAdmin, async (req, res, next) => {
    try {
      ...
      if (entwurf) {
        ...
        aktionen.push(`<a class="rg-knopf sekundaer" href="/buchhaltung/rechnungen/chat?beleg=${r.id}">${ICON.gespraech} Im Chat ändern</a>`);
