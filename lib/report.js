...
// Dashboard-Daten (Kalender morgen + CRM) einsammeln — als Kontext fuer Hermes
// UND als Grundlage fuer den Sonnet-Fallback.
...
// Kurz & sprechfreundlich, denn er wird laut vorgelesen.
async function hermesReport(daten) {
  ...
  const auftrag =
    "Erstelle einen KURZEN, sprechfreundlichen Abend-Report fuer Lukas — 3 bis 5 Saetze, " +
    "er wird laut vorgelesen. Blick auf morgen: die wichtigsten Termine (werte und priorisiere, " +
    "nicht alles aufzaehlen), dann kurz die wichtigsten offenen To-Dos und auffaellige neue " +
    "Geschaeftsmails von heute. Nutze fuer Kalender und Gmail deine Tools (Anleitung: " +
    "/opt/data/vault/referenzen/google-workspace.md, GWS_ENCRYPTION=none). Sprechregeln: " +
    "gesprochene Uhrzeiten wie 'elf Uhr', KEINE roh vorgelesenen Zahlen, IDs, Adressen oder URLs. " +
    "Beginne mit 'Hey, kurzer Blick auf morgen:'. Sende KEINE Mails, aendere nichts. Gib NUR den " +
    "fertigen Report-Text zurueck, sonst nichts.\n\n" +
...
// Sonnet-Fallback aus den Dashboard-Daten — schnell und zuverlaessig.
async function sonnetReport(daten, art) {
  ...
  const system = stimme +
    ...
    "Hoerregeln (Uhrzeiten wie 'elf Uhr', keine Rohzahlen). Beginne mit 'Hey, kurzer Blick auf morgen:'. " +
    "Nichts erfinden — nur was in den Daten steht.";
  return (await schnell.frage(system, daten, { maxTokens: 320, timeoutMs: 25000 })).trim();
}

// Rohdaten -> Report-Text. Hermes zuerst (reich), Sonnet als Ausfallnetz.
async function baueReport({ art = "abend" } = {}) {
  const { daten, termine } = await datenSammeln(art);
  try {
    const text = await hermesReport(daten);
  ...
  try {
    return { ok: true, text: await sonnetReport(daten, art), termine: termine.length, quelle: "sonnet" };
  } catch {
