...
async function sonnetReport(daten, art) {
  ...
  // Ueber denke() statt frage(): Die Funktion heisst "sonnetReport", rief aber
  // das Standardmodell (Haiku) — sichtbar geworden am 05.08., als sie bei
  // gewachsener Datenmenge eine LEERE Antwort lieferte ("max_tokens"). Mit
  // Aufwand "low" bleibt Sonnet schnell, und die Tokengrenze hat jetzt Luft:
  // Sonnet denkt von sich aus mit, und Denken zaehlt gegen dieselbe Grenze.
  const t = await schnell.denke(system, daten, {
    webSuche: false, aufwand: "low",
    maxTokens: istMorgen ? 1200 : 900, timeoutMs: 30000,
  });
  return String(t || "").trim();
}
