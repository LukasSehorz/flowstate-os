...
eintragen({
  ...
  unter: [{ id: "zentrale-todos", modul: "todos", titel: "To-Dos", icon: "todo", href: "/todos", nach: "kalender" }],
});
...
module.exports = function (app) {
  ...
  app.get("/todos", angemeldet, async (req, res, next) => {
    try {
      const u = req.nutzer;
      const [alle, firmen, team] = await Promise.all([
        crm.todoListe(u),
      ...
      // "fuer Louis" / "von Lukas" (05.09.2026): Eine Aufgabe hat seit 0058
      // zwei Personen — wer sie angelegt hat (besitzer) und wer sie erledigt
      // (verantwortlich). Beide sehen sie in ihrer Liste; das Abzeichen sagt,
      // welche Rolle die andere Person hat. Steht nichts da, ist sie ganz
      // meine. Vorname reicht — wie die Fahne "von Lukas" auf dem Whiteboard.
      ...
      // Der Kunde einer Aufgabe: als Link in die Akte — oder, wenn die Akte
      // fuer mich nicht freigegeben ist (RLS auf firmen: Lukas hat mir eine
      // Aufgabe zu SEINEM Kunden gegeben), nur der Name (aufgabe_firma_name,
      // 0058). "Allgemein" waere falsch: die Aufgabe hat einen Kunden, ich
      // darf nur die Akte nicht sehen.
      const kunde = (t) => !t.firma_name
        ? `<span class="caption">Allgemein</span>`
        : t.firma_sichtbar === false
          ? `<span class="caption" title="Kundenakte nicht freigegeben">${ICON.kunden} ${e(t.firma_name)}</span>`
        ...
        return `<dialog id="td-dlg-${t.id}" class="todo-dialog">
          ...
          <div class="todo-dlg-grid">
            ...
            <div><span>An wen</span><b>${t.geschaeftsfuehrer
              ...
              : "—"}</b></div>
            <div><span>Verantwortlich</span><b>${t.verantwortlich && t.verantwortlich !== u.id
              ? e(t.verantwortlich_name || "—") : "ich"}${t.besitzer && t.besitzer !== u.id
              ? ` · von ${e(t.besitzer_name || "—")}` : ""}</b></div></div>
          <h3>Was genau</h3>
          <p class="todo-dlg-text">${t.notiz ? e(t.notiz) : "—"}</p>
          ${t.firma_name && t.firma_sichtbar !== false ? `<h3>Kundenakte · ${e(t.firma_name)}</h3>
            <dl class="def">
          ...
          <div class="dialog-fuss">
            ...
            ${t.firma_id && t.firma_sichtbar !== false
          ...
          <form method="post" action="/todos/anlegen">
            ...
            <div class="feld-paar">
              ...
              ${/* Verantwortlich (05.09.2026): wer die Aufgabe erledigt. Bei einer
                    Kunden-Aufgabe entscheidet das, auf wessen Whiteboard sie
                    erscheint (Kategorie Kunden). Vorbelegt bin ich. */""}
              <div class="feld"><label>Verantwortlich</label>
                ...
                ${/* Der Hinweis steht UNTER dem Feld, nicht in der Beschriftung:
                      dort brach er dreizeilig um und schob das Feld unter den
                      Nachbarn (Pruefung 05.09.2026). */""}
