...
async function firma(name) {
  ...
  const [deals, aufgaben, letzte] = await Promise.all([
    ...
    frag("select titel, faellig from aufgaben where firma_id = $1 and not erledigt order by faellig nulls last limit 3", [f.id]),
...
async function projekt(name) {
  ...
  // Bei vielen Projekten ist der Titel der Firmenname — dann klang es im ersten
  // Live-Lauf so: "Pim Pool GmbH für Pim Pool GmbH". Die Firma wird nur genannt,
  // wenn sie im Titel NICHT schon drinsteht.
  const wem = (p) => {
    const t = String(p.titel || "").trim().toLowerCase();
    const f = String(p.firma || "").trim();
    return f && !t.includes(f.toLowerCase()) ? " für " + f : "";
  };
  // Status kommt aus der Datenbank ohne Umlaute ("laeuft") — vorgelesen klingt
  // das falsch.
  const STATUS = { laeuft: "läuft", offen: "offen", fertig: "fertig", pausiert: "pausiert", geplant: "geplant" };
  const stand = (p) => STATUS[String(p.status || "").toLowerCase()] || p.status || "läuft";

  if (!name) {
    const liste = treffer.map((p) => {
      const frist = p.deadline ? `, bis ${new Date(p.deadline).toLocaleDateString("de-DE", { day: "numeric", month: "long" })}` : "";
      return `${String(p.titel).trim()}${wem(p)} — ${stand(p)}${frist}`;
    });
    return { ok: true, reply: `${treffer.length === 1 ? "Ein Projekt läuft" : treffer.length + " Projekte laufen"}: ${liste.join(". ")}.` };
  }

  const p = treffer[0];
  const teile = [`${String(p.titel).trim()}${wem(p)}: ${stand(p)}`];
  if (p.deadline) {
...
async function contentStand() {
  ...
  if (kanal?.length) {
    // "marke" ist in der Datenbank eine Kennung, kein Name — im ersten
    // Live-Lauf las sie "Instagram ac2096dd-6df8-4215-bebe-da3b605a703a" vor.
    // Eine Kennung sagt niemandem etwas, also bleibt sie draussen.
    const echterName = (m) => (m && !/^[0-9a-f-]{20,}$/i.test(String(m)) ? " " + m : "");
    teile.push(kanal.map((k) => `${k.kanal}${echterName(k.marke)}: ${zahl(k.abonnenten)} Abos`).join(", "));
  }
