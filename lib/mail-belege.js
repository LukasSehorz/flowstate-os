...
async function suchen(tage = 2) {
  const frage = `has:attachment filename:pdf newer_than:${tage}d -label:${ETIKETT.replace(/\//g, "-")}`;
  ...
  return liste.filter((m) => RECHNUNG.test(String(m.subject || "")) || RECHNUNG.test(String(m.from || "")));
}
