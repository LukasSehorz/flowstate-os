...
// NUR BESTIMMTE ABSENDER (18.09.2026). Lukas' Hotmail ist ein privates
// Postfach, in dem ein paar geschaeftliche Ausgaben ankommen ("fuer bestimmte
// Ausgaben"). Eine Datei absender.txt neben der Zugangsdatei — je Zeile eine
// Adresse oder Domaene — macht aus "alles, was nach Rechnung klingt" ein
// "nur von diesen". Ohne Datei: wie bei jedem anderen Postfach.
function erlaubteAbsender(ordner) {
  try {
    return fs.readFileSync(path.join(ordner, "absender.txt"), "utf-8").split(/\r?\n/)
      .map((z) => z.replace(/#.*$/, "").trim().toLowerCase()).filter(Boolean);
  } catch { return []; }
}
const absenderErlaubt = (m, erlaubt) => {
  if (!erlaubt || !erlaubt.length) return true;
  const von = String(m.from || "").toLowerCase();
  return erlaubt.some((a) => von.includes(a));
};
...
function direktPostfach(k) {
  return {
    name: k.name,
    erlaubt: erlaubteAbsender(path.dirname(k.tokenPfad)),
...
// Ein Outlook.com-/Hotmail-Konto (lib/outlook-direkt.js). Graph versteht die
// Gmail-Anfrage nicht — es bekommt Zeitraum und Etikett als Werte.
function outlookPostfach(k) {
  return {
    name: k.name,
    erlaubt: erlaubteAbsender(path.dirname(k.tokenPfad)),
    suchen: (frage, max, { tage, seit } = {}) => k.suchen({ tage, seit, etikett: ETIKETT, max }),
    anhaenge: (id) => k.anhaenge(id),
    anhangHolen: (id, aid) => k.anhangHolen(id, aid),
    etikettSetzen: (id) => k.etikettSetzen(id, ETIKETT),
  };
...
// Gmail-Konten (token.json) und Outlook-Konten (outlook.json).
function allePostfaecher() {
  ...
  let outlook = null;
  try { outlook = require("./outlook-direkt.js"); } catch { outlook = null; }
  if (outlook) {
    let namen = [];
    try { namen = fs.readdirSync(gmail.POSTFAECHER).filter((n) => /^[a-z0-9_.-]+$/i.test(n)).sort(); } catch { namen = []; }
    for (const n of namen) {
      const k = outlook.konto(path.join(gmail.POSTFAECHER, n, "outlook.json"), n);
      if (k.bereit()) raus.push(outlookPostfach(k));
    }
  }
  return raus;
}
...
async function kandidaten(postfach, { tage = 2, seit = "", max = MAX_TAEGLICH } = {}) {
  const liste = await postfach.suchen(frageBauen({ tage, seit }), max, { tage, seit });
  const sicher = [], unklar = [];
  ...
  // Mit Absenderliste (absender.txt): Nur die Gelisteten, und die gelten mit
  // PDF als Rechnung — wer dort steht, steht dort, weil er Rechnungen schickt.
  const erlaubt = postfach.erlaubt || [];
  for (const m of liste) {
    if (vonUns(m)) continue;
    if (!absenderErlaubt(m, erlaubt)) continue;
    (erlaubt.length || sicherRechnung(m) ? sicher : unklar).push(m);
  }
...
module.exports = {
  laufen, suchen, kandidaten, frageBauen, passtDateiname, allePostfaecher, absenderErlaubt,
