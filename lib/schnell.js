...
async function frage(system, nutzer, { maxTokens = 400, temp = 0.4, timeoutMs = 15000, model, denken = "aus" } = {}) {
  // STUFE 3.1 (19.09.2026): SCHNELL_PROVIDER=hermes — der Kurzaufruf laeuft
  // ueber Hermes' Runs-API mit eigener Anweisung und dem kleinen Modell aus
  // dem Abo (lib/hermes.js kurz). Kein Anthropic-Schluessel mehr noetig.
  // Ein von aussen gewuenschtes Claude-Modell wird dabei ignoriert.
  if (PROVIDER === "hermes") {
    return require("./hermes.js").kurz(system, nutzer, {
      timeoutMs: Math.max(timeoutMs, 20000),
      model: model && !/^claude/i.test(String(model)) ? model : undefined,
    });
  }
...
// NICHT JEDES MODELL KENNT "effort" (Fix 20.08.2026).
//
// mitWerkzeugen() und mitWerkzeugenStrom() setzten output_config.effort IMMER.
// Bei Sonnet 5 und Opus 5 ist das richtig; Haiku 4.5 antwortet darauf mit
// HTTP 400 "This model does not support the effort parameter." — gemessen im
// Container am 20.08.:
//
//   haiku-4-5 mit effort  -> 400 nach 273 ms
//   haiku-4-5 ohne effort -> 200 nach 886 ms (mit Werkzeugaufruf)
//   sonnet-5  mit effort  -> 200 nach 2711 ms
//
// Damit war der schnellste Weg zu einem Werkzeugaufruf faktisch gesperrt: Wer
// model: "claude-haiku-4-5" uebergab, bekam immer 400 — nicht wegen seines
// Aufrufs, sondern wegen eines Feldes, das er nie gesetzt hat. Die Entscheidung
// gehoert deshalb hierher, an die eine Stelle, durch die alle laufen.
function kenntAufwand(modell) {
  return !/haiku/i.test(String(modell || ""));
}

async function denke(system, nutzer, { webSuche = true, maxSuchen = 2, maxTokens = 700, timeoutMs = 30000, aufwand } = {}) {
  // STUFE 3.1 (19.09.2026): Mit SCHNELL_PROVIDER=hermes ist die Denkspur ein
  // kurzer Auftrag an Hermes (eigene Anweisung statt SOUL.md, Terra ueber das
  // Abo, Websuche ueber Hermes' eigene Werkzeuge). Steht mit Nummer im
  // Auftragsbuch (Quelle denkspur). Gemessen 19.09.: 13 bis 30 s.
  if (PROVIDER === "hermes") {
    const hermes = require("./hermes.js");
    const r = await hermes.auftrag(String(nutzer), {
      quelle: "denkspur",
      instructions: String(system || "") + (webSuche
        ? "\n\nRecherchiere bei Bedarf im Web und nenne die Quelle. Antworte in Klartext, ohne JSON."
        : "\n\nAntworte in Klartext, ohne JSON, ohne Werkzeuge."),
      model: process.env.DENKER_MODEL_HERMES || "gpt-5.6-terra",
      fristMs: Math.max(timeoutMs * 3, 60000),
    });
    if (!r.ok) throw new Error("Denkspur (Hermes): " + (r.hint || "unbekannt"));
    return hermes.klartext(r.reply);
  }
...
async function mitWerkzeugen(system, nutzer, werkzeuge,
  ...
  const modell = model || DENKER_MODELL;
  const r = await fetch("https://api.anthropic.com/v1/messages", {
    method: "POST",
    headers: {
      "content-type": "application/json",
    ...
    body: JSON.stringify({
      ...
      ...(kenntAufwand(modell) ? { output_config: { effort: aufwand } } : {}),
      tools: werkzeuge,
      messages: [{ role: "user", content: nutzer }],
    }),
    signal: AbortSignal.timeout(timeoutMs),
...
module.exports = { frage, denke, mitWerkzeugen, mitWerkzeugenStrom, verfuegbar, kenntAufwand, SATZENDE };
