...
function zeitplan(wann, dauerMin = 45) {
  ...
  const dauer = Math.max(15, Math.min(8 * 60, Number(dauerMin) || 45));
...
function beschreibungErstgespraech(firma, notiz) {
  ...
  if (n) zeilen.push("Notiz: " + n.slice(0, 200));
  return zeilen.join("\n");
}

async function erstgespraechSynchronisieren(user, firmaId, { wann, ort = "", notiz = "", dauerMin = 45 } = {}) {
  try {
    const id = Number(firmaId);
    ...
    const beschreibung = beschreibungMitAkte(beschreibungErstgespraech(firma, notiz), id);
    ...
    if (alt) {
      const r = await werkzeuge.terminAendern({
        ...
        ort: ortText || undefined, beschreibung,
      });
    ...
    if (!f.ok) {
      ...
      await werkzeuge.terminAbsagen({ id: r.id }).catch(() => null);
...
module.exports = {
  // Vertrag mit dem CRM
  erstgespraechSynchronisieren, termineVonFirma,
