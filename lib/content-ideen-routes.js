...
module.exports = function (app) {
  ...
  app.get("/content/ideen", angemeldet, async (req, res, next) => {
    try {
      ...
      const [alle, tafel, deckung] = await Promise.all([
      ...
      // zurücksetzt.
      const url = (aend = {}) => {
        const w = { marke, kanal, rubrik: rub, ...aend };
      ...
      const karteAngenommen = (i) => `
        <div class="karte id-karte id-karte--ja">
          <div class="id-kopf">
            <div class="id-marken">${stufeMarke(i)}${rubrikMarke(i)}
              <span class="badge">${e(i.kanal)}</span></div>
            <a href="/content/idee/${i.id}" class="knopf sekundaer klein">${
              i.skript ? "Skript & auf die Tafel" : "Skript schreiben"} →</a>
          </div>
          <h3 class="id-titel">${e(i.titel)}</h3>
          <p class="caption id-fuss">angenommen ${datum(i.entschieden_am)} ·
            ${i.skript ? "Skript liegt vor" : "Skript fehlt noch"}</p>
        </div>`;
      ...
      const kaertchen = (p) => {
        ...
        return `<article class="pt-karte${ueberfaellig ? " pt-karte--spaet" : ""}" draggable="true" data-id="${p.id}">
          ...
          <h4 class="pt-titel">${e(p.titel)}</h4>
          <div class="pt-zeile">
      ...
      res.send(schale({
        ...
        ${vorschlaege.length
          ? `<div class="id-liste">${vorschlaege.map(karteVorschlag).join("")}</div>`
          : `<div class="karte"><p class="caption">Keine offenen Vorschläge${
        ...
        <h2 class="id-abschnitt">Produktionstafel</h2>
        <p class="pt-anleitung">${ICON.info} Kärtchen mit der Maus in die nächste Spalte ziehen —
          das wird sofort gespeichert. Auf jedem Kärtchen steht, für wann es geplant ist.</p>
        <div class="pt-tafel">${ct.STUFEN.map(spalte).join("")}</div>

        <!-- Die Deckung: die Frage, ob überhaupt neue Ideen gebraucht werden. -->
        <p class="pt-deckung">
          ${deckung.bis
            ? `Videos geplant bis <strong>${datum(deckung.bis)}</strong> — das sind
               ${gedeckt} Tag${gedeckt === 1 ? "" : "e"} Vorlauf bei ${deckung.ab_heute} offenen Videos.`
            : `Noch nichts terminiert — es gibt keinen Vorlauf.`}
          ${luecken.length
            ? `<span class="pt-deckung-luecke">${luecken.length} Tag${luecken.length === 1 ? "" : "e"}
               unter ${PRO_TAG} Videos: ${luecken.slice(0, 6).map((t) => datumKurz(t.tag)).join(", ")}${
               luecken.length > 6 ? " …" : ""}</span>`
            : (deckung.bis ? `<span class="pt-deckung-voll">Jeder Tag ist mit ${PRO_TAG} Videos gedeckt.</span>` : "")}
        </p>
        ...
        <div class="karte id-holen">
          ...
          <p class="caption id-holen-fuss">${ICON.info}<span>
            Der Abendlauf um ${process.env.IDEEN_STUNDE || 19} Uhr <strong>setzt von selbst aus</strong>,
            solange ${VORLAUF_TAGE} Tage im Voraus gedeckt sind oder ${OFFEN_GENUG} Vorschläge
            unentschieden liegen — es hat keinen Sinn, Ideen auf Ideen zu stapeln.
            ${genugVorrat
              ? `<strong>Gerade ist das der Fall</strong>, es wird also nicht automatisch gesucht.`
              : `Gerade läuft er.`}
        ...
        document.getElementById('hl-datei').addEventListener('change', function(){
          ...
          bar.hidden = false; innen.style.width = '0%';
          ...
          x.upload.onprogress = function(ev){
            if (ev.lengthComputable) innen.style.width = Math.round(ev.loaded / ev.total * 100) + '%';
          };
          x.onload = function(){
            if (x.status === 200) { innen.style.width = '100%'; bar.classList.add('fertig'); }
  ...
  app.post("/content/idee/entscheiden", angemeldet, formular, async (req, res, next) => {
    try {
      ...
      if (b.status === "angenommen" && ideen.bereit()) {
        ...
        return res.redirect("/content/idee/" + encodeURIComponent(b.id));
      }
