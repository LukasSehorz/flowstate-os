...
const FEHLERTEXT = {
  ...
  "nicht-verworfen": "Diese Idee liegt gar nicht im Archiv — die Seite war vermutlich veraltet.",
...
module.exports = function (app) {
  ...
  app.get("/content/ideen", angemeldet, async (req, res, next) => {
    try {
      ...
      // Zwei verschiedene Dinge, die bisher in einem Topf lagen:
      //   zurueckgestellt  vom Abendlauf aussortiert, weil an dem Tag etwas
      //                    Staerkeres kam. NICHT schlecht — nur die schwaechere
      //                    von zweien. Der Vorrat fuer Wochen mit mehr Zeit.
      //   abgelehnt        von Hand mit X verworfen. Soll nicht wiederkommen.
      const verworfen = alle.filter((i) => i.status === "verworfen" && passt(i));
      const zurueckgestellt = verworfen.filter((i) => i.verworfen_art === "ersetzt");
      const abgelehnt = verworfen.filter((i) => i.verworfen_art !== "ersetzt");
      ...
      // ---------------------------------------------- Archiv
      //
      // Verworfene bleiben stehen und bleiben ERREICHBAR. Ohne den Weg zurueck
      // waere "bleibt stehen" nur eine hoefliche Umschreibung fuer geloescht.
      const karteVerworfen = (i) => `
        <div class="karte id-karte id-karte--weg">
          <div class="id-kopf">
            <div class="id-marken">${stufeMarke(i)}${rubrikMarke(i)}
              <span class="badge">${e(i.kanal)}</span></div>
            <form method="post" action="/content/idee/zurueckholen" class="id-form">
              <input type="hidden" name="id" value="${i.id}">
              <input type="hidden" name="zurueck" value="${e(url())}">
              <button type="submit" class="sekundaer klein"
                title="Zurück zu den offenen Vorschlägen">↩ Zurückholen</button>
            </form>
          </div>
          ${i.ueberschrift && i.ueberschrift !== i.titel
            ? `<span class="id-etikett">${e(i.ueberschrift)}</span>` : ""}
          <h3 class="id-titel">${e(i.titel)}</h3>
          <details class="id-mehr">
            <summary>Ablauf und Begründung</summary>
            <p class="id-text">${e(i.idee)}</p>
            ${i.warum ? `<div class="id-warum"><span>Warum das zieht</span><p>${e(i.warum)}</p></div>` : ""}
            <p class="caption id-fuss">${i.verworfen_art === "ersetzt" ? "zurückgestellt" : "abgelehnt"}
              ${datum(i.entschieden_am)}</p>
          </details>
        </div>`;
      ...
      res.send(schale({
        ...
        ${hinweis === "zurueckgeholt" ? `<div class="hinweis erfolg">${ICON.check}<div>
          Liegt wieder oben bei „Warten auf deine Entscheidung".</div></div>` : ""}
        ...
        <p class="pt-deckung">
          ${(() => {
            ...
            // Kein zusaetzlicher Punkt hinter dem Datum: datumKurz endet schon
            // auf einen ("02.08."), sonst steht dort "02.08. .".
        ...
        <h2 class="id-abschnitt">Zurückgestellt <span>${zurueckgestellt.length}</span></h2>
        <p class="id-archiv-hinweis">${ICON.info} Diese hat der Abendlauf aussortiert, weil an
          dem Tag etwas Stärkeres kam — sie waren <strong>nicht schlecht</strong>, nur die
          schwächere von zweien. Wenn ihr mehr vorproduzieren könnt, ist das hier der Vorrat.</p>
        <div class="id-liste">${zurueckgestellt.slice(0, 24).map(karteVerworfen).join("")}</div>` : ""}

        ${abgelehnt.length ? `
        <h2 class="id-abschnitt">Von dir abgelehnt <span>${abgelehnt.length}</span></h2>
        <p class="id-archiv-hinweis">${ICON.info} Bleiben stehen und gehen in den nächsten
          Auftrag ein, damit dieselbe Idee nicht in zwei Wochen wieder auf dem Tisch liegt.
          Zurückholen geht trotzdem.</p>
        <div class="id-liste">${abgelehnt.slice(0, 24).map(karteVerworfen).join("")}</div>` : ""}
  ...
  app.post("/content/idee/zurueckholen", angemeldet, formular, async (req, res, next) => {
    try {
      const b = req.body || {};
      const zurueck = sicherZurueck(b.zurueck);
      const trenn = zurueck.includes("?") ? "&" : "?";
      const r = await ideen.zurueckholen(req.nutzer, b.id);
      if (!r.ok) return res.redirect(zurueck + trenn + "fehler=" + encodeURIComponent(r.grund));
      res.redirect(zurueck + trenn + "hinweis=zurueckgeholt");
    } catch (err) { next(err); }
  });
