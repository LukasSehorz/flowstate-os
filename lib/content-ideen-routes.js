...
module.exports = function (app) {
  ...
  app.get("/content/ideen", angemeldet, async (req, res, next) => {
    try {
      ...
      const tag = (/^\d{4}-\d{2}-\d{2}$/.test(req.query.tag || "") || req.query.tag === "spaet")
        ? req.query.tag : null;

      const [alle, tafel, deckung, plan] = await Promise.all([
        ...
        <div class="karte id-karte id-karte--ja">
          <div class="id-kopf">
            ...
            <div class="id-tasten">
              ...
              <form method="post" action="/content/idee/auf-tafel" class="id-form">
                <input type="hidden" name="id" value="${i.id}">
                <input type="hidden" name="zurueck" value="${e(url())}">
                <button type="submit" class="id-ja" ${i.skript ? "" : "disabled"}
                  title="${i.skript ? "Auf die Produktionstafel — Termin wird automatisch vergeben" : "Erst wenn das Skript da ist"}">${ICON.check}</button>
              </form>
            </div>
          </div>
          <h3 class="id-titel">${e(i.titel)}</h3>
      ...
      res.send(schale({
        ...
        <h2 class="id-abschnitt">Produktionstafel${
          tag ? ` <span class="id-tagfilter">nur ${datumKurz(tag)} · <a href="${
            e(url({ tag: null }))}">alle Tage zeigen</a></span>` : ""}</h2>
        ...
        <div class="tp-leiste">
          ...
          ${deckung.ueberfaellig ? `<a class="tp-tag tp-spaet${
            tag === "spaet" ? " tp-gewaehlt" : ""}" href="${e(url({ tag: tag === "spaet" ? null : "spaet" }))}"
          ...
          ${plan.map((d) => {
            ...
            return `<a class="tp-tag tp-${zustand}${heute ? " tp-heute" : ""}${
              tag === key ? " tp-gewaehlt" : ""}"
              href="${e(url({ tag: tag === key ? null : key }))}"
        ...
        <p class="pt-anleitung">${ICON.info} Kärtchen mit der Maus in die nächste Spalte ziehen —
          das wird sofort gespeichert. Ein Klick auf einen Tag oben zeigt nur dessen Videos.</p>
  ...
  app.post("/content/idee/auf-tafel", angemeldet, formular, async (req, res, next) => {
    try {
      ...
      const frei = await ct.naechsterFreierTag(req.nutzer, i.marke, PRO_TAG);
      const r = await ideen.inPlanUebernehmen(req.nutzer, b.id, frei);
      if (!r.ok) return res.redirect(zurueck + trenn + "fehler=" + encodeURIComponent(r.grund));
      res.redirect(zurueck + trenn + "hinweis=auf-tafel");
    } catch (err) { next(err); }
  });
