...
//   - VERKNUEPFUNGEN (06.09.2026): eine Zelle mit { wert, link } wird
//     anklickbar — blau (#0563C1) und unterstrichen, wie Excel es selbst
//     macht. Ziel darf ein relativer Pfad ("Belege/0042 Hetzner.pdf") oder
//     eine Adresse ("https://…") sein.
...
//   - Verknuepfungen stehen NICHT in der Zelle, sondern in einem eigenen
//     <hyperlinks>-Block am Ende des Blattes; das Ziel selbst liegt noch eine
//     Ebene tiefer in xl/worksheets/_rels/sheetN.xml.rels. Die Zelle nennt nur
//     eine Beziehungs-Id (r:id="rId1"). Das ist der Umweg, den OOXML
//     vorschreibt — eine Zelle kann kein Ziel tragen.
//   - Relative Ziele UND Web-Ziele laufen beide ueber TargetMode="External".
//     "Internal" waere ein Sprung innerhalb der Mappe (ein anderes Blatt),
//     nicht das, was hier gebraucht wird.
//
// Aufruf:
...
//              null (leer) oder { wert, typ, fett, stil, link, hinweis }
//              link    Ziel der Verknuepfung (relativ oder https://…)
//              hinweis Text, den Excel beim Zeigen einblendet (tooltip)
...
// Das Ziel einer Verknuepfung als URI. Das Target-Attribut in einer
// .rels-Datei ist laut OPC ein URI, kein Dateiname — ein rohes Leerzeichen
// oder ein Umlaut darf dort nicht stehen. Zwei Faelle:
//
//   Absolute Adresse ("https://…", "mailto:…"): Aufbau nicht anfassen, sonst
//   wuerde aus "://" ein "%3A%2F%2F" und der Link zeigt ins Leere. Nur das
//   Leerzeichen muss weg.
//
//   Relativer Pfad ("Belege/Rechnung März & Co.pdf"): jeder Teil zwischen den
//   Schraegstrichen einzeln kodiert, die Schraegstriche bleiben Trenner (sonst
//   waere der Unterordner Teil des Dateinamens). Aus dem Leerzeichen wird
//   %20, aus "ä" %C3%A4, aus "&" %26 — dieselbe Kodierung, die Excel selbst
//   schreibt, wenn man dort eine Verknuepfung auf eine Datei anlegt.
//
// Leere Teile fliegen raus: "Belege//x.pdf" und "/Belege/x.pdf" wuerden sonst
// aus dem entpackten Ordner herauszeigen.
function zielUri(ziel) {
  const s = String(ziel ?? "").replace(/[\u0000-\u001F\u007F]/g, "").trim();
  if (!s) return "";
  if (/^[A-Za-z][A-Za-z0-9+.-]*:/.test(s)) return s.replace(/ /g, "%20");
  return s.split("/").filter((t) => t !== "" && t !== "." && t !== "..")
...
const STIL = {
  text: 0, kopf: 1, geld: 2, datum: 3, prozent: 4, geldFett: 5, titel: 6, fett: 7, hinweis: 8,
  link: 9, zahl: 0,
};
...
//   links  Sammelliste des Blattes; eine Zelle mit { link } traegt sich hier
//          ein. Erst nach der Leer-Pruefung — eine leere Zelle wird gar nicht
//          geschrieben, und eine Verknuepfung auf eine Zelle, die es nicht
//          gibt, ist genau die Art Fehler, bei der Excel "Reparieren?" fragt.
function zelle(ref, wert, typ = "text", links = null) {
  let v = wert, t = typ, fett = false, stil = null, link = null, hinweis = "";
  if (v && typeof v === "object" && !(v instanceof Date)) {
    ({ wert: v, typ: t = typ, fett = false, stil = null, link = null, hinweis = "" } = v);
  }
  if (v === null || v === undefined || v === "") return "";
  const ziel = links ? zielUri(link) : "";
  if (ziel) {
    links.push({ ref, ziel, hinweis });
    // Blau und unterstrichen — aber nur, wenn die Zelle nicht ohnehin einen
    // eigenen Stil mitbringt (dann hat der Aufrufer sich etwas dabei gedacht).
    if (!stil && typeof v !== "number" && !(v instanceof Date) && t !== "datum") stil = "link";
  }
...
function blattXml(blatt, erstes) {
  ...
  const zeileSchreiben = (werte, typen, stilAlle) => {
    ...
    const zellen = werte.map((w, i) => {
      ...
      return zelle(ref, w, typen ? (typen[i] || "text") : "text", links);
    }).filter(Boolean);
  ...
  // Die Reihenfolge der Elemente ist in OOXML vorgeschrieben (CT_Worksheet):
  // sheetData, dann autoFilter, dann hyperlinks, dann pageMargins. Steht der
  // Block an der falschen Stelle, faellt Excel beim Oeffnen ueber das Schema —
  // "Reparieren?". Ohne Verknuepfungen gar kein Element: ein leeres
  // <hyperlinks/> ist laut Schema ungueltig (minOccurs=1 fuer hyperlink).
  const verknuepfungen = links.length
    ? `<hyperlinks>${links.map((l, i) =>
        `<hyperlink ref="${l.ref}" r:id="rId${i + 1}"${l.hinweis ? ` tooltip="${x(l.hinweis)}"` : ""}/>`).join("")}</hyperlinks>`
    : "";
...
// Die Beziehungsdatei eines Blattes: hier steht, wohin jede Verknuepfung
// zeigt. TargetMode="External" fuer beides — den relativen Pfad in den
// entpackten Ordner und die Adresse im Netz.
function blattRelsXml(links) {
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
${links.map((l, i) =>
  `<Relationship Id="rId${i + 1}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink" Target="${x(l.ziel)}" TargetMode="External"/>`).join("\n")}
</Relationships>`;
}
...
module.exports.bauen = function ({ blaetter = [], autor = "Flowstate OS", erstellt = new Date() } = {}) {
  ...
  // Blaetter einmal bauen (nicht zweimal — die Verknuepfungen entstehen dabei
  // und muessen zu genau dem XML passen, das ins Paket geht).
  const gebaut = blaetter.map((b, i) => blattXml(b, i === 0));

  // [Content_Types].xml: Die Beziehungsdateien der Blaetter brauchen KEINEN
  // eigenen Override — der <Default Extension="rels"> deckt jede .rels-Datei
  // im ganzen Paket ab, egal in welchem Ordner sie liegt. Ein zusaetzlicher
  // Override waere nicht falsch, aber doppelt gemoppelt.
...
<Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties"><Application>${x(autor)}</Application></Properties>`;
  ...
  const eintraege = [
    ...
    ...gebaut.map((g, i) => ({ name: `xl/worksheets/sheet${i + 1}.xml`, daten: g.xml })),
    // Nur Blaetter MIT Verknuepfungen bekommen eine Beziehungsdatei. Eine
    // leere .rels waere gueltig, aber Ballast — und jeder Teil im Paket, den
    // niemand braucht, ist eine Stelle, an der etwas kaputtgehen kann.
    ...gebaut.flatMap((g, i) => (g.links.length
      ? [{ name: `xl/worksheets/_rels/sheet${i + 1}.xml.rels`, daten: blattRelsXml(g.links) }] : [])),
  ];
...
module.exports.zielUri = zielUri;
