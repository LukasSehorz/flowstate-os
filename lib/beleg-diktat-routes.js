...
//
// Seiten
//   GET  /buchhaltung/rechnungen/chat              das Gespräch (der Hauptweg)
//   POST /buchhaltung/rechnungen/chat              eine Äußerung, dann Umleitung
//   GET  /buchhaltung/rechnungen/diktat            das eine Textfeld (Vorläufer)
...
const bc = require("./beleg-chat.js");
...
const FEHLERTEXTE = {
  ...
  "migration-chat": "<b>Das Gespräch lässt sich hier noch nicht speichern.</b> In der Datenbank fehlt Migration 0063 — einspielen mit <code>node scripts/migrieren.js --einspielen</code>.",
...
const migrationFehlt = (err) => /relation "(rechnungen|beleg_links|beleg_versand|beleg_chat)" does not exist/.test(String(err && err.message));
...
<p>Links auf ein Dokument gelten ${bv.LINK_TAGE} Tage. Melden Sie sich gerne bei uns — wir schicken Ihnen das Dokument erneut.</p>
  ...
  // =====================================================================
  // Das Gespräch — ein Faden, ein Beleg
  //
  // Warum diese Seite den Weg über das Diktat ablöst (Lukas, 05.09.2026):
  // "Wenn ich später etwas an der Rechnung ändern will, soll die bestehende
  // Rechnung genommen, angepasst und wieder abgelegt werden." Das Diktat
  // konnte einen Satz in einen Entwurf verwandeln und war danach fertig; ab
  // dem zweiten Satz musste man ins Formular. Hier bleibt der Faden am Beleg.
  //
  // Serverseitig gerendert wie alles im Haus: Absenden ist ein POST, danach
  // eine Umleitung auf ?beleg=<id> (Post/Redirect/Get). Wer die Seite neu
  // lädt, schickt damit nichts zweimal ab — bei einem Vorgang, der Geld auf
  // ein Blatt schreibt, ist das keine Feinheit. Das bisschen JavaScript auf
  // der Seite macht nur, was ohne es unbequem wäre: Enter schickt ab, die
  // Beispiele füllen das Feld, der Verlauf springt nach unten.
  // =====================================================================
  const CHAT_URL = "/buchhaltung/rechnungen/chat";

  // Die zwei Beispiele in der ersten Nachricht sind ECHTE Sätze aus Lukas'
  // Alltag, keine Feldnamen: Wer zum ersten Mal vor einem leeren Feld sitzt,
  // braucht einen Satz zum Nachbauen.
  const CHAT_BEISPIELE = [
    "Rechnung für Anderka GmbH, Website 2.500 €",
    "Angebot für Elektro Albonni, Website 500 €",
  ];

  const uhr = new Intl.DateTimeFormat("de-DE", { hour: "2-digit", minute: "2-digit", timeZone: "Europe/Berlin" });
  const zeitKurz = (iso) => { const d = new Date(iso); return Number.isFinite(d.getTime()) ? uhr.format(d) : ""; };
  const TAT_WORT = { anlegen: "Beleg angelegt", aendern: "Beleg geändert", "art-wechsel": "Art gewechselt" };

  // Eine Blase. Ich rechts, das System links — und darunter, was dabei mit
  // dem Beleg passiert ist. Ohne diese Zeile wäre im Verlauf später nicht
  // mehr zu sehen, welcher Satz welche Zahl bewegt hat.
  function blase(n) {
    const ich = n.rolle === "nutzer";
    const tat = n.tat && typeof n.tat === "object" ? n.tat : null;
    const felder = tat && Array.isArray(tat.felder) ? tat.felder.filter(Boolean) : [];
    return `<div class="dk-blase ${ich ? "dk-ich" : "dk-os"}">${e(n.text)}</div>`
      + (ich && n.zeit ? `<div class="dk-zeit">${e(zeitKurz(n.zeit))}</div>` : "")
      + (!ich && tat ? `<div class="dk-tat">${ICON.check}<span>${e(TAT_WORT[tat.art] || "Beleg geändert")}${felder.length ? ` · ${e(felder.join(" · "))}` : ""}</span></div>` : "");
  }

  // Der Beleg rechts: Kurzfassung, PDF-Vorschau, die Knöpfe von D1.
  function belegTafel(r, letztes) {
    if (!r) {
      return `<div class="os-leer os-leer--klein"><div class="os-leer-icon">${ICON.beleg}</div>
        <h3>Noch kein Beleg</h3>
        <p>Sag links, für wen er ist, was drauf soll und was es kostet — dann steht er hier, mit Vorschau, und du kannst ihn im selben Gespräch weiter ändern.</p>
        ${letztes && letztes.beleg ? `<p class="rg-sub">Zuletzt besprochen: <a href="${CHAT_URL}?beleg=${letztes.beleg.id}">${e(artWort(letztes.beleg.art))} ${e(nummerText(letztes.beleg))} · ${e(letztes.beleg.empfaenger.name || "ohne Kunde")}</a></p>` : ""}</div>`;
    }
    const basis = `/buchhaltung/rechnungen/${r.id}`;
    const entwurf = r.status === "entwurf";
    // Cache-Stempel aus dem Änderungszeitpunkt: Nach jeder Änderung ist die
    // Adresse eine andere, also lädt der Rahmen wirklich neu. (Die PDF-Route
    // schickt bei Entwürfen zwar "no-store" mit — aber genau daran soll die
    // Vorschau nicht hängen.)
    const stempel = Number(new Date(r.geaendert)) || Date.now();
    const zeile = (label, wert) =>
      `<div class="os-zeile os-zeile--text"><span class="os-zeile-label">${e(label)}</span>
        <div class="os-zeile-wert" data-leer="—">${wert || ""}</div></div>`;
    const pos = (r.positionen || []).map((p, i) => `<li><span>${i + 1}. ${e(p.titel || "ohne Titel")}</span>
      <span class="rg-betrag">${geld(rg.positionSumme(p))}</span></li>`).join("");

    return `<section class="os-gruppe">
        <div class="os-gruppe-titel"><span>${e(artWort(r.art))} ${e(nummerText(r))}</span>
          <span class="os-gruppe-werkzeug"><a href="${basis}">Vorgang öffnen</a></span></div>
        <div class="os-gruppe-rumpf">
          ${zeile("Status", `<span class="os-pille os-pille--${e(r.status_farbe || "grau")} os-pille--punkt">${e(r.status_text || r.status)}</span>`)}
          ${zeile("Kunde", e(r.empfaenger.name || "") + (r.empfaenger.ansprechperson ? ` <span class="rg-sub">${e([r.empfaenger.anrede, r.empfaenger.ansprechperson].filter(Boolean).join(" "))}</span>` : ""))}
          ${zeile("Belegdatum", datum(r.datum))}
          ${zeile(r.art === "angebot" ? "Gültig bis" : "Fällig am", datum(r.faellig))}
          ${zeile("Positionen", pos ? `<ul class="dk-posliste">${pos}</ul>` : "")}
          ${zeile("Summe", `<b>${geld(r.summe)}</b>`)}
        </div>
      </section>
      <div class="dk-pdf">
        <iframe src="${basis}.pdf?v=${stempel}#toolbar=0&navpanes=0&view=FitH" title="Vorschau ${e(artWort(r.art))} ${e(nummerText(r))}" loading="lazy"></iframe>
        <div class="dk-pdf-fuss">${entwurf ? "Entwurf — noch ohne Nummer. Das endgültige PDF entsteht beim Stellen." : "Eingefrorenes PDF, so wie es beim Kunden liegt."}</div>
      </div>
      <div class="dk-aktionen">
        ${entwurf ? `<form method="post" action="${basis}/stellen"><button type="submit" class="dunkel">${ICON.check} ${e(r.art === "angebot" ? "Angebot stellen" : "Rechnung stellen")}</button></form>` : ""}
        <a class="rg-knopf sekundaer" href="${basis}/nachricht">${ICON.mail} Nachricht schreiben</a>
        <a class="rg-knopf dk-still" href="${entwurf ? `${basis}/bearbeiten` : basis}">${ICON.stift} Im Formular öffnen</a>
      </div>`;
  }

  function chatSeite(u, { chat, beleg, letztes = null, q = {} }) {
    const modellDa = bc.bereit();
    const verlauf = chat && Array.isArray(chat.nachrichten) ? chat.nachrichten : [];
    const chatId = chat ? chat.id : "";
    const belegId = beleg ? beleg.id : "";

    // Die erste Nachricht des Systems steht immer da — auch in einem
    // laufenden Gespräch. Sie ist die Gebrauchsanweisung, und die soll man
    // nicht wegscrollen müssen, um sie wiederzufinden.
    const start = `<div class="dk-blase dk-os">Sag mir in einem Satz, für wen der Beleg ist, was drauf soll und was es kostet — ich lege ihn an. Danach ändern wir ihn hier weiter: „mach 900 daraus", „setz das Datum auf den 1.", „schreib noch eine Position Hosting 120 € dazu".
      <div class="dk-beispiele">${CHAT_BEISPIELE.map((b) =>
        `<button type="button" class="dk-beispiel-knopf" data-satz="${e(b)}">${e(b)}</button>`).join("")}</div></div>`;

    return seite(u, "Beleg im Gespräch", beleg ? `${artWort(beleg.art)} ${nummerText(beleg)} · ${beleg.empfaenger.name || ""}` : "Rechnung oder Angebot, im Gespräch geschrieben", `
      <div class="os-kopf"><div><div class="os-kopf-titel">Rechnung oder Angebot im Gespräch</div>
        <div class="os-kopf-unter">Ein Gespräch gehört zu einem Beleg. Was du hier sagst, ändert genau diesen einen — es entsteht kein zweiter.</div></div>
        <div class="os-kopf-aktionen">
          ${chatId || belegId ? `<a class="rg-knopf sekundaer" href="${CHAT_URL}">${ICON.neu} Neuer Beleg</a>` : ""}
          <a class="rg-knopf sekundaer" href="/buchhaltung/rechnungen">Alle Vorgänge</a>
          <a class="rg-knopf dk-still" href="/buchhaltung/rechnungen/neu">${ICON.stift} Formular</a></div></div>
      ${hinweise(q)}
      ${modellDa ? "" : `<div class="os-hinweis os-hinweis--warn">${ICON.warnung}<div><b>Ohne Sprachmodell kann ich aus deinem Text keinen fertigen Beleg bauen</b> — auf dem Server läuft es.
        Du kannst trotzdem schreiben: Der erste Satz wird als Notiz an einem leeren Entwurf gespeichert, die Zahlen trägst du dann im Formular nach.</div></div>`}

      <div class="dk-chat">
        <section class="dk-gespraech">
          <div class="dk-verlauf" id="dk-verlauf">${start}${verlauf.map(blase).join("")}</div>
          <form method="post" action="${CHAT_URL}" class="dk-schreiben" id="dk-form">
            <input type="hidden" name="chat" value="${e(chatId)}">
            <input type="hidden" name="beleg" value="${e(belegId)}">
            <textarea name="text" id="dk-text" rows="2" autofocus
              placeholder="${beleg ? "Was soll anders werden?" : e(CHAT_BEISPIELE[0])}"></textarea>
            <button type="submit" class="dunkel" id="dk-senden">${ICON.senden} Senden</button>
          </form>
          <div class="dk-tastenhilfe">Enter schickt ab, Umschalt+Enter macht eine neue Zeile.</div>
        </section>
        <aside class="dk-beleg">${belegTafel(beleg, letztes)}</aside>
      </div>

      <script>
      (function(){
        var f=document.getElementById("dk-form"), t=document.getElementById("dk-text"),
            v=document.getElementById("dk-verlauf"), k=document.getElementById("dk-senden");
        // Beim Öffnen unten stehen: Das Neueste ist das, was interessiert.
        ...
        if(t){
          t.addEventListener("keydown",function(ev){
            // Umschalt+Enter macht eine Zeile. isComposing schützt die
            // Umlaut-Eingabe auf fremden Tastaturen vor einem Fehlstart.
            if(ev.key==="Enter"&&!ev.shiftKey&&!ev.isComposing){ev.preventDefault();absenden();}
          });
          // Das Feld wächst mit, bis 220 px — danach rollt es.
          t.addEventListener("input",function(){t.style.height="auto";t.style.height=Math.min(t.scrollHeight,220)+"px";});
        }
        Array.prototype.forEach.call(document.querySelectorAll(".dk-beispiel-knopf"),function(b){
          b.addEventListener("click",function(){t.value=b.getAttribute("data-satz");t.style.height="auto";t.style.height=Math.min(t.scrollHeight,220)+"px";t.focus();});
        });
        // Zweimal abschicken heißt zweimal denken lassen und zweimal warten.
        // Der Knopf sagt außerdem, dass etwas läuft — ein Modellaufruf mit
        // Werkzeugen dauert ein paar Sekunden, und eine Seite, die dabei
        // stumm bleibt, sieht kaputt aus.
        if(f){f.addEventListener("submit",function(){
          if(f.dataset.laeuft){return false;}
          f.dataset.laeuft="1";
          if(k){k.disabled=true;k.textContent="Einen Moment …";}
        });}
      })();
      </script>`);
  }

  app.get("/buchhaltung/rechnungen/chat", nurAdmin, async (req, res, next) => {
    try {
      const u = req.nutzer;
      const belegId = /^\d+$/.test(String(req.query.beleg || "")) ? String(req.query.beleg) : "";
      const chatId = /^\d+$/.test(String(req.query.chat || "")) ? String(req.query.chat) : "";
      let chat = null, letztes = null;
      try {
        // Ein GET schreibt nichts. Kommt jemand über "Im Chat ändern" auf
        // einen Beleg ohne Gespräch, bleibt die Seite leer und der erste
        // Satz legt den Faden an (siehe POST) — sonst entstünden Chatzeilen
        // allein durchs Hinschauen.
        if (belegId || chatId) chat = await bc.chatHolen(u, { chatId, rechnungId: belegId });
        else {
          letztes = await bc.letztesGespraech(u);
          // Der Leerzustand nennt den Beleg beim Namen statt bei der Nummer —
          // "Rechnung Entwurf · Anderka GmbH" sagt einem Menschen etwas,
          // "Vorgang 334" nicht.
          if (letztes && letztes.rechnung_id) letztes.beleg = await rg.eine(u, letztes.rechnung_id);
        }
      } catch (err) {
        if (!migrationFehlt(err)) throw err;
        return res.redirect(`${CHAT_URL}?fehler=migration-chat`);
      }
      const id = chat && chat.rechnung_id ? chat.rechnung_id : (belegId || "");
      const beleg = id ? await rg.eine(u, id) : null;
      res.send(chatSeite(u, { chat, beleg, letztes, q: req.query }));
    } catch (err) { next(err); }
  });

  app.post("/buchhaltung/rechnungen/chat", nurAdmin, async (req, res, next) => {
    try {
      const u = req.nutzer;
      const b = req.body || {};
      const text = String(b.text || "").slice(0, bc.MAX_TEXT);
      const chatId = /^\d+$/.test(String(b.chat || "")) ? String(b.chat) : "";
      const belegId = /^\d+$/.test(String(b.beleg || "")) ? String(b.beleg) : "";
      const zurueck = (zusatz = {}) => {
        const p = new URLSearchParams();
        if (belegId) p.set("beleg", belegId); else if (chatId) p.set("chat", chatId);
        for (const [k, w] of Object.entries(zusatz)) p.set(k, w);
        const s = p.toString();
        return s ? `${CHAT_URL}?${s}` : CHAT_URL;
      };
      if (!text.trim()) return res.redirect(zurueck({ fehler: "leer" }));

      let erg;
      try { erg = await bc.antworten(u, { text, chatId, rechnungId: belegId }); }
      catch (err) {
        if (!migrationFehlt(err)) throw err;
        return res.redirect(`${CHAT_URL}?fehler=migration-chat`);
      }
      // Nach dem Zug hängt der Faden am Beleg — von da an genügt ?beleg=<id>.
      const p = new URLSearchParams();
      if (erg.rechnungId) p.set("beleg", String(erg.rechnungId));
      else if (erg.chatId) p.set("chat", String(erg.chatId));
      res.redirect(`${CHAT_URL}?${p.toString()}`);
    } catch (err) { next(err); }
  });
